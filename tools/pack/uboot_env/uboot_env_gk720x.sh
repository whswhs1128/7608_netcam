...
nfsboot=setenv bootargs console=\$\{consoledev\},\$\{baudrate\} root=/dev/nfs rw nfsroot=\$\{nfsserver\}:\$\{rootpath\} ip=\$\{ipaddr\}::\$\{gatewayip\}:\$\{netmask\}:\$\{ethact\}:\$\{netdev\}:off init=/linuxrc mem=\$\{mem\} phytype=\$\{phytype\}, mtdparts=\$\{mtdparts\}; sf probe 0 0;sf read 0xC1000000 0x0B0000 0x1A0000;bootm\r
mtdparts=gk_flash:256k(Uboot),64k(Envs),384k(Config),1664k(Kernel),2944k(Rootfs),2816k(Resource),64k(User),8M@0(all)\r
phytype=${phytype}\r
...
sfboot=setenv bootargs console=\$\{consoledev\},\$\{baudrate\} initrd=0xC11A0000,0x2e0000 root=/dev/ram init=/linuxrc mem=\$\{mem\} phytype=\$\{phytype\}, mtdparts=\$\{mtdparts\}; sf probe 0 0;sf read 0xC1000000 0x0B0000 0x1A0000;sf read 0xC11A0000 0x250000 0x2e0000; bootm\r
sfkernel=0x40000\r
stderr=serial\r
stdin=serial\r
stdout=serial\r
