...
# produce mini rootfs 
mkdir -p ${ROOTFS_DIR}
tar -xf ${SDK_TOP}/linux/rootfs/rootfs_uclibc.tgz -C ${ROOTFS_DIR}
...
if  [ $sensordetect -eq 0 ]; then
	sed -e "s/REPSENSOR/${sensor}/g" -e "s/REPBOARD/${board}/g" script/startup_his.sh >${ROOTFS_DIR}/usr/local/bin/startup.sh
else
	sed -e "s/REPSENSOR/ /g" -e "s/REPBOARD/ /g" script/startup_his.sh >${ROOTFS_DIR}/usr/local/bin/startup.sh
...
if [ "$wifi"x != ""x ];then
	...
	#wifi device support
	if [ $board = "his3516ev300_imx335" ];then
		echo -e "add his3516ev300_imx335 wifi  ...\n"
		tar -jxf other/rootfs_uClibc_${wifi}_his.tar.bz2 -C ${ROOTFS_DIR}
		cp -f imgtools/libiw.so.29 ${ROOTFS_DIR}/lib/
		cp -f imgtools/udhcpc ${ROOTFS_DIR}/sbin/udhcpc
		cp -f imgtools/udhcpd ${ROOTFS_DIR}/usr/sbin/udhcpd
		cp -f script/wifi_his.sh ${ROOTFS_DIR}/usr/local/bin/wifi.sh
	else
...
mv ${ROOTFS_DIR}/mpp ${ROOTFS_DIR}/opt/resource
sudo chmod 777 ${ROOTFS_DIR}/opt/resource/mpp/*
ln -sv /opt/resource/mpp ${ROOTFS_DIR}/mpp
...
rm -rf ${ROOTFS_DIR}/mpp
mv ${ROOTFS_DIR}/opt/resource/mpp ${ROOTFS_DIR}/mpp
