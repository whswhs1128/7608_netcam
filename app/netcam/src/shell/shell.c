...
static void local_msg_client_connect(void *data)
{
    ...
    while(1) {
        ...
        while(1) {
            ...
            if (num != sizeof(ch)) {
                perror("cannot read client data1");
                break;
            }
            if (ch == 0) {
                perror("cannot read client data2");
                break;
            } else {
                fputc(ch, stderr);
            }
        ...
        if (num != sizeof(msg_ret)) {
            perror("cannot read client data3");
            break;
        }
        set_shell_enable();
...
static SDK_ERR shell_quit_register_testcase(void)
{
    SDK_ERR   retVal =  SDK_OK;
...
static SDK_ERR shell_cd_register_testcase(void)
{
    SDK_ERR   retVal =  SDK_OK;
    (void)shell_registercommand (
        "cd",
...
static SDK_S16 gIndex = 0;
static SDK_S16 gHistoryIndex = 0;
static SDK_U16 i = 0;
static SDK_U16 cur_flag = 0;
static SDK_U16 args_buff_len[SHELL_CCAHE];
...
static void function_thread(void *data)
{
    ...
    printf("###########################################################\n");
    printf("#  gkos sensor test shell uage: \n");
    printf("#    Input \"quit\" is quit program.\n");
    printf("#    example: \n");
    printf("#      ds-test@goke# quit\n");
    printf("###########################################################\n");
    ...
    while(1)
    {
        ...
        do
        {
            ...
            fprintf(stderr, "\rds-test@goke# %s", tmp_buff);
            if(cur_flag < i) {
