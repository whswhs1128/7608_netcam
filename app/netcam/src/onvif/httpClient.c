...
char *goke_http_form_data_request(char* url, HTTP_MULTI_META_DATA *dataStart, int* dataLength, int *dataRecvLen)
{
    ...
    do
    {
		...
		//send video
		fd_x = open(tmpData->fileName, O_RDWR, 0777);
		ret = lseek(fd_x, 0, SEEK_END);
		totalLen +=  ret;
		close(fd_x);
            }
            else
    ...
    do
    {
		...
		fd_x = open(tmpData->fileName, O_RDWR, 0777);
                do{
                ret = read(fd_x, video_buf, 2048);
		send(reqFd, video_buf, ret, 0);
                }while(ret > 0);
                close(fd_x);
		}
            }
...
int *dataRecvLen, HTTP_METHOD type, char *sPostParam,char *dataPtr,int dataSize)
{
...
#endif
    {
		...
		http_tcp_select_send(req_fd, dataPtr, dataSize,1);
