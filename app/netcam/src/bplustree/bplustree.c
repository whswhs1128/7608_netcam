...
#define data(tree,node) ((value_t *)(offset_ptr(node) + tree->_max_entries * sizeof(key_t)))
#define sub(tree,node) ((off_t *)(offset_ptr(node) + (tree->_max_order - 1) * sizeof(key_t)))
...
static inline struct bplus_node *cache_refer(struct bplus_tree *tree)
{
        ...
        for (i = 0; i < MIN_CACHE_NUM; i++) {
                if (!tree->used[i]) {
                        tree->used[i] = 1;
                        char *buf = tree->caches + tree->_block_size * i;
...
static inline void cache_defer(struct bplus_tree *tree, struct bplus_node *node)
{
        ...
        char *buf = (char *) node;
        int i = (buf - tree->caches) / tree->_block_size;
        tree->used[i] = 0;
}
...
static struct bplus_node *node_fetch(struct bplus_tree *tree, off_t offset)
{
        ...
        struct bplus_node *node = cache_refer(tree);
        int len = pread(tree->fd, node, tree->_block_size, offset);
        assert(len == tree->_block_size);
        return node;
}
...
static struct bplus_node *node_seek(struct bplus_tree *tree, off_t offset)
{
        ...
        for (i = 0; i < MIN_CACHE_NUM; i++) {
                if (!tree->used[i]) {
                        char *buf = tree->caches + tree->_block_size * i;
                        int len = pread(tree->fd, buf, tree->_block_size, offset);
                        assert(len == tree->_block_size);
                        return (struct bplus_node *) buf;
                }
        }
...
{
        if (node != NULL) {
                int len = pwrite(tree->fd, node, tree->_block_size, node->self);
                assert(len == tree->_block_size);
...
static off_t new_node_append(struct bplus_tree *tree, struct bplus_node *node)
{
        ...
        if (bpt_list_empty(&tree->free_blocks)) {
                node->self = tree->file_size;
                tree->file_size += tree->_block_size;
        } else {
                struct free_block *block;
                block = bpt_list_first_entry(&tree->free_blocks, struct free_block, link);
                bpt_list_del(&block->link);
                	...
                	struct bplus_node *left, struct bplus_node *right)
{
        ...
        bpt_list_add_tail(&block->link, &tree->free_blocks);
        /* return the node cache borrowed from */
        cache_defer(tree, node);
}
                		   ...
                		   int index, struct bplus_node *sub_node)
{
        ...
        sub(tree,parent)[index] = sub_node->self;
...
static value_t* bplus_tree_search(struct bplus_tree *tree, key_t key)
{
        ...
        while (node != NULL) {
                ...
                if (is_leaf(node)) {
                        p = (i >= 0) ? &(data(tree,node)[i]) : NULL;
                        break;
                } else {
                        if (i >= 0) {
                                node = node_seek(tree, sub(tree,node)[i + 1]);
                        } else {
                                i = -i - 1;
                                node = node_seek(tree, sub(tree,node)[i]);
                        }
                }
        }
        return p;
                             ...
                             struct bplus_node *r_ch, key_t key)
{
        if (l_ch->parent == INVALID_OFFSET && r_ch->parent == INVALID_OFFSET) {
                ...
                key(parent)[0] = key;
                sub(tree,parent)[0] = l_ch->self;
                sub(tree,parent)[1] = r_ch->self;
                parent->children = 2;
                	         ...
                	         struct bplus_node *r_ch, key_t key, int insert)
{
        ...
        int pivot = insert;
        left->children = split;
        node->children = tree->_max_order - split + 1;
        ...
        memmove(&sub(tree,left)[0], &sub(tree,node)[0], pivot * sizeof(off_t));
        ...
        memmove(&sub(tree,left)[pivot + 1], &sub(tree,node)[pivot], (split - pivot - 1) * sizeof(off_t));

        /* flush sub-nodes of the new splitted left node */
        for (i = 0; i < left->children; i++) {
                if (i != pivot && i != pivot + 1) {
                        sub_node_flush(tree, left, sub(tree,left)[i]);
                }
        }
        ...
        if (pivot == split - 1) {
                ...
                sub(tree,node)[0] = sub(tree,node)[split - 1];
        ...
        memmove(&sub(tree,node)[1], &sub(tree,node)[split], (node->children - 1) * sizeof(off_t));
                        	   ...
                        	   struct bplus_node *r_ch, key_t key, int insert)
{
        ...
        int pivot = 0;
        node->children = split;
        right->children = tree->_max_order - split + 1;
        ...
        /* replicate from key[split] to key[tree->_max_order - 2] */
        memmove(&key(right)[pivot + 1], &key(node)[split], (right->children - 2) * sizeof(key_t));
        memmove(&sub(tree,right)[pivot + 2], &sub(tree,node)[split + 1], (right->children - 2) * sizeof(off_t));

        /* flush sub-nodes of the new splitted right node */
        for (i = pivot + 2; i < right->children; i++) {
                sub_node_flush(tree, right, sub(tree,right)[i]);
        }
                        	   ...
                        	   struct bplus_node *r_ch, key_t key, int insert)
{
        ...
        right->children = tree->_max_order - split;

        /* sum = right->children = pivot + 2 + (tree->_max_order - insert - 1) */
        /* replicate from key[split + 1] to key[insert] */
        memmove(&key(right)[0], &key(node)[split + 1], pivot * sizeof(key_t));
        memmove(&sub(tree,right)[0], &sub(tree,node)[split + 1], pivot * sizeof(off_t));
        ...
        memmove(&key(right)[pivot + 1], &key(node)[insert], (tree->_max_order - insert - 1) * sizeof(key_t));
        memmove(&sub(tree,right)[pivot + 2], &sub(tree,node)[insert + 1], (tree->_max_order - insert - 1) * sizeof(off_t));
                        	   ...
                        	   key_t key, int insert)
{
        ...
        memmove(&sub(tree,node)[insert + 2], &sub(tree,node)[insert + 1], (node->children - 1 - insert) * sizeof(off_t));
        /* insert new key and sub-nodes */
        key(node)[insert] = key;
                	   ...
                	   struct bplus_node *l_ch, struct bplus_node *r_ch, key_t key)
{
        ...
        /* node is full */
        if (node->children == tree->_max_order) {
                key_t split_key;
                /* split = [m/2] */
                	     ...
                	     struct bplus_node *left, key_t key, value_t data, int insert)
{
        ...
        int pivot = insert;
        left->children = split;
        leaf->children = tree->_max_entries - split + 1;
        ...
        memmove(&data(tree,left)[0], &data(tree,leaf)[0], pivot * sizeof(value_t));

        /* insert new key and data */
        key(left)[pivot] = key;
        data(tree,left)[pivot] = data;

        /* replicate from key[insert] to key[split - 1] */
        memmove(&key(left)[pivot + 1], &key(leaf)[pivot], (split - pivot - 1) * sizeof(key_t));
        memmove(&data(tree,left)[pivot + 1], &data(tree,leaf)[pivot], (split - pivot - 1) * sizeof(value_t));

        /* original leaf left shift */
        memmove(&key(leaf)[0], &key(leaf)[split - 1], leaf->children * sizeof(key_t));
        memmove(&data(tree,leaf)[0], &data(tree,leaf)[split - 1], leaf->children * sizeof(value_t));
                	      ...
                	      struct bplus_node *right, key_t key, value_t data, int insert)
{
        ...
        leaf->children = split;
        right->children = tree->_max_entries - split + 1;

        /* sum = right->children = pivot + 1 + (tree->_max_entries - pivot - split) */
        /* replicate from key[split] to key[children - 1] in original leaf */
        memmove(&key(right)[0], &key(leaf)[split], pivot * sizeof(key_t));
        memmove(&data(tree,right)[0], &data(tree,leaf)[split], pivot * sizeof(value_t));

        /* insert new key and data */
        key(right)[pivot] = key;
        data(tree,right)[pivot] = data;

        /* replicate from key[insert] to key[children - 1] in original leaf */
        memmove(&key(right)[pivot + 1], &key(leaf)[insert], (tree->_max_entries - insert) * sizeof(key_t));
        memmove(&data(tree,right)[pivot + 1], &data(tree,leaf)[insert], (tree->_max_entries - insert) * sizeof(value_t));
                	       ...
                	       key_t key, value_t data, int insert)
{
        ...
        memmove(&data(tree,leaf)[insert + 1], &data(tree,leaf)[insert], (leaf->children - insert) * sizeof(value_t));
        key(leaf)[insert] = key;
        data(tree,leaf)[insert] = data;
        leaf->children++;
}
...
static int leaf_insert(struct bplus_tree *tree, struct bplus_node *leaf, key_t key, value_t data)
{
        ...
        int i = ((char *) leaf - tree->caches) / tree->_block_size;
        tree->used[i] = 1;

        /* leaf is full */
        if (leaf->children == tree->_max_entries) {
                key_t split_key;
                /* split = [m/2] */
                int split = (tree->_max_entries + 1) / 2;
                        	     ...
                        	     int parent_key_index, int remove)
{
        ...
        memmove(&sub(tree,node)[1], &sub(tree,node)[0], (remove + 1) * sizeof(off_t));
        ...
        sub(tree,node)[0] = sub(tree,left)[left->children - 1];
        sub_node_flush(tree, node, sub(tree,node)[0]);
                        	     ...
                        	     int parent_key_index, int remove)
{
        ...
        memmove(&sub(tree,left)[left->children], &sub(tree,node)[0], (remove + 1) * sizeof(off_t));

        /* sub-node sum = node->children - 1 */
        memmove(&key(left)[left->children + remove], &key(node)[remove + 1], (node->children - remove - 2) * sizeof(key_t));
        memmove(&sub(tree,left)[left->children + remove + 1], &sub(tree,node)[remove + 2], (node->children - remove - 2) * sizeof(off_t));
                        	      ...
                        	      int parent_key_index)
{
        ...
        sub(tree,node)[node->children] = sub(tree,right)[0];
        sub_node_flush(tree, node, sub(tree,node)[node->children]);
        node->children++;

        /* right sibling left shift*/
        memmove(&key(right)[0], &key(right)[1], (right->children - 2) * sizeof(key_t));
        memmove(&sub(tree,right)[0], &sub(tree,right)[1], (right->children - 1) * sizeof(off_t));
                        	      ...
                        	      int parent_key_index)
{
        ...
        memmove(&sub(tree,node)[node->children - 1], &sub(tree,right)[0], right->children * sizeof(off_t));

        /* flush sub-nodes of the new merged node */
        int i, j;
        for (i = node->children - 1, j = 0; j < right->children; i++, j++) {
                sub_node_flush(tree, node, sub(tree,node)[i]);
        }
...
static inline void non_leaf_simple_remove(struct bplus_tree *tree, struct bplus_node *node, int remove)
{
        ...
        memmove(&sub(tree,node)[remove + 1], &sub(tree,node)[remove + 2], (node->children - remove - 2) * sizeof(off_t));
        node->children--;
}
...
static void non_leaf_remove(struct bplus_tree *tree, struct bplus_node *node, int remove)
{
        if (node->parent == INVALID_OFFSET) {
                ...
                if (node->children == 2) {
                        ...
                        struct bplus_node *root = node_fetch(tree, sub(tree,node)[0]);
        ...
        } else if (node->children <= (tree->_max_order + 1) / 2) {
                ...
                if (sibling_select(l_sib, r_sib, parent, i)  == LEFT_SIBLING) {
                        if (l_sib->children > (tree->_max_order + 1) / 2) {
                        ...
                        if (r_sib->children > (tree->_max_order + 1) / 2) {
                		 ...
                		 int parent_key_index, int remove)
{
        ...
        memmove(&data(tree,leaf)[1], &data(tree,leaf)[0], remove * sizeof(value_t));

        /* borrow the last element from left sibling */
        key(leaf)[0] = key(left)[left->children - 1];
        data(tree,leaf)[0] = data(tree,left)[left->children - 1];
        left->children--;
                		 ...
                		 struct bplus_node *left, int parent_key_index, int remove)
{
        ...
        memmove(&data(tree,left)[left->children], &data(tree,leaf)[0], remove * sizeof(value_t));
        memmove(&key(left)[left->children + remove], &key(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(key_t));
        memmove(&data(tree,left)[left->children + remove], &data(tree,leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(value_t));
                                  ...
                                  int parent_key_index)
{
        ...
        data(tree,leaf)[leaf->children] = data(tree,right)[0];
        leaf->children++;

        /* left shift in right sibling */
        memmove(&key(right)[0], &key(right)[1], (right->children - 1) * sizeof(key_t));
        memmove(&data(tree,right)[0], &data(tree,right)[1], (right->children - 1) * sizeof(value_t));
        right->children--;
                                         ...
                                         struct bplus_node *right)
{
        ...
        memmove(&data(tree,leaf)[leaf->children], &data(tree,right)[0], right->children * sizeof(value_t));
...
static inline void leaf_simple_remove(struct bplus_tree *tree, struct bplus_node *leaf, int remove)
{
        ...
        memmove(&data(tree,leaf)[remove], &data(tree,leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(value_t));
        leaf->children--;
}
...
static int leaf_remove(struct bplus_tree *tree, struct bplus_node *leaf, key_t key)
{
        ...
        if (leaf->parent == INVALID_OFFSET) {
        ...
        } else if (leaf->children <= (tree->_max_entries + 1) / 2) {
                ...
                /* decide which sibling to be borrowed from */
                if (sibling_select(l_sib, r_sib, parent, i) == LEFT_SIBLING) {
                        if (l_sib->children > (tree->_max_entries + 1) / 2) {
                        ...
                        if (r_sib->children > (tree->_max_entries + 1) / 2) {
        ...
        while (node != NULL) {
                ...
                if (is_leaf(node)) {
                        ...
                        while (node != NULL && key(node)[i] <= max) {
                                start = data(tree,node)[i].u32Index;
                                if (++i >= node->children) {
                                        node = node_seek(tree, node->next);
                                        i = 0;
                                }
...
void bplus_refresh_boot(struct bplus_tree *tree)
{
	...
	if(tree->bootfd > 0)
	{
		lseek(tree->bootfd,0,SEEK_SET);
		assert(offset_store(tree->bootfd, tree->root) == ADDR_STR_WIDTH);
        assert(offset_store(tree->bootfd, tree->_block_size) == ADDR_STR_WIDTH);
        assert(offset_store(tree->bootfd, tree->file_size) == ADDR_STR_WIDTH);
		lseek(tree->bootfd,0,SEEK_SET);
		fsync(tree->bootfd);	
		fsync(tree->fd);
	}
...
struct bplus_tree *bplus_tree_init(char *filename, int block_size)
{
        ...
        assert(tree != NULL);
        bpt_list_init(&tree->free_blocks);
        ...
        if (fd >= 0) {
                ...
                while ((i = offset_load(fd)) != INVALID_OFFSET) {
                        ...
                        assert(block != NULL);
                        block->offset = i;
                        bpt_list_add(&block->link, &tree->free_blocks);
                }
                //close(fd);
				tree->bootfd = fd;
        } else {
		...
		tree->_block_size = _block_size;
		tree->_max_order = _max_order;
		tree->_max_entries = _max_entries;
        printf("config node order:%d and leaf entries:%d,block size:%d\n", 
			tree->_max_order, tree->_max_entries,tree->_block_size);

        /* init free node caches */
        tree->caches = malloc(tree->_block_size * MIN_CACHE_NUM);
...
void bplus_tree_deinit(struct bplus_tree *tree)
{
        ...
        assert(offset_store(fd, tree->_block_size) == ADDR_STR_WIDTH);
        assert(offset_store(fd, tree->file_size) == ADDR_STR_WIDTH);

        /* store free blocks in files for future reuse */
        struct bpt_list_head *pos, *n;
        bpt_list_for_each_safe(pos, n, &tree->free_blocks) {
                bpt_list_del(pos);
                struct free_block *block = bpt_list_entry(pos, struct free_block, link);
...
void bplus_tree_dump(struct bplus_tree *tree)
{
        ...
        for (; ;) {
                if (node != NULL) {
                        ...
                        /* Move deep down */
                        node = is_leaf(node) ? NULL : node_seek(tree, sub(tree,node)[sub_idx]);
                } else {
