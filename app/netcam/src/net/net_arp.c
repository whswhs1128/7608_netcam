...
{
    int ret =  utility_net_resolve_host(in_addr,str);
	if(ret == 0)
		return 0;
	else 
		return -1;
                     ...
                     char (*query_mac)[18], int out_size, int timeout)
{
    ...
    */
    ...
    ARP_CHECK_RET_GOTO(net_utils_macstrtobyte(addr_ii.sll_addr, src_mac, ETH_HW_ADDR_LEN));
    ...
    pkt.op = htons(type);
    ARP_CHECK_RET_GOTO(net_utils_macstrtobyte(pkt.targ_hw_addr, dst_mac, ETH_HW_ADDR_LEN));
    ARP_CHECK_RET_GOTO(net_utils_macstrtobyte(pkt.rcpt_hw_addr, query_mac[0], ETH_HW_ADDR_LEN));
    ARP_CHECK_RET_GOTO(net_utils_macstrtobyte(pkt.src_hw_addr, src_mac, ETH_HW_ADDR_LEN));
    ARP_CHECK_RET_GOTO(net_utils_macstrtobyte(pkt.sndr_hw_addr, mac, ETH_HW_ADDR_LEN));
    ...
    ARP_CHECK_RET_GOTO(net_utils_macstrtobyte(sa.sll_addr, dst_mac, ETH_HW_ADDR_LEN));
...
int ARP_send_request(char *query_ip, char (*query_mac)[18], int max_size, int timeout)
{
    ...
    return arp_send_packet(OP_ARP_REQUEST, BROADCAST_HW_ADDR, mac, ip, mac, query_ip, query_mac, max_size, timeout);
}

//get one mac by broadcast ip
int ARP_query(char *query_ip, char *query_mac)
{
    ...
    ret =  arp_send_packet(OP_ARP_REQUEST, BROADCAST_HW_ADDR, mac, ip, mac, query_ip, out_mac, 1, ARP_RECV_TIMEOUT);
    strcpy(query_mac, out_mac[0]);
    return ret;
}

// get one mac by arp cach table
int ARP_get_arptable(char *query_ip, char *query_mac)
{
    int sock;
    struct arpreq areq;
   ...
   char def_eth[128];
    if(NULL == getenv("DEF_ETH")){
		snprintf(def_eth, sizeof(def_eth), LAN);
	}else{
		snprintf(def_eth, sizeof(def_eth), "%s", getenv("DEF_ETH"));
	}
    strncpy(sArpDefEther, def_eth, sizeof(sArpDefEther));
    ...
    hw_addr = (unsigned char*)areq.arp_ha.sa_data;
...
//get ip by broadcast mac address
int RARP_send_request(char *query_ip, char *query_mac)
{
    ...
    return arp_send_packet(OP_RARP_REQUEST, BROADCAST_HW_ADDR, mac, ip, mac,query_ip , __mac, 1, ARP_RECV_TIMEOUT);
}


/*
 ***************************************************************
 *	recvie arp packet and process hook function.
 *
 *
 ***************************************************************
 */
...
{
    sdk_sys_thread_set_name("arp_proc");
    int sock = -1;
    char buf[10000];
    struct sockaddr  da;
    socklen_t addrlen;
