...
static hi_vb_src g_vdec_vb_src = HI_VB_SRC_MOD;
static hi_bool g_vdec_line_ldy_en = HI_FALSE;

static hi_vb_pool g_pic_vb_pool[HI_VB_MAX_POOLS] = { [0 ... (HI_VB_MAX_POOLS - 1)] = HI_VB_INVALID_POOL_ID };
static hi_vb_pool g_tmv_vb_pool[HI_VB_MAX_POOLS] = { [0 ... (HI_VB_MAX_POOLS - 1)] = HI_VB_INVALID_POOL_ID };
...
hi_s32 sample_comm_config_ldy_attr(hi_s32 i, hi_u32 height)
{
    ...
    if (g_vdec_line_ldy_en == HI_TRUE) {
        check_chn_return(hi_mpi_vdec_get_low_delay_attr(i, &ldy_attr), i, "hi_mpi_vdec_get_low_delay_attr");
        ldy_attr.enable = HI_TRUE;
        ldy_attr.line_cnt = SAMPLE_VDEC_LOW_DELAY_LINE_CNT;
        check_chn_return(hi_mpi_vdec_set_low_delay_attr(i, &ldy_attr), i, "hi_mpi_vdec_set_low_delay_attr");
