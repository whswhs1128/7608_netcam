...
hi_s32 sample_inner_codec_cfg_audio(hi_audio_sample_rate sample_rate)
{
    ...
    if (ioctl(fd_acodec, HI_ACODEC_SOFT_RESET_CTRL)) {
        printf("reset audio codec error\n");
    }

    if (inner_codec_get_i2s_fs(sample_rate, &i2s_fs_sel) != HI_SUCCESS) {
        return HI_FAILURE;
    }

    if (ioctl(fd_acodec, HI_ACODEC_SET_I2S1_FS, &i2s_fs_sel)) {
    ...
    if (ioctl(fd_acodec, HI_ACODEC_SET_MIXER_MIC, &input_mode)) {
    ...
    if (ioctl(fd_acodec,  HI_ACODEC_SET_INPUT_VOLUME, &acodec_input_vol)) {
...
hi_s32 sample_comm_audio_start_hdmi(const hi_aio_attr *aio_attr)
{
    hi_s32 ret;
    hi_hdmi_attr hdmi_attr = {0};
    hi_hdmi_id hdmi = HI_HDMI_ID_0;
    hi_vo_pub_attr pub_attr = {0};
    const hi_vo_dev vo_dev = 0;

    pub_attr.bg_color = 0x000000ff; /* 0x000000ff:bg color */
    pub_attr.intf_type = HI_VO_INTF_HDMI;
    pub_attr.intf_sync = HI_VO_OUT_1080P60;

    if (sample_comm_vo_start_dev(vo_dev, &pub_attr, HI_NULL, 0) != HI_SUCCESS) {
