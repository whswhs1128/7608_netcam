...
DAHUA_SOCK_T g_sock_sub;

DAHUA_SOCK_T g_sock_main;
int send_main = 1;
int send_sub = 0;
...
static int dahua_ack_search(int sock_send)
{
    ...
    strcpy(str_dev_search->HostName, DEV_DEVICE_NAME); //Ö÷»úÃû
...
//    PRINT_INFO("ip:%s\n", (char *)inet_ntoa(net_addr));
    ...
    str_dev_search->TCPPort = TCP_LISTEN_PORT; // 37777
    str_dev_search->TCPMaxConn = MAX_LINK_NUM; // 10
    ...
    n_extra_len = strlen(a_extra_sting); //¶à¸ö 00
        ...
        char ch_width, char ch_high, unsigned char fps)
{
    ...
    if (g_sock_main.fd > 0)
    {
        ...
        ret = ZhiNuo_Send_Main_Unit(&g_sock_main, send_buff, n_offset + data_len + sizeof(ZhiNuo_Media_Frame_Tail), DAHUA_ONE_PACKAGE_SIZE);
        if(ret <= 0)
        {
...
int ZhiNuo_Send_Main_Stream()
{
    ...
    {
        if(g_sock_main.fd < 0 || send_main == 0)
        {
            usleep(40000);
            continue;
        } 
        ...
        ret = mediabuf_read_frame((MEDIABUF_HANDLE)handle, (void **)&p_data_buff, &data_len, &frame_header);
        if (ret < 0) {
			PRINT_ERR("read main stream fail.\n");
        ...
        if(ret < 0)
        {
            PRINT_ERR("send main stream fail.\n");
            break;
        }
    }  
...
int ZhiNuo_LogOn(char* bufPost, DAHUA_SOCK_T *sock)
{
    ...
    if (t_msg_head_rcv->dvrip.dvrip_v == 6) 
    {
        PRINT_INFO("extralen:%d, extra:%s\n", t_msg_head_rcv->dvrip.dvrip_extlen, extra); 
    }
...
int ZhiNuo_Connect_Request(char* bufPost, DAHUA_SOCK_T* sock)
{
    ...
    if (1 == ch_request_type)
    {
        g_sock_main.fd = sock->fd;
        g_sock_main.n_flag = n_flag;
        PRINT_ERR("connect sock:%d to send %s stream\n", sock->fd, ch_request_type == 1 ? "main": "sub");
        b_connect_full = FALSE;

    }
    else if (2 == ch_request_type)
    {
        g_sock_sub.fd = sock->fd;
        g_sock_sub.n_flag = n_flag;
        PRINT_ERR("connect sock:%d to send %s stream\n", sock->fd, ch_request_type == 1 ? "main": "sub");
        b_connect_full = FALSE;
    }
...
int ZhiNuo_Unkown_Cmd(char* bufPost, DAHUA_SOCK_T *sock)
{
    ...
    sprintf(send_msg + DVRIP_HEAD_T_SIZE, "{ \"id\" : %d, \"params\" : null, \"result\" : true, \"session\" : %ld }\n", ch_id, n_session);
    t_msg_head->dvrip.dvrip_extlen = strlen(send_msg + DVRIP_HEAD_T_SIZE);
...
int ZhiNuo_CMDDealBody(char* bufPost, DAHUA_SOCK_T *sock, int datalen)
{
    ...
    while(1)
    {
        ...
        PRINT_INFO("sock:%d, flag:%d, t_msg_head->dvrip.cmd @@@>>>>>>>:%x\n", sock->fd, sock->n_flag, t_msg_head->dvrip.cmd);
        switch(t_msg_head->dvrip.cmd)
        {
            case CMD_DAHUA_LOG_ON:
                PRINT_INFO("dahua logon\n");
...
void *ZhiNuo_Session_Thread(void *param)
{
	...
	*((int*)param) = -1;
    ...
    while(1)
    {
        ...
        if (n_rcv_len <= 0) 
        {
            PRINT_ERR("ZhiNuo_SockRecv err fd:%d, n_rcv_len\n", dahua_sock.fd, n_rcv_len);
            break;
        }

        //´¦ÀíÃüÁî
        ret = ZhiNuo_CMDDealBody(arrstr_rcv_buff, &dahua_sock, n_rcv_len);
        if(1 == ret) //Èç¹ûÊÇÁ¬½ÓÇëÇóÔòÕâ¸öÏß³Ì´¦ÀíÍê¾Í¿ÉÒÔ¹Ø±ÕÁË
        {
            PRINT_INFO("ret=1, out loop\n");
            return NULL;
        }
        if(ret < 0)
        {
            PRINT_ERR("ZhiNuo_CMDDealBody err, ret=%d\n", ret);
            break;
        }
    ...
    if (g_sock_main.fd > 0)
    {
        close(g_sock_main.fd);
        g_sock_main.fd = -1;
        g_sock_main.n_flag = 0;
    }
...
int dahua_protocol_start()
{
    ...
    memset(&g_sock_sub, 0, sizeof(g_sock_sub));
    g_sock_sub.fd = -1;
    g_sock_sub.n_flag = 0;

    memset(&g_sock_main, 0, sizeof(g_sock_main));
    g_sock_main.fd = -1;
    g_sock_main.n_flag = 0;
