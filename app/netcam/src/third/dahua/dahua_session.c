...
static DAHUA_SESSION_CTRL  *gSock = NULL;
static int motionLast = 0;
static int motionTimer = 0;

static int Dahua_Cfg_MotionDetect_Handle(char type);
...
//bruce
    ...
    switch(*cmd)
    {
        ...
        case DMS_PTZ_CMD_DOWN_RIGHT:
            ...
            break;
        case DMS_PTZ_CMD_START_CRU:
            netcam_ptz_hor_ver_cruise(speed);
            break;
        case DMS_PTZ_CMD_STOP_CRU:
            netcam_ptz_stop();
            break;
...
int goke_api_set_ioctrl(int command, void *param)
{
	...
	switch(command)
	{
		...
		case SET_VIDEO_CFG:
        {
            ...
            ST_GK_ENC_STREAM_H264_ATTR streamCfg;
            for (i = 0; i < 2; i++)
            {
                streamCfg.enctype   = video_cfg->vencStream[i].enctype;
                streamCfg.width     = video_cfg->vencStream[i].h264Conf.width;
                streamCfg.height    = video_cfg->vencStream[i].h264Conf.height;
                streamCfg.fps       = video_cfg->vencStream[i].h264Conf.fps;
                streamCfg.gop       = video_cfg->vencStream[i].h264Conf.gop;
                streamCfg.profile   = video_cfg->vencStream[i].h264Conf.profile;
                streamCfg.rc_mode   = video_cfg->vencStream[i].h264Conf.rc_mode;
                streamCfg.bps       = video_cfg->vencStream[i].h264Conf.bps;
                streamCfg.quality   = video_cfg->vencStream[i].h264Conf.quality;
                streamCfg.re_rec_mode = video_cfg->vencStream[i].h264Conf.re_rec_mode;
                ret |= netcam_video_set(0, i, (PS_GK_ENC_STREAM_H264_ATTR)&streamCfg);
            }
...
static int printhex(unsigned char hex[], unsigned int len)
{
    int i;
    if(NULL == hex || len <= 0)
        return -1;
...
static int imagesize_dahua2goke(CAPTURE_SIZE_T dahua_image_size, GK_ENC_STREAM_H264_ATTR *h264Conf)
{
    switch(dahua_image_size)
    {
        ...
        case 0x30:
            h264Conf->width = 2688;
            h264Conf->height = 1520;
            break;
        case 0x33:
            h264Conf->width = 2560;
            h264Conf->height = 1440;
            break;

        case CAPTURE_SIZE_3M:
            h264Conf->width = 2304;
            h264Conf->height = 1296;
            break;
...
//bruce
    ...
    if(CAPTURE_BRC_CBR == str_captue->MainOption[0].BitRateControl )// 固定码流
    {
        main_h264Conf->rc_mode = 0;
    }else /*if(CAPTURE_BRC_VBR == str_captue->MainOption[0].BitRateControl) //可变码流*/
    {
        main_h264Conf->rc_mode = 1;
    }
    switch(str_captue->MainOption[0].ImgQlty)/*!< 码流的画质 档次1-6 6最好 */
    {
        case 6:
    ...
    main_h264Conf->fps= str_captue->MainOption[0].Frames;    /*!< 帧率　档次N制1-6,P制1-5 */
    main_h264Conf->bps = str_captue->MainOption[0].usBitRate * 8;    //码率 单位为K
    ...
    if(CAPTURE_BRC_CBR == str_captue->AssiOption[0].BitRateControl )// 固定码流
    {
        second_h264Conf->rc_mode = 0;
    }
    if(CAPTURE_BRC_VBR == str_captue->AssiOption[0].BitRateControl) //可变码流
    {
        second_h264Conf->rc_mode = 1;
    }
    switch(str_captue->AssiOption[0].ImgQlty)/*!< 码流的画质 档次1-6 6最好 */
    {
        case 6:
    ...
    second_h264Conf->fps= str_captue->AssiOption[0].Frames;;    /*!< 帧率　档次N制1-6,P制1-5 */
    second_h264Conf->bps = str_captue->AssiOption[0].usBitRate * 8;    //码率 单位为K
...
int ZhiNuo_SockConnect(int sock, struct sockaddr_in *server, int TimeOut)
{
    ...
    FD_ZERO(&set_fd);
    FD_SET(sock, &set_fd);
    timeout.tv_sec = TimeOut; //连接超时15秒
    timeout.tv_usec = 0;
...
int ZhiNuo_Send_Second_Unit(DAHUA_SESSION_CTRL* sock, char *buf, unsigned long len, int nOnePacketSize)
{
    ...
    t_msg_head.c[9] = 0; //数据序号，累加（每客户端单独累加）
    t_msg_head.c[8] = 0;  //通道号，如为预览数据，填通道数量，如16路机器，填16

    /* 返回码 0:正常 1:无权限 2:不支持此操作 3:资源不足 4:无法获得所请求序列号的数据
10:数据结束（针对图片、I 帧等有限数据）*/
    t_msg_head.c[16] = 0;
    t_msg_head.c[24] = 5; //副码流
        ...
        char ch_width, char ch_high, int fps)
{
    ...
    t_media_data_head->ch_child_type = 0; //没有子类型
    t_media_data_head->ch_channel_num = 0;  //通道号为
    ...
    else if(frame_type == FRAME_TYPE_P)
    {
        ...
        t_media_data_head->n_frame_len = sizeof(ZhiNuo_Media_Frame_Head) + data_len
            + sizeof(ZhiNuo_Media_Frame_Tail);
        t_media_data_head->ch_expand_len = 0; //没有扩展帧头
    }
    else
    {
#if 1
        ...
        t_expand_playback_type->ch_reserve = 0xff; //保留位为0
        t_expand_playback_type->ch_video_type = (char)H264;
        //bruce
        t_expand_playback_type->ch_frame_rate = fps; //获取帧率
#endif
    ...
    t_media_data_head->t_date.second = tmip->tm_sec; /* 秒–取值区间为[0,59] */
    t_media_data_head->t_date.minute = tmip->tm_min; /* 分 - 取值区间为[0,59] */
    t_media_data_head->t_date.hour = tmip->tm_hour; /* 时 - 取值区间为[0,23] */
    t_media_data_head->t_date.day = tmip->tm_mday; /* 一个月中的日期 - 取值区间为[1,31] */
    t_media_data_head->t_date.month = tmip->tm_mon + 1; /* 月份取值区间为[1,12] */
    t_media_data_head->t_date.year = tmip->tm_year + 1900 - 2000; /* 年份，其值[0-63],2000-2063 */

    //帧头校验和填充
    int i = 0;
    unsigned char * p_temp = (unsigned char *)t_media_data_head;
    unl_sum = 0;
    for(i = 0; i < 23; i++) //前23个字节累加和
    {
        unl_sum += *p_temp;
        p_temp = p_temp + 1;
    }
...
int DahuaSendSubStream()
{
    ...
    while(1)
    {
        ...
        ret = DahuaPackSubStream(send_buff, p_data_buff, data_len, ch_frame_type,
                ...
                frame_header.frame_rate); //8像素点为1单位
        ...
        char ch_width, char ch_high, unsigned char fps)
{
    ...
    if(frame_type == AUDIO_TYPE)
    {
        ...
        t_expand_audio_format->ch_audio_channel = (char)SINGLE_CHANNEL;
        t_expand_audio_format->ch_audio_type =   (char)G711A;
        t_expand_audio_format->ch_sample_frequency = (char)SAMPLE_FREQ_8000; // 抓包 8000 和 16000 都是4 ?
    }
    else if(frame_type == FRAME_TYPE_P)
    {
        //视频帧头的特殊部分填充
    ...
    else
    {
        ...
        t_expand_playback_type->ch_video_type = (char)H264;
        #else
        t_expand_playback_type->ch_reserve = 0x32; //保留位为0
...
int DahuaTcpSelectSend(int sock, char *szbuf, int len, int timeout, int re_counts)
{
    ...
    if (re_counts == 0)
        re_counts = 2;
    while(send_total < len) {
        if(re_counts == n_timeout_num) {//如果超时3次则关闭此次会话
...
static int DahuaAddMediaHead(char *buf, DahuaFrameInfo *p_frame_info, unsigned long media_len, int stream_type)
{
    int picSizeLen = 0;
    ...
    else if(p_frame_info->ch_frame_type == FRAME_TYPE_I)
    {
        ...
        if (p_frame_info->ch_width == 0 && p_frame_info->ch_height == 0 && p_frame_info->video_reso != 0)
        {
            ZhiNuo_Expand_Picture_Size2 *t_expand_picture_size2 = (ZhiNuo_Expand_Picture_Size2 *)
                (buf + sizeof(ZhiNuo_Media_Frame_Head));
            t_expand_picture_size2->ch_cmd = EXPAND_PICTURE_SIZE2;
            t_expand_picture_size2->ch_coding_type = ONE_FRAME;
            t_expand_picture_size2->ch_width = (unsigned short)(p_frame_info->video_reso >> 16);
            t_expand_picture_size2->ch_high = (unsigned short)(p_frame_info->video_reso & 0xFFFF);
            picSizeLen = sizeof(ZhiNuo_Expand_Picture_Size2);
        }
        else
        {
            //图像尺寸扩展帧头填充
            ...
            picSizeLen = sizeof(ZhiNuo_Expand_Picture_Size);
        }
        t_media_data_head->ch_expand_len = picSizeLen + sizeof(ZhiNuo_Expand_PlayBack_Type);

        //回放类型扩展帧头填充
        ZhiNuo_Expand_PlayBack_Type *t_expand_playback_type = (ZhiNuo_Expand_PlayBack_Type *)
            (buf + sizeof(ZhiNuo_Media_Frame_Head) + picSizeLen);
        t_expand_playback_type->ch_cmd = EXPAND_PLAY_BACK_TYPE;
        t_expand_playback_type->ch_frame_rate = p_frame_info->ch_fps;
#if 1
        t_expand_playback_type->ch_reserve = 0xff; //保留位为0
        if (runVideoCfg.vencStream[stream_type].enctype == 1)
        {
            t_expand_playback_type->ch_video_type = (char)H264;
        }
        else if (runVideoCfg.vencStream[stream_type].enctype == 3)
        {
            t_expand_playback_type->ch_video_type = (char)H265;
        }
        else if (runVideoCfg.vencStream[stream_type].enctype == 2)
        {
            t_expand_playback_type->ch_video_type = (char)MPEG4;
        }
#else
        t_expand_playback_type->ch_reserve = 0x32; //保留位为0
        t_expand_playback_type->ch_video_type = 0x08;
...
static int DahuaAddAlarmHead(char *buf, unsigned long media_len)
{
    ...
    t_media_data_head->ch_child_type = 0x0d; //没有子类型
    t_media_data_head->ch_channel_num = 0;  //通道号为
    t_media_data_head->ch_child_sequence = 0;
    t_media_data_head->n_frame_sequence = 0;
    t_media_data_head->n_frame_len = media_len;
    //t_media_data_head->ch_expand_len = 0; //没有扩展帧头 抓包是8
...
static int DahuaAddAlarmMsgHead(char *buf, unsigned char cmd, unsigned int media_len)
{
    ...
    //p_msg_head->s[7] = timetick;  // 需要进一步确定是否是 short, todo
...
static int DahuaAddMsgHead(char *buf, unsigned char cmd, int stream_type, int seq, int pack_len)
{
    ...
    p_msg_head->dvrip.cmd = cmd;
    p_msg_head->c[9] = 0; //数据序号，累加（每客户端单独累加）
    p_msg_head->c[8] = 0;  //通道号，如为预览数据，填通道数量，如16路机器，填16

    /* 返回码 0:正常 1:无权限 2:不支持此操作 3:资源不足 4:无法获得所请求序列号的数据
    10:数据结束（针对图片、I 帧等有限数据）*/
    p_msg_head->c[16] = 0;
    if (stream_type == 0)
        p_msg_head->c[24] = 0; //主码流  todo
    else if (stream_type == 1)
        p_msg_head->c[24] = 5; //子码流
    //t_msg_head.s[7] = g_media_seq

    p_msg_head->dvrip.dvrip_extlen = pack_len;
    p_msg_head->s[7] = seq;  // 需要进一步确定是否是 short, todo

    return 0;
}


static char * DahuaPackFrame(DahuaFrameInfo *p_frame_info, unsigned long *all_len, int stream_type)
{
    int expandSizeLen = 0;
    //计算媒体数据总长度
    ...
    if (p_frame_info->ch_frame_type == AUDIO_TYPE) {
        ...
        {
            //expand 0x82
            media_len = sizeof(ZhiNuo_Media_Frame_Head) + sizeof(ZhiNuo_Expand_Picture_Size2)
    ...
    //帧头填充
    DahuaAddMediaHead(media_buf, p_frame_info, media_len, stream_type);
...
{
    //分包发送，一次发送数据大小: DAHUA_ONE_PACKAGE_SIZE + DVRIP_HEAD_T_SIZE
    int nOnePacketSize = DAHUA_ONE_PACKAGE_SIZE;
...
static int DahuaPackSendStream(DahuaFrameInfo *p_frame_info, int stream_type)
{
    // pack stream
    unsigned long media_len = 0;
    char *media_buf = DahuaPackFrame(p_frame_info, &media_len, stream_type);
    if (NULL == media_buf) {
...
static int DahuaReadSendStream(MEDIABUF_HANDLE reader, int stream_type, DahuaFrameInfo *p_frame_info)
{
    ...
    if (frame_header.frame_type == GK_NET_FRAME_TYPE_A) {
        ...
        if ((frame_header.video_reso >> 16) >= (0xFF * 8) || (frame_header.video_reso & 0xFFFF) >= (0xFF * 8))
        {
            //分辨率大于0xFF*8
            p_frame_info->video_reso = frame_header.video_reso;
            p_frame_info->ch_width = 0;
            p_frame_info->ch_height = 0;
        }
        else
        {
            p_frame_info->video_reso = 0;
...
int Dahua_LogOn(char* bufPost, DAHUA_SESSION_CTRL *pCtrl)
{
    ...
    t_msg_head->c[2] = 0; //0: 支持16路 1:支持32路
    ...
    if (check == 0) {
        ...
        /* 登录失败返回码  0:密码不正确 1:帐号不存在 3:帐号已经登录 4:帐号被锁定
        5:帐号被列入黑名单 6:资源不足，系统忙 7:版本不对，无法登陆 */
        t_msg_head->c[9] = 0; //登录失败返回码
        t_msg_head->l[4] = 0;
    }

    t_msg_head->c[10] = 1;  //通道数
    //t_msg_head->c[11] = 9;   //视频编码方式 8:MPEG4 9:H.264
    t_msg_head->c[11] = 0;   //视频编码方式 8:MPEG4 9:H.264
    ...
    //t_msg_head->s[15] = 0x8101; //第30、31字节保留以下值 代理网关产品号0x8101, 0x8001, 0x8002, 0x8003,这个值nvr没用到随便填
    #else
    t_msg_head->c[20] = 1;
    t_msg_head->c[24] = 0x06;
    t_msg_head->c[26] = 0xf9;
...
int ZhiNuo_Search_Status(char* bufPost, DAHUA_SESSION_CTRL *pCtrl)
{
    ...
    //录像工作状态，0~31按位表示32个通道的录像状态，0表示没有录像，1表示正在录像
    t_msg_head->c[9] = 0;
    t_msg_head->c[10] = 0;
    t_msg_head->c[11] = 0;
    t_msg_head->c[12] = 0;

    /*报警返回有效性标识，每位对应一种类型的报警，值0表示有相应报警，值1表示无相应报警
      第一位：表示外部报警
      第二位：表示视频丢失
      第三位：表示动态检测
      第四位：表示第三方报警（当W08的值为2时，此位无效）
      第五位：表示串口故障报警
     */
    //t_msg_head->c[13] = 0xfb;
    t_msg_head->c[13] = 0;

    //外部报警状态，由低到高按位表示32个报警
    t_msg_head->l[4] = 0;

    //视频丢失报警状态，由低到高按位表示32个报警
    t_msg_head->l[5] = 0;
    //动态检测报警状态，由低到高按位表示32个报警
    ...
    //串口故障报警状态，由低到高按位表示8个报警
    t_msg_head->c[28] = 0;

    //音频输入状态，由低到高按位表示16个报警
    t_msg_head->c[29] = 0;
 ...
 * 输出参数：
 * 返回值  ：
 * 备注    :收到操作类型为2:订阅报警状态 事件类型为1:$外部$报警2:$动态检测$报警3:$视频丢失$报警4:$遮挡检测$报警
 *************************************************************/
int ZhiNuo_Dev_Event(char* bufPost, DAHUA_SESSION_CTRL * sock)
{
    ...
    /*操作类型:0:保留1:查询报警状态2:订阅报警状态3:*取消*订阅报警状态
      W28 ~W31 四个字节按位表示十六个通道，置 1为订阅状态*/
    ...
    t_msg_head->c[16] = 0;   //返回码 0:成功1:失败2:无权限3:设备忙
...
int ZhiNuo_Search_Media_Capacity(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    //通道号查询1-16, 0-表示不按通道查询   查询信息类别 0:查询主码流   1:查询辅码流 2:查询分辩率信息
    ...
    //返回码 0:正常 1:失败 5:失败，原因：不支持不按通道号查询能力集 6:失败，原因，不支持按通道号查询能力
    t_msg_head->c[8] = 0;
    t_msg_head->c[9] = ch_search_type; //返回信息类别（与查询对应）
    t_msg_head->c[10] = ch_channel_num; //通道号查询（与查询对应）
    ...
    if(0 == ch_search_type)
    {
        ...
        str_fram_caps.Compression |= (1 << CAPTURE_COMP_H265);
        ...
        if(strstr(resolution_str, "2304x1296") || strstr(resolution_str, "2048x1536")){
            str_fram_caps.ImageSize |= (1 << CAPTURE_SIZE_3M);
            PRINT_INFO("3M\n");
        }
        if(strstr(resolution_str, "1920x1080")){
            str_fram_caps.ImageSize |= (1 << CAPTURE_SIZE_2_5M);
            PRINT_INFO("1080p\n");
        }
        ...
        #if 0
        //抓包获得
        unsigned char ack2[]="\x83\x00\x00\x68\x08\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x80\x00\x00\x00\x00\x00\x02\x00";
        t_msg_head->dvrip.dvrip_extlen = sizeof(ack2) - DVRIP_HEAD_T_SIZE;
        memcpy(send_msg, ack2, sizeof(ack2));
        #endif
    }
    ...
    if(1 == ch_search_type)
    {
        t_msg_head->c[11] = 1; //抓包发现这个字段填充的1但是不知道是什么意思
        t_msg_head->c[15] = 1; //配置版本号
        ...
        str_fram_caps.ExtraStream |= 1; //表示支持主码流
        str_fram_caps.ExtraStream |= 2; //表示支持副码流1
        str_fram_caps.ExtraStream |= 16;//表示支持jpg抓图
        ...
        #ifdef ACKDATA
        ////抓包获得
        unsigned char ack2[]="";
        t_msg_head->dvrip.dvrip_extlen = sizeof(ack2) - DVRIP_HEAD_T_SIZE;
        memcpy(send_msg, ack2, sizeof(ack2));
        #endif
    }
    ...
    if(2 == ch_search_type)
    {
        ...
        //第一位表示支持主码流第二位表示支持辅码流1第三位表示支持辅码流2第五位表示支持jpg抓图
...
int ZhiNuo_Search_Config(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    //打印搜索类型 16:编码能力信息 38:摄像头参数配置，127:双码流图像配置 126:颜色配置 124:获得事件配置
    ...
    t_msg_head->c[16] = search_type; //搜索类型(39	:网卡配置 )
    ...
    switch(search_type)
    {
        ...
        case 0:
        {
            ...
            {
                case 0:
                    stTime.c[15] = 0; // 0 年月日， 1 月日年， 2 日月年
                    stTime.c[16] = 1; // 0 . ; 1 - ; 2 /
                    break;
                case 1:
                    stTime.c[15] = 1;
            ...
            #if 0
            //抓包获得
            unsigned char ack[]="\xb3\x00\x00\x68\x16\x00\x00\x00\x63\x6f\x6e\x66\x69\x67\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x08\x00\x00\x01\x01\x0a\x00\x00\x00\x00\x00\x00\x00\x00";
            t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
            memcpy(send_msg, ack, sizeof(ack));
            #endif
        ...
        case 16: //0x10
            {
                ...
                unsigned char capData[] = "\xb3\x00\x00\x68\x08\x00\x00\x00\x63\x6f\x6e\x66\x69\x67\x00\x00\x10\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xd4\x8f\x05\x01\x00\x00\x00";
                //t_msg_head->dvrip.dvrip_extlen = sizeof(CAPTURE_ENCCHIPCAPABILITY);
                t_msg_head->dvrip.dvrip_extlen = sizeof(capData) - DVRIP_HEAD_T_SIZE;
                ...
                //test
                //n_max_resolution_main = 1280 * 720;/*最大的分辨率*/
                //n_max_resolution_second = 640 * 480;/*最大的分辨率*/
                ...
                str_encchip_capability.EncChipInfo[0].nMaxEncodePower = n_max_resolution_main * 60 + n_max_resolution_second * 60; //主码流和子码流的能力之和)///< DSP 支持的最高编码能力。
                str_encchip_capability.EncChipInfo[0].nMaxSupportChannel = 1;	///< DSP 支持最多输入视频通道数。
                str_encchip_capability.EncChipInfo[0].bChannelMaxSetSync = 0;	///< DSP 每通道的最大编码设置是否同步 0-不同步, 1 -同步。
                str_encchip_capability.EncChipInfo[0].nExpandChannel = 0; ////< DSP 支持的扩展通道数，主要是多路回放使用，目前只是一个

                //memcpy(send_msg + DVRIP_HEAD_T_SIZE, &str_encchip_capability, sizeof(CAPTURE_ENCCHIPCAPABILITY));
                memcpy(send_msg, capData, sizeof(capData));

                #ifdef ACKDATA
                //抓包获得
                unsigned char ack[]="\xb3\x00\x00\x68\x08\x00\x00\x00\x63\x6f\x6e\x66\x69\x67\x00\x00\x10\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xd4\x8f\x05\x01\x00\x00\x00";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
            }
        ...
        case 34://0x22 osd 覆盖
            {//osd 覆盖
                PRINT_INFO("search_type:%d, 获取视频区域遮挡\n", search_type);
                ...
                if(0 == goke_api_get_ioctrl(GET_COVER_CFG, cover))
                {
                    ...
                    for(i = 0,j = 0; i < 4; i++)
                    {
                        if(cover[i].enable == 1 && (cover[i].width > 0) && (cover[i].height > 0))
                        {
                            ...
                            cover_attr[j].tBlock.right = cover_attr[j].tBlock.left + (int)(cover[i].width * 8192.0);
                            cover_attr[j].tBlock.bottom = cover_attr[j].tBlock.top + (int)(cover[i].height * 8192.0);
                            cover_attr[j].Color = 0;
                            cover_attr[j].iBlockType = 0;
                            cover_attr[j].Encode = 0;
                            cover_attr[j].Priview = 0;
                            cover_attr[j].iRev[0] = 0x00;//抓包得知
                            cover_attr[j].iRev[1] = 0x01;//抓包得知
                ...
                #if 0
                //抓包获得
                unsigned char ack[]="";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
            }
            break;
        case 38://0x26:摄像头参数配置
            {
                PRINT_INFO("search_type:%d, 摄像头参数配置\n", search_type);
                ...
                cam_control.Mirror = str_channel_color_info.mirrorEnabled;
                cam_control.Flip = str_channel_color_info.flipEnabled;
                cam_control.LensFunction = 0x10;
                cam_control.WhiteBalance = str_channel_color_info.wbMode;
                ...
                #if 0
                //抓包获得
                unsigned char ack[]="";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
        ...
        case 39://0x27 网络信息
            ...
            break;
		#if ADD_MD_INFO
        case 124://0x7c:获得事件配置
            {
                PRINT_INFO("search_type:%d,search_child_type:%d, 获得事件配置\n", search_type, search_child_type);
                if(4 == search_child_type)
                {
                    ...
                    t_msg_head->dvrip.dvrip_extlen = sizeof(CONFIG_MOTIONDETECT) * 16;   //要发回16个这样结构体大小的数据
        ...
        case 126:// 7e 颜色配置
            {
                PRINT_INFO("search_type:%d,%02x, to get color\n", search_type, search_type);
                GK_NET_IMAGE_CFG str_channel_color_info;
                ...
                //目前只有数组0元素生效
                t_msg_head->dvrip.dvrip_extlen = sizeof(CONFIG_COLOR_OLD) * 16;   //要发回16个这样结构体大小的数据
                ...
                str_color.Color[0].Gain = 50;				/*!< 增益	0-100		*/ //界面上没用到，暂时不用
                ...
                #ifdef ACKDATA
                //抓包获得
                unsigned char ack[]="";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
            }
        ...
        case 127://7f 双码流图像配置
            {
                PRINT_INFO("search_type:%d,%02x, 获得编码配置\n", search_type, search_type);
                int i = 0;
                GK_NET_VIDEO_CFG channel_pic_info;
                unsigned char *tt = NULL;
                unsigned char tmp[]={"\xff\xff\xff\x00\x00\x00\x00\x80\x65\x02\x65\x02"};
                ...
                t_msg_head->dvrip.dvrip_extlen = sizeof(CONFIG_CAPTURE_OLD);  //要发回16个这样结构体大小的数据
                ...
                switch(w * h)
                {
                    case 2688*1520:
                        //no 4m
                        str_captue.MainOption[0].ImageSize = 0x30;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 2560*1440:
                        //no 4m
                        str_captue.MainOption[0].ImageSize = 0x33;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 2304*1296:
                    case 2048*1536:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_3M;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1920 * 1080:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_1080p;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1080 * 960:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_1_3M;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1280 * 720:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_720p;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 704 * 576:
                    case 704 * 480:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_D1;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 640*480:
                    case 640*360:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_VGA;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 352*576:
                    case 352*480:
                    ...
                    case 352*288:
                    case 352*240:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_CIF;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 320*240:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_QVGA;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 176*144:
                    case 176*120:
                        str_captue.MainOption[0].ImageSize = CAPTURE_SIZE_QCIF;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    default:
                ...
                if(0 == channel_pic_info.vencStream[0].h264Conf.rc_mode){// 固定码流
                    str_captue.MainOption[0].BitRateControl = CAPTURE_BRC_CBR;  /*!< 码流控制 参照枚举capture_brc_t(DVRAPI.H) */
                    PRINT_INFO("CAPTURE_BRC_CBR\n");
                }else{
                    str_captue.MainOption[0].BitRateControl = CAPTURE_BRC_VBR;  /*!< 码流控制 参照枚举capture_brc_t(DVRAPI.H) */
                    PRINT_INFO("CAPTURE_BRC_VBR\n");
                }
                switch(channel_pic_info.vencStream[0].h264Conf.quality)
                {
                    case 0:
                        str_captue.MainOption[0].ImgQlty = 1;  /*!< 码流的画质 档次1-6 6最好 */
                        break;
                    case 1:
                ...
                str_captue.MainOption[0].Frames = channel_pic_info.vencStream[0].h264Conf.fps;    /*!< 帧率　档次N制1-6,P制1-5 */
                str_captue.MainOption[0].AVEnable = 3;   /*!< 音视频使能 1位为视频，2位为音频。ON为打开，OFF为关闭 */
                str_captue.MainOption[0].IFrameInterval = channel_pic_info.vencStream[0].h264Conf.gop;  /*!< I帧间隔帧数量，描述两个I帧之间的P帧个数，0-149, 255表示此功能不支持设置*/
                str_captue.MainOption[0].usBitRate = channel_pic_info.vencStream[0].h264Conf.bps/8;    //码率 单位为K
                ...
                switch(w * h)
                {
                    ...
                    case 2304*1296:
                    case 2048*1536:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_3M;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1920 * 1080:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_1080p;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1080 * 960:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_1_3M;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1280 * 720:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_720p;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 704 * 576:
                    case 704 * 480:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_D1;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 640*480:
                    case 640*360:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_VGA;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 352*576:
                    case 352*480:
                    ...
                    case 352*288:
                    case 352*240:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_CIF;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 320*240:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_QVGA;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 176*144:
                    case 176*120:
                        str_captue.AssiOption[0].ImageSize = CAPTURE_SIZE_QCIF;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    default:
                ...
                if(0 == channel_pic_info.vencStream[1].h264Conf.rc_mode){// 固定码流
                    str_captue.AssiOption[0].BitRateControl = CAPTURE_BRC_CBR;  /*!< 码流控制 参照枚举capture_brc_t(DVRAPI.H) */
                    PRINT_INFO("CAPTURE_BRC_CBR\n");
                }else{
                    str_captue.AssiOption[0].BitRateControl = CAPTURE_BRC_VBR;  /*!< 码流控制 参照枚举capture_brc_t(DVRAPI.H) */
                    PRINT_INFO("CAPTURE_BRC_VBR\n");
                }
                switch(channel_pic_info.vencStream[1].h264Conf.quality)
                {
                    case 0:
                        str_captue.AssiOption[0].ImgQlty = 1;  /*!< 码流的画质 档次1-6 6最好 */
                        break;
                    case 1:
                ...
                PRINT_INFO("str_captue.MainOption[0].ImgQlty:%d\n", str_captue.MainOption[0].ImgQlty);
                str_captue.AssiOption[0].Frames = channel_pic_info.vencStream[1].h264Conf.fps;    /*!< 帧率　档次N制1-6,P制1-5 */
                str_captue.AssiOption[0].AVEnable = 3;   /*!< 音视频使能 1位为视频，2位为音频。ON为打开，OFF为关闭 */
                str_captue.AssiOption[0].IFrameInterval = channel_pic_info.vencStream[1].h264Conf.gop;  /*!< I帧间隔帧数量，描述两个I帧之间的P帧个数，0-149, 255表示此功能不支持设置*/
                str_captue.AssiOption[0].usBitRate = channel_pic_info.vencStream[1].h264Conf.bps/ 8;    //码率 单位为K
                ...
                str_captue.Compression = 0;
                //实际来看此处的cover没有用
                //GK_NET_SHELTER_RECT cover;
                //if(0 == goke_api_get_ioctrl(GET_COVER_CFG, &cover))
                {
                    str_captue.CoverEnable = 0;
                    //str_captue.CoverEnable = cover.enable;
                    str_captue.Cover.left = 0;
                    str_captue.Cover.top = 0;
                    str_captue.Cover.right = 0;
                    str_captue.Cover.bottom = 0;
                }
                ...
                if(0 == goke_api_get_ioctrl(GET_OSD_CFG, &channel_info))
                {
                    ...
                    str_captue.TimeTitle.TitlefgRGBA = 0xffffff;			/*!< 标题的前景RGB，和透明度 */
                    str_captue.TimeTitle.TitlebgRGBA = 0x80000000;		/*!< 标题的后景RGB，和透明度*/
                    str_captue.TimeTitle.TitleLeft = channel_info.osdDatetime.x * 8192;			/*!< 标题距左边的距离与整长的比例*8192 */
                    str_captue.TimeTitle.TitleTop =  channel_info.osdDatetime.x * 8192;				/*!< 标题的上边的距离与整长的比例*8192 */
                    str_captue.TimeTitle.TitleRight = channel_info.osdDatetime.y * 8192;;				/*!< 标题的右边的距离与整长的比例*8192 */
                    str_captue.TimeTitle.TitleBottom = channel_info.osdDatetime.y * 8192;;				/*!< 标题的下边的距离与整长的比例*8192 */
                    str_captue.TimeTitle.TitleEnable = channel_info.osdDatetime.enable;				/*!< 标题使能 */

                    str_captue.ChannelTitle.TitlefgRGBA = 0xffffff;			/*!< 标题的前景RGB，和透明度 */
                    str_captue.ChannelTitle.TitlebgRGBA = 0x80000000;		/*!< 标题的后景RGB，和透明度*/
                    str_captue.ChannelTitle.TitleLeft = channel_info.osdChannelName.x * 8192;			/*!< 标题距左边的距离与整长的比例*8192 */
                    str_captue.ChannelTitle.TitleTop = channel_info.osdChannelName.x * 8192;				/*!< 标题的上边的距离与整长的比例*8192 */
                    str_captue.ChannelTitle.TitleRight = channel_info.osdChannelName.y * 8192;				/*!< 标题的右边的距离与整长的比例*8192 */
                    str_captue.ChannelTitle.TitleBottom = channel_info.osdChannelName.y * 8192;				/*!< 标题的下边的距离与整长的比例*8192 */
                    str_captue.ChannelTitle.TitleEnable = channel_info.osdChannelName.enable;				/*!< 标题使能 */
                }
                ...
                switch(w * h)
                {
                    ...
                    case 2304*1296:
                    case 2048*1536:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_3M;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1920 * 1080:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_1080p;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1080 * 960:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_1_3M;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 1280 * 720:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_720p;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 704 * 576:
                    case 704 * 480:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_D1;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 640*480:
                    case 640*360:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_VGA;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 352*576:
                    case 352*480:
                    ...
                    case 352*288:
                    case 352*240:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_CIF;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 320*240:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_QVGA;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    case 176*144:
                    case 176*120:
                        str_captue.PicOption[0].ImageSize = CAPTURE_SIZE_QCIF;   /*!< 分辨率 参照枚举capture_size_t(DVRAPI.H) */
                        break;
                    default:
                        PRINT_ERR("unknown enc w:%d,h:%d\n", w, h);
                        break;
                ...
                str_captue.PicOption[0].BitRateControl = CAPTURE_BRC_CBR;  /*!< 码流控制 参照枚举capture_brc_t(DVRAPI.H) */
                switch(channel_pic_info.vencStream[0].h264Conf.quality)
                {
                    case 0:
                        str_captue.PicOption[0].ImgQlty = 1;  /*!< 码流的画质 档次1-6 6最好 */
                        break;
                    case 1:
                ...
                str_captue.PicOption[0].Frames = 15;    /*!< 帧率　档次N制1-6,P制1-5 */
                str_captue.PicOption[0].AVEnable = 1;   /*!< 音视频使能 1位为视频，2位为音频。ON为打开，OFF为关闭 */
                str_captue.PicOption[0].IFrameInterval = 50;  /*!< I帧间隔帧数量，描述两个I帧之间的P帧个数，0-149, 255表示此功能不支持设置*/
                ...
                //#ifdef ACKDATA
                #if 0
                unsigned char a37f[] = "\xb3\x00\x00\x68\xb4\x00\x00\x00\x63\x6f\x6e\x66\x69\x67\x00\x00" \
                    "\x7f\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" \
                    "\x49\x50\x43\x00\x00\x00\x00\x00\x16\x00\x04\x19\x01\x32\x00\x10" \
                    "\x16\x00\x04\x19\x01\x32\x00\x10\x16\x00\x04\x19\x01\x32\x00\x10" \
                    "\x00\x00\x04\x19\x01\x32\x00\x02\x00\x00\x04\x03\x00\x06\x00\x02" \
                    "\x00\x00\x04\x19\x00\x32\x00\x02\x08\x00\x00\x00\x00\x00\x00\x00" \
                    "\x00\x00\x00\x00\x00\x04\x00\x00\x00\x04\x00\x00\xff\xff\xff\x00" \
                    "\x00\x00\x00\x80\xc7\x14\x60\x01\xf9\x1e\x01\x03\x01\x12\xb9\xda" \
                    "\xff\xff\xff\x00\x00\x00\x00\x80\x94\x00\x57\x1d\xed\x06\xf8\x1e" \
                    "\x01\x22\x62\x22\x16\xf6\x50\xff\xaa\xb6\x94\x4d\x16\x0f\x50\xff" \
                    "\x86\x2d\x8c\x65\x7e\xa3\xd7\x40\xe6\xd4\xb1\x73\xcb\xcb\x09\x97" \
                    "\x1f\x16\x77\xdd\xa1\x95\xbc\xc9\x49\xe0\xe2\x22\xfa\xf7\x36\x07" \
                    "\x5d\x11\x36\x79\x26\x85\xb2\x0a\x33\x76\x1f\x27\x72\x9f\xaa\x9d" \
                    "\x87\x04\x8b\xba";
                t_msg_head->dvrip.dvrip_extlen = sizeof(a37f) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, a37f, 32);
                memcpy(send_msg + 108, a37f + 108, sizeof(a37f) - 108);
                #endif
                break;
            }
        default:
...
int ZhiNuo_Search_Channel_Name(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    char tem_buf[DVRIP_HEAD_T_SIZE] = {0};
    char strName[CHANNEL_NAME_LEN] = {0};	//!一个通道的通道名称
    ...
    //查询通道名称类别 0:老的通道协议 1:新的字符串形式的协议
    ...
    if((0 == ret) && (strlen(channelInfo.osdChannelName.text) > 0))
    {
        ...
        utility_gbk_to_utf8(channelInfo.osdChannelName.text, strName, sizeof(strName)-1);
    } else{
        sprintf(strName, " ");
    }
    ...
    t_msg_head->dvrip.dvrip_extlen = CHANNEL_NAME_LEN * 16; //把字符串最后那两个&&符号发出去
    ...
    //#ifdef ACKDATA
    #if 0
    unsigned char ackData0[] = "";
    unsigned char ackData1[] = "\xb8\x00\x00\x68\x09\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xe5\xa4\xa7\xe5\x8d\x8e\x49\x50\x43";
    if (t_msg_head->c[8] == 0)
    {
        memcpy(send_msg, ackData0, sizeof(ackData0));
        t_msg_head->dvrip.dvrip_extlen = sizeof(ackData0) - DVRIP_HEAD_T_SIZE;
    }
    else if (t_msg_head->c[8] == 1)
    {
        memcpy(send_msg, ackData1, sizeof(ackData1));
        t_msg_head->dvrip.dvrip_extlen = sizeof(ackData1) - DVRIP_HEAD_T_SIZE;
    }
    #endif
...
int ZhiNuo_Search_System_Info(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    switch(search_type)
    {
        ...
        case 1:
            {
                PRINT_INFO("search_type:%d, 查询设备属性信息\n", search_type);
                //抓包获得
                unsigned char ack2[]="";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack2) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack2, sizeof(ack2));
            }
            break;
        case 0x2:
            {
                PRINT_INFO("search_type:%d, 查询设备属性信息\n", search_type);
                //抓包获得
                unsigned char ack[]="";
                int ret, i;
                ret = string2hex(ack, send_msg);
                printhex(send_msg, ret);

                #ifdef ACKDATA
//#if 0
                //抓包获得
                unsigned char ack2[]="";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack2) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack2, sizeof(ack2));
                #endif
            }
        ...
        case 7:
            {
                PRINT_INFO("search_type:%d, 查询设备ID号\n", search_type);
                //抓包获得
...
//#if 0
                //抓包获得
                unsigned char ack2[]="\xb4\x00\x00\x68\x10\x00\x00\x00\x07\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x34\x45\x30\x34\x45\x39\x32\x50\x41\x47\x43\x44\x44\x41\x43\x00";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack2) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack2, sizeof(ack2));
                #endif
            }
        ...
        case 8:
            {
                PRINT_INFO("search_type:%d, 查询设备版本\n", search_type);
                //抓包获得
                unsigned char ack[]="b40000580f000000080000000000000000000000000000000000000000000000322e3430302e303030302e302e5200";
                int ret, i;
                ret = string2hex(ack, send_msg);
...
//#if 0
                //抓包获得
                unsigned char ack2[]="\xb4\x00\x00\x68\x12\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x32\x2e\x36\x32\x32\x2e\x30\x30\x30\x30\x30\x30\x30\x2e\x35\x2e\x52\x00";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack2) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack2, sizeof(ack2));
                #endif
            }
        ...
        case 14:
            {
                PRINT_INFO("search_type:%d, 查询视频动态检测属性信息\n", search_type);
...
//#if 0
                //抓包获得
                unsigned char ack[]="\xb4\x00\x00\x00\x00\x00\x00\x00\x0e\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
            }
        ...
        case 15:
            {
                PRINT_INFO("search_type:%d, 查询视频区域遮挡属性信息\n", search_type);
...
//#if 0
                //抓包获得
                unsigned char ack[]="";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
            }
        ...
        case 16:
            {
                ...
                //#if 0
                //抓包获得
                unsigned char ack[]="\xb4\x00\x00\x00\x00\x00\x00\x00\x10\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
            }
        ...
        case 26: //0x1a
            {//查询设备功能列表
                ...
                //抓包获得
                unsigned char ack[]="";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
            }
            break;
        case 31:
            {//31:功能能力查询
                PRINT_INFO("search_type:%d, 查询功能能力查询\n", search_type);
                //查询的抓拍参数nvr暂时没有用到，具体值以后可以再更改
                t_msg_head->dvrip.dvrip_extlen =
        ...
        case 32://视频前端采集能力查询
            {
                PRINT_INFO("search_type:%d, 查询视频前端采集能力\n", search_type);
                ...
                #ifdef ACKDATA
                unsigned char ack[]="\xb4\x00\x00\x00\x00\x00\x00\x00\x20\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
                t_msg_head->dvrip.dvrip_extlen = sizeof(ack) - DVRIP_HEAD_T_SIZE;
                memcpy(send_msg, ack, sizeof(ack));
                #endif
            }
    ...
    t_msg_head->c[12] = ex_type;  //这个暂时没用，但是智诺要求最好填上。
    t_msg_head->c[16] = 0; //返回码 0:正常 1:无权限 5:无对应信息提供
    t_msg_head->c[20] = function_capacity; //功能能力号
...
int Dahua_ConnectRequest(char* bufPost, DAHUA_SESSION_CTRL* pCtrl)
{
    ...
    //请求类型(1为实时监控，2，多画面预览，3，语音对讲，4，下载回放）
    ...
    t_resp_msg_head->l[2] = n_flag; //登陆的唯一标识号,也就是0xb0返回的那个 大华NVR始终是46
    t_resp_msg_head->c[12] = ch_request_type; //请求类型
    t_resp_msg_head->c[13] = ch_channel_num; //请求通道号

    t_resp_msg_head->c[14] = 0; //返回码 0:连接映射成功 1:失败 2:子链接已建立

    //bruce
    //ch_request_type, 1 主码流， 2 次码流
...
{
    PRINT_INFO("***************************\n");
    PRINT_INFO("TransactionID:%u\n", p->transaction_id);
    PRINT_INFO("Method:%s\n", p->method);
    PRINT_INFO("ParameterName:%s\n", p->parameter_name);
    PRINT_INFO("SessionID:%u\n", p->sid);
    PRINT_INFO("ConnectionID:%u\n", p->connect_id);
    PRINT_INFO("***************************\n");
}
#endif
...
int Dahua_ControlConnect(char* bufPost, DAHUA_SESSION_CTRL* pCtrl)
{
    ...
    if (!strcmp(m_f.parameter_name, "Dahua.Device.Network.Monitor.General")) {
        ...
        if (m_f.stream == UT_MAIN_STREAM) {
            //把 tmp_sock 添加成 main_stream_sock, 并打开码流
            UtSetLvSockStatus2(pCtrl->fSessionInt, 1, UT_MAIN_STREAM, &g_dahua_session);
        } else {
            //把 tmp_sock 添加成 sub_stream_sock, 并打开码流
        ...
        // 把sock 通过 sid 和命令sock联系在一起
        #if MULTI_CONNECT
        /* 将当前socket添加进数组，用来做媒体流传输
         * 当前还不确定是用于传输主码流还是子码流，多一个connection_id
         */
...
int ZhiNuo_I_Frame_Request(char* bufPost, DAHUA_SESSION_CTRL * pCtrl)
{
    ...
    //码流通道号 c[12] --（0~64） 0-主码流 1-辅码流1  2-辅码流2  3 -辅码流3
    PRINT_INFO("requeset_type:%d, channel_num:%d\n", t_msg_head->c[12], t_msg_head->c[8]);

    if(0 == t_msg_head->c[12]) { //主码流请求I帧
        #if MULTI_CONNECT
        ...
        send_sub = 0;
        #endif
    } else if(1 == t_msg_head->c[12]) { //次码流请求I帧
        #if MULTI_CONNECT
    ...
    t_msg_head->c[8] = 0; //返回码 ―― 0-成功，1-失败
    ZhiNuo_SockSend(pCtrl, send_msg, DVRIP_HEAD_T_SIZE, 100);
...
int ZhiNuo_DEV_Control(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    t_msg_head->c[12] = ch_control_type;//返回码0:成功1:没权限2:暂时无法执行
...
int ZhiNuo_Set_Config(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    //打印搜索类型 16:编码能力信息 127:双码流图像配置 126:颜色配置 124:获得事件配置
    ...
    if(0 == ch_param_type)
    {
        ...
        if (pTime->c[16] == 0) { // 0 . ; 1 - ; 2 /
            ...
            if (pTime->c[15] == 0) { // 0 年月日， 1 月日年， 2 日月年
    ...
    if(34 == ch_param_type)
    {
        ...
        PRINT_INFO("search_type:%d, 设置视频区域遮挡, num:%d, check_recv_len:%d\n", ch_param_type, video_cover->iCoverNum,
    ...
    if(38 == ch_param_type)
    {
        ...
        switch(cam_control->AutoColor2BW)/*0-总是彩色 1- 自动 2-总是黑白*/
        {
            case 0:
        ...
        t_msg_head->c[9] = 0; //0：不需要重启 1：需要重启才能生效
    }
    if(126 == ch_param_type)
    {
        ...
        str_channel_color_info.contrast = str_color->Color[0].Contrast;			/*!< 对比度	0-100		*/
        str_channel_color_info.saturation = str_color->Color[0].Saturation;			/*!< 饱和度	0-100		*/
    ...
    else if(124 == ch_param_type)
    {
        if(4 == ch_child_type_limit)
        {
            ...
            //2014-05-14：智诺3.3版本之前NVR，str_motion_detect.bEnable字段是无效的，因此用区域是否有效作为是否启动移动侦测标记
            //智诺是22*18格子，整形数组第一个元素的前22位有效，一个位标示一个格子是否选中，总共18个元素有效
            int x = 0;
            int y = 0;
            int enable_flag = 0;
            for(y = 0; y < 18; y++)
            ...
            str_dms_motion_detect.dwSensitive = (str_motion_detect->iLevel - 1) * 100 / 5; //智诺等级为1-6转化为我们的0-100
...
int ZhiNuo_Time_Manage(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    if(0 == ch_cmd_code)
    {
        ...
        t_msg_head->c[9] = 0;  //返回码0:成功1:出错2:无权限3: 时间冲突，设置失败
...
int ZhiNuo_Set_Channel_Name(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    if((0 == ret) && (strlen(channel_info.osdChannelName.text) > 0))
    {
        ...
        memcpy(channel_info.osdChannelName.text, extra, n_extra_len);//把"&&"这个去掉
    ...
    {
        char name[128]={0};
        utility_utf8_to_gbk(extra, name, sizeof(name));
        runChannelCfg.channelInfo[0].osdChannelName.enable = 1;
        netcam_video_set_stream_name(0, name);
        //sprintf(runChannelCfg.channelInfo[0].osdChannelName.text, "%s", extra);
        runChannelCfg.channelInfo[1].osdChannelName.enable = 1;
        netcam_video_set_stream_name(1, name);
        //sprintf(runChannelCfg.channelInfo[1].osdChannelName.text, "%s", extra);
...
int ZhiNuo_PTZ_Control(char* bufPost, DAHUA_SESSION_CTRL *sock)
{
    ...
    switch(cmd)
    {
        case ZhiNuo_PTZ_CMD_STOP:		/*停止云台活动*/
            {
                PRINT_INFO("PTZ STOP\n");
        ...
        case ZhiNuo_PTZ_CMD_RIGHT_DOWN:
            ...
            goke_api_set_ioctrl(SET_PTZ_CMD, &ctrol_cmd);
            break;
        case ZhiNuo_PTZ_CMD_ZOOM_SUB:	/*变倍+*/
            {
                PRINT_INFO("PTZ ZOOM SUB\n");
                break;
            }
        case ZhiNuo_PTZ_CMD_ZOOM_ADD:	/*变倍-*/
            {
                PRINT_INFO("PTZ ZOOM ADD\n");
                break;
            }
        case ZhiNuo_PTZ_CMD_FOCUS_ADD:	/*聚焦+*/
            {
                PRINT_INFO("PTZ FOCUS ADD\n");
                break;
            }
        case ZhiNuo_PTZ_CMD_FOCUS_SUB:	/*聚焦-*/
            {
                PRINT_INFO("PTZ FOCUS SUB\n");
                break;
            }
        case ZhiNuo_PTZ_CMD_IRIS_ADD: 	/*光圈+*/
            {
                PRINT_INFO("PTZ IRIS ADD\n");
                break;
            }
        case ZhiNuo_PTZ_CMD_IRIS_SUB: 	/*光圈-*/
            {
                PRINT_INFO("PTZ IRIS SUB\n");
                break;
            }
        case ZhiNuo_PTZ_CMD_LIGHT_BRUSH: /*灯光雨刷*/
            {
                if(0x01 == param1) //0x01开启 0x00关闭
                {
        ...
        case ZhiNuo_PTZ_CMD_AUTO:			/*云台自动扫描*/
            {
                // 76 开始  99 自动 96 停止
                if(76 == param3)
                {
        ...
        case ZhiNuo_PTZ_CMD_ADD_POS_CRU:		/* 将预置点加入巡航序列 */
            {
                PRINT_INFO("PTZ ADD POS CRU\n");
                break;
            }
        case ZhiNuo_PTZ_CMD_DEL_POS_CRU:		/* 将预置点从巡航序列中删除 */
            {
                PRINT_INFO("PTZ DEL POS CRU\n");
                return rcv_msg_len;
            }
        case ZhiNuo_PTZ_CMD_CLR_POS_CRU:		/* 清空巡航点 */
            {
        ...
        case ZhiNuo_PTZ_CMD_PRESET_GO:		/*调到预设的预置点*/
            {
        ...
        case ZhiNuo_PTZ_CMD_PRESET_SET:		/*设置预置点*/
            {
...
static int Dahua_DevVideoInput_GetCaps(DAHUA_SESSION_CTRL *pCtrl, int id, int sid, int result)
{
    ...
    cJSON_AddTrueToObject(caps, "BrightnessCompensation"); //true 亮度补偿
    ...
    cJSON_AddFalseToObject(caps, "FishEye"); //鱼眼
    ...
    cJSON_AddNumberToObject(caps, "GlareInhibition", 1); // 强光抑制
...
static int Dahua_DevVideoInput_GetCapsEx(DAHUA_SESSION_CTRL *pCtrl, int id, int sid, int result)
{
    ...
    cJSON_AddTrueToObject(VideoInColor, "Sharpness");
...
static int Dahua_DevVideoEncode_GetCaps(DAHUA_SESSION_CTRL *pCtrl, int id, int sid, int result)
{
    ...
    cJSON_AddItemToArray(encodeTypes, cJSON_CreateString("H.265"));
...
static int Dahua_MagicBox_GetProductDefinition(DAHUA_SESSION_CTRL *pCtrl, int id, int sid, cJSON *json)
{
    ...
    if (!strcmp(name, "CifPFrameSize")) {
        ...
        cJSON_AddFalseToObject(root, "result");
        cJSON_AddItemToObject(root, "params", params = cJSON_CreateNull());
        cJSON_AddNullToObject(params, "error");
        cJSON_AddNumberToObject(root, "id", id);
        cJSON_AddNumberToObject(root, "session", sid);
    } else {
...
static void Dahua_Init_Cfg_Profile()
{
    ...
    if (first_get_cfg_profile == 1) {
        ...
        if (image.irCutMode == 1) { //彩色 白天
            nDayNightColor = 0;
        } else if (image.irCutMode == 2) { //黑白 晚上
            nDayNightColor = 2;
        } else { //自动
            nDayNightColor = 1;
        }
...
static void add_CapsFormat(int chId, cJSON *extraFormat, GK_NET_VIDEO_CFG videoConfig)
{
    cJSON *extraFormat1;
    cJSON *e_audio, *e_a_compressionTypes;
    cJSON *e_video, *e_bitRateOptions, *e_v_compressionTypes, *e_h264Profile, *e_resolutionTypes;
    char *resolution_str = (char*) videoConfig.vencStream[chId].resolution;
    ...
    if (chId == 0)
    {
        cJSON_AddItemToArray(e_bitRateOptions, cJSON_CreateNumber(512));
        cJSON_AddItemToArray(e_bitRateOptions, cJSON_CreateNumber(4096));
        cJSON_AddItemToObject(e_video, "BitRateSuggested", cJSON_CreateNumber(2048));
    }
    else
    {
    ...
    cJSON_AddItemToArray(e_h264Profile, cJSON_CreateString("Main"));
    ...
    if(strstr(resolution_str, "2688x1520")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("2688x1520"));
    }

    if(strstr(resolution_str, "2560x1440")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("2560x1440"));
    }

    if(strstr(resolution_str, "2048x1536")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("3M"));
    }
    if(strstr(resolution_str, "2304x1296")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("2304x1296"));
    }
    if(strstr(resolution_str, "1920x1080")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("1080P"));
    }
    if(strstr(resolution_str, "1280x960")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("1_3M"));
    }
    if(strstr(resolution_str, "1280x720")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("720P"));
    }
    if(strstr(resolution_str, "704x576") || strstr(resolution_str, "704x480")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("D1"));
    }
    if(strstr(resolution_str, "720x576")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("720x576"));
    }
    if(strstr(resolution_str, "640x480") || strstr(resolution_str, "640x360")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("VGA"));
    }
    if(strstr(resolution_str, "352x576") || strstr(resolution_str, "352x480")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("HD1"));
    }
    if(strstr(resolution_str, "352x288") || strstr(resolution_str, "352x240")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("CIF"));
    }
    if(strstr(resolution_str, "320x240")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("QVGA"));
    }
    if(strstr(resolution_str, "176x144") || strstr(resolution_str, "176x120")){
        cJSON_AddItemToArray(e_resolutionTypes, cJSON_CreateString("QCIF"));
    }
...
static void add_CapsMainFormat(cJSON *mainFormat)
{
    ...
    //cJSON_AddItemToArray(e_v_compressionTypes, cJSON_CreateString("MJPG"));
    cJSON_AddItemToArray(m_v_compressionTypes, cJSON_CreateString("H.265"));
    ...
    cJSON_AddItemToArray(m_h264Profile, cJSON_CreateString("Main"));
    ...
    //CAPTURE_SIZE_T
    cJSON_AddItemToArray(m_resolutionTypes, cJSON_CreateString("3M")); //diff
...
static void add_CapsSnapFormat(cJSON *snapFormat)
{
    ...
    cJSON_AddItemToObject(snapVideo, "MaxROICount", cJSON_CreateNumber(0));
...
static int Dahua_Encode_GetConfigCaps(DAHUA_SESSION_CTRL *pCtrl, int id, int sid, int result, cJSON *json)
{
    char *out;
    cJSON *root;
    root = cJSON_CreateObject();//创建项目
    ...
    GK_NET_VIDEO_CFG videoConfig;
    GK_NET_VIDEO_CFG videoConfigTmp;
    memset(&videoConfig, 0, sizeof(GK_NET_VIDEO_CFG));
    goke_api_get_ioctrl(GET_VIDEO_CFG, &videoConfig);
    memcpy(&videoConfigTmp, &videoConfig, sizeof(GK_NET_VIDEO_CFG));
    PRINT_INFO("-->main2,w:%d, h:%d, enctype:%d, bitc:%d, kbps:%d, fps:%d, gop:%d\n", videoConfig.vencStream[0].h264Conf.width, videoConfig.vencStream[0].h264Conf.height,
        videoConfig.vencStream[0].enctype, videoConfig.vencStream[0].h264Conf.rc_mode, videoConfig.vencStream[0].h264Conf.bps,
        videoConfig.vencStream[0].h264Conf.fps, videoConfig.vencStream[0].h264Conf.gop);
    PRINT_INFO("-->ext2,w:%d, h:%d, enctype:%d, bitc:%d, kbps:%d, fps:%d, gop:%d\n", videoConfig.vencStream[1].h264Conf.width, videoConfig.vencStream[1].h264Conf.height,
        videoConfig.vencStream[1].enctype, videoConfig.vencStream[1].h264Conf.rc_mode, videoConfig.vencStream[1].h264Conf.bps,
        videoConfig.vencStream[1].h264Conf.fps, videoConfig.vencStream[1].h264Conf.gop);

    /* ExtraFormat */
    cJSON *paramsSet = cJSON_GetObjectItem(json, "params");
    cJSON *configs = cJSON_GetObjectItem(paramsSet, "config");
    cJSON *config = cJSON_GetArrayItem(configs, 0);
    cJSON *ExtraFormats = cJSON_GetObjectItem(config, "ExtraFormat");
    cJSON *ExtraFormat0 = cJSON_GetArrayItem(ExtraFormats, 0);
    cJSON *ExtraFormat0Video = cJSON_GetObjectItem(ExtraFormat0, "Video");
    cJSON *ExtraFormat0Widht = cJSON_GetObjectItem(ExtraFormat0Video, "Width");
    cJSON *ExtraFormat0Height = cJSON_GetObjectItem(ExtraFormat0Video, "Height");
    cJSON *ExtraFormat0FPS = cJSON_GetObjectItem(ExtraFormat0Video, "FPS");
    cJSON *ExtraFormat0BitRateControl = cJSON_GetObjectItem(ExtraFormat0Video, "BitRateControl");
    cJSON *ExtraFormat0GOP = cJSON_GetObjectItem(ExtraFormat0Video, "GOP");
    cJSON *ExtraFormat0BitRate = cJSON_GetObjectItem(ExtraFormat0Video, "BitRate");
    cJSON *ExtraFormat0Compression = cJSON_GetObjectItem(ExtraFormat0Video, "Compression");
    PRINT_INFO("-->ExtraFormat,w:%d,h:%d,compress:%s\n", ExtraFormat0Widht->valueint, ExtraFormat0Height->valueint,
        ExtraFormat0Compression->valuestring);
    videoConfigTmp.vencStream[1].h264Conf.width = ExtraFormat0Widht->valueint;
    videoConfigTmp.vencStream[1].h264Conf.height = ExtraFormat0Height->valueint;
    videoConfigTmp.vencStream[1].h264Conf.fps = ExtraFormat0FPS->valueint;
    if (strcmp(ExtraFormat0BitRateControl->valuestring, "CBR") == 0)
    {
        videoConfigTmp.vencStream[1].h264Conf.rc_mode = 0;
    }
    else
    {
        videoConfigTmp.vencStream[1].h264Conf.rc_mode = 1;
    }
    videoConfigTmp.vencStream[1].h264Conf.gop = ExtraFormat0GOP->valueint;
    videoConfigTmp.vencStream[1].h264Conf.bps = ExtraFormat0BitRate->valueint;
    if (strcmp(ExtraFormat0Compression->valuestring, "H.264") == 0)
    {
        videoConfigTmp.vencStream[1].enctype = 1;
    }
    else if (strcmp(ExtraFormat0Compression->valuestring, "H.265") == 0)
    {
        videoConfigTmp.vencStream[1].enctype = 3;
    }
    else if (strcmp(ExtraFormat0Compression->valuestring, "MJPG") == 0)
    {
        videoConfigTmp.vencStream[1].enctype = 2;
    }
    else
    {
        videoConfigTmp.vencStream[1].enctype = 0;
    }

    cJSON *MainFormats = cJSON_GetObjectItem(config, "MainFormat");
    cJSON *MainFormat0 = cJSON_GetArrayItem(MainFormats, 0);
    cJSON *MainFormat0Video = cJSON_GetObjectItem(MainFormat0, "Video");
    cJSON *MainFormat0Widht = cJSON_GetObjectItem(MainFormat0Video, "Width");
    cJSON *MainFormat0Height = cJSON_GetObjectItem(MainFormat0Video, "Height");
    cJSON *MainFormat0FPS = cJSON_GetObjectItem(MainFormat0Video, "FPS");
    cJSON *MainFormat0BitRateControl = cJSON_GetObjectItem(MainFormat0Video, "BitRateControl");
    cJSON *MainFormat0GOP = cJSON_GetObjectItem(MainFormat0Video, "GOP");
    cJSON *MainFormat0BitRate = cJSON_GetObjectItem(MainFormat0Video, "BitRate");
    cJSON *MainFormat0Compression = cJSON_GetObjectItem(MainFormat0Video, "Compression");
    PRINT_INFO("-->MainFormat,w:%d,h:%d,compress:%s\n", MainFormat0Widht->valueint, MainFormat0Height->valueint,
        MainFormat0Compression->valuestring);
    videoConfigTmp.vencStream[0].h264Conf.width = MainFormat0Widht->valueint;
    videoConfigTmp.vencStream[0].h264Conf.height = MainFormat0Height->valueint;
    videoConfigTmp.vencStream[0].h264Conf.fps = MainFormat0FPS->valueint;
    if (strcmp(MainFormat0BitRateControl->valuestring, "CBR") == 0)
    {
        videoConfigTmp.vencStream[0].h264Conf.rc_mode = 0;
    }
    else
    {
        videoConfigTmp.vencStream[0].h264Conf.rc_mode = 1;
    }
    videoConfigTmp.vencStream[0].h264Conf.gop = MainFormat0GOP->valueint;
    videoConfigTmp.vencStream[0].h264Conf.bps = MainFormat0BitRate->valueint;
    if (strcmp(MainFormat0Compression->valuestring, "H.264") == 0)
    {
        videoConfigTmp.vencStream[0].enctype = 1;
    }
    else if (strcmp(MainFormat0Compression->valuestring, "H.265") == 0)
    {
        videoConfigTmp.vencStream[0].enctype = 3;
    }
    else if (strcmp(MainFormat0Compression->valuestring, "MJPG") == 0)
    {
        videoConfigTmp.vencStream[0].enctype = 2;
    }
    else
    {
        videoConfigTmp.vencStream[0].enctype = 0;
    }

    //memcpy(&videoConfigTmp, &videoConfig, sizeof(GK_NET_VIDEO_CFG));
    if(memcmp(&videoConfigTmp.vencStream[0], &videoConfig.vencStream[0], sizeof(GK_NET_VENC_STREAM)) != 0
        || memcmp(&videoConfigTmp.vencStream[1], &videoConfig.vencStream[1], sizeof(GK_NET_VENC_STREAM)) != 0)
    {
        PRINT_INFO("-->main2,w:%d, h:%d, enctype:%d, bitc:%d, kbps:%d, fps:%d, gop:%d\n", videoConfigTmp.vencStream[0].h264Conf.width, videoConfigTmp.vencStream[0].h264Conf.height,
            videoConfigTmp.vencStream[0].enctype, videoConfigTmp.vencStream[0].h264Conf.rc_mode, videoConfigTmp.vencStream[0].h264Conf.bps,
            videoConfigTmp.vencStream[0].h264Conf.fps, videoConfigTmp.vencStream[0].h264Conf.gop);
        PRINT_INFO("-->ext2,w:%d, h:%d, enctype:%d, bitc:%d, kbps:%d, fps:%d, gop:%d\n", videoConfigTmp.vencStream[1].h264Conf.width, videoConfigTmp.vencStream[1].h264Conf.height,
            videoConfigTmp.vencStream[1].enctype, videoConfigTmp.vencStream[1].h264Conf.rc_mode, videoConfigTmp.vencStream[1].h264Conf.bps,
            videoConfigTmp.vencStream[1].h264Conf.fps, videoConfigTmp.vencStream[1].h264Conf.gop);
        goke_api_set_ioctrl(SET_VIDEO_CFG, &videoConfigTmp);
    }
    else
    {
        PRINT_INFO("-->enc not change\n");
    }
    ...
    add_CapsFormat(1, extraFormat, videoConfigTmp);
    ...
    //add_CapsMainFormat(mainFormat);
    //add_CapsMainFormat(mainFormat);
    add_CapsFormat(0, mainFormat, videoConfigTmp);
    add_CapsFormat(0, mainFormat, videoConfigTmp);
    add_CapsFormat(0, mainFormat, videoConfigTmp);
    ...
    add_CapsSnapFormat(snapFormat); // 抓包，发了3次
...
static void add_Color(cJSON *table00, int index, int flag)
{
    if (flag == 1) {
	    ...
	    tmp_image_cfg.contrast = 50;
	    tmp_image_cfg.brightness = 50;
	    tmp_image_cfg.saturation = 50;
	    tmp_image_cfg.sharpness = 50;
    } else {
...
static int Dahua_Cfg_GetVideoInOptions(int id, int sid, cJSON *root, int flag)
{
    ...
    cJSON_AddNumberToObject(table0, "Sharpness", image.sharpness);
...
static void add_VideoInDayNight(cJSON *table00, int index)
{
    ...
    if (stDahuaColorProfile[index].day_night_color == 0)
        cJSON_AddStringToObject(table00, "Mode", "Color"); //白天，彩色
    else if (stDahuaColorProfile[index].day_night_color == 2)
        cJSON_AddStringToObject(table00, "Mode", "BlackWhite"); //晚上，黑白
    else
        cJSON_AddStringToObject(table00, "Mode", "Brightness"); //自动
...
static void add_EncMainFormat(cJSON *mainFormat)
{
    ...
    cJSON_AddStringToObject(m_video1, "Profile", "Baseline");//"High"
    ...
    PRINT_INFO("runVideoCfg.vencStream[0].enctype:%d\n", runVideoCfg.vencStream[0].enctype);

    if(runVideoCfg.vencStream[0].enctype == 3)
    {
        cJSON_AddItemToObject(m_video1, "Compression", cJSON_CreateString("H.265"));
    }
    else if(runVideoCfg.vencStream[0].enctype == 1)
    {
...
static void add_EncExtraFormat(cJSON *extraFormat, int v_enable, int a_enable)
{
    ...
    cJSON_AddStringToObject(e_video, "Profile", "Baseline");//"High"
    ...
    if (runVideoCfg.vencStream[1].enctype == 3)
    {
        cJSON_AddItemToObject(e_video, "Compression", cJSON_CreateString("H.265"));
    }
    else
    {
...
static void add_EncSnapFormat(cJSON *snapFormat)
{
    ...
    cJSON_AddStringToObject(snapVideo1, "Profile", "Baseline");
...
static int Dahua_Cfg_GetEncode(int id, int sid, cJSON *root)
{
    ...
    add_EncMainFormat(mainFormat); // 3种主码流分辨率
    ...
    add_EncSnapFormat(snapFormat);  // 3种主码流分辨率
    add_EncSnapFormat(snapFormat);  // 3种主码流分辨率
    add_EncSnapFormat(snapFormat);  // 3种主码流分辨率
...
{
    const int nBackColor[4] = {0, 0, 0, 0};
...
static void add_cover_item(cJSON *title, int nEncodeBlend, int nPreviewBlend, int rect[])
{
    //cJSON *CoversItem;
    //cJSON_AddItemToArray(title, CoversItem = cJSON_CreateObject());
    cJSON *BackColor;
    cJSON_AddItemToArray(title, BackColor = cJSON_CreateObject());
    add_title(BackColor, nEncodeBlend, nPreviewBlend, rect);
}



// flag == 1, get default
static int Dahua_Cfg_GetVideoWidget(int id, int sid, cJSON *root, int flag)
{
    //
    GK_NET_SHELTER_RECT cover[4];

    cJSON *params, *table, *tableArray;
    cJSON_AddNumberToObject(root, "id", id);
    cJSON_AddItemToObject(root, "params", params = cJSON_CreateObject());
    //cJSON_AddItemToObject(params, "table", tableArray = cJSON_CreateArray());
    cJSON_AddItemToObject(params, "table", table  = cJSON_CreateObject());
    //cJSON_AddItemToArray(tableArray, table = cJSON_CreateObject());
    ...
    runChannelCfg.channelInfo[0].osdChannelName.enable = 1;
    int rect[4] = {0, 7581, 2621, 8017};
    rect[0] = (int)(runChannelCfg.channelInfo[0].osdChannelName.x * 8192);
    rect[1] = (int)(runChannelCfg.channelInfo[0].osdChannelName.y * 8192);
    add_title(ChannelTitle, runChannelCfg.channelInfo[0].osdChannelName.enable, 1, rect);
    //todo
    cJSON *Covers;
    cJSON_AddItemToObject(table, "Covers", Covers = cJSON_CreateArray());
    int rect1[4] = {0, 0, 1032, 1017};
    if(0 == goke_api_get_ioctrl(GET_COVER_CFG, cover))
    {
        int i;
        for(i = 0; i < 4; i++)
        {
            rect1[0] = (int)(cover[i].x * 8192.0);
            rect1[1] = (int)(cover[i].y * 8192.0);
            rect1[2] = rect1[0] + (int)(cover[i].width * 8192.0);
            rect1[3] = rect1[1] + (int)(cover[i].height * 8192.0);
            add_cover_item(Covers, cover[i].enable, 0, rect1);
        }
    }


    cJSON *CustomTitle;
    cJSON_AddItemToObject(table, "CustomTitle", CustomTitle = cJSON_CreateArray());
    cJSON_AddItemToArray(CustomTitle, cJSON_CreateObject());
    cJSON *bkItem;
    cJSON_AddItemToArray(CustomTitle, bkItem = cJSON_CreateObject());
    int rectCustomTitle[4] = {5324, 7436, 7945, 7901};
    add_title(bkItem, 1, 1, rectCustomTitle);
    cJSON_AddStringToObject(bkItem, "Text", "||");
    cJSON_AddNumberToObject(bkItem, "TextAlign", 2);
    cJSON_AddItemToArray(CustomTitle, cJSON_CreateObject());
    cJSON_AddItemToArray(CustomTitle, cJSON_CreateObject());

    cJSON *FontSize;
    cJSON_AddItemToObject(table, "FontSize", FontSize = cJSON_CreateNumber(24));
    cJSON *FontSizeExtra1;
    cJSON_AddItemToObject(table, "FontSizeExtra1", FontSizeExtra1 = cJSON_CreateNumber(10));
    cJSON *FontSizeExtra2;
    cJSON_AddItemToObject(table, "FontSizeExtra2", FontSizeExtra2 = cJSON_CreateNumber(10));
    cJSON *FontSizeExtra3;
    cJSON_AddItemToObject(table, "FontSizeExtra3", FontSizeExtra3 = cJSON_CreateNumber(10));
    cJSON *FontSizeScale;
    cJSON_AddItemToObject(table, "FontSizeScale", FontSizeScale = cJSON_CreateNumber(1));
    cJSON *FontSizeSnapshot;
    cJSON_AddItemToObject(table, "FontSizeSnapshot", FontSizeSnapshot = cJSON_CreateNumber(10));
    cJSON *OSDMobileState;
    cJSON_AddItemToObject(table, "OSDMobileState", OSDMobileState = cJSON_CreateObject());
    cJSON *PTZCoordinates;
    cJSON_AddItemToObject(table, "PTZCoordinates", PTZCoordinates = cJSON_CreateObject());
    cJSON *PTZDirection;
    cJSON_AddItemToObject(table, "PTZDirection", PTZDirection = cJSON_CreateObject());
    cJSON *PTZOSDMenu;
    cJSON_AddItemToObject(table, "PTZOSDMenu", OSDMobileState = cJSON_CreateObject());
    cJSON *PTZOSDMenuViaApp;
    cJSON_AddItemToObject(table, "PTZOSDMenuViaApp", PTZOSDMenuViaApp = cJSON_CreateObject());
    cJSON *PTZPreset;
    cJSON_AddItemToObject(table, "PTZPreset", PTZPreset = cJSON_CreateObject());
    cJSON *PTZZoom;
    cJSON_AddItemToObject(table, "PTZZoom", PTZZoom = cJSON_CreateObject());
    cJSON *PictureTitle;
    cJSON_AddItemToObject(table, "PictureTitle", PictureTitle = cJSON_CreateObject());
    cJSON *PtzPattern;
    cJSON_AddItemToObject(table, "PtzPattern", PtzPattern = cJSON_CreateObject());
    cJSON *PtzRS485Detect;
    cJSON_AddItemToObject(table, "PtzRS485Detect", PtzRS485Detect = cJSON_CreateObject());
    cJSON *Temperature;
    cJSON_AddItemToObject(table, "Temperature", Temperature = cJSON_CreateObject());

    cJSON *TimeTitle;
    cJSON_AddItemToObject(table, "TimeTitle", TimeTitle = cJSON_CreateObject());
    int rectTimeTitle[4] = {0, 29, 2621, 465};
    rectTimeTitle[0] = (int)(runChannelCfg.channelInfo[0].osdDatetime.x * 8192);
    rectTimeTitle[1] = (int)(runChannelCfg.channelInfo[0].osdDatetime.y * 8192);
    add_title(TimeTitle, runChannelCfg.channelInfo[0].osdDatetime.enable, 1, rectTimeTitle);
    cJSON_AddTrueToObject(TimeTitle, "ShowWeek");
    cJSON_AddStringToObject(TimeTitle, "WeekPosition", "Right");
    cJSON *VoltageStatus;
    cJSON_AddItemToObject(table, "VoltageStatus", VoltageStatus = cJSON_CreateObject());
    cJSON *WideHeightRatio;
    cJSON_AddItemToObject(table, "WideHeightRatio", WideHeightRatio = cJSON_CreateNumber(0));
...
static int Dahua_Cfg_Get(DAHUA_SESSION_CTRL *pCtrl, int id, int sid, cJSON *json)
{
    ...
    } else if (!strcmp(name, "VideoWidget")) {
        //因为宽高度不好计算，不取也可以
        Dahua_Cfg_GetVideoWidget(id, sid, root, 0);
...
static int Dahua_Cfg_GetDefault(DAHUA_SESSION_CTRL *pCtrl, int id, int sid, cJSON *json)
{
    ...
    if (!strcmp(name, "VideoAnalyseRule")) {
        ...
        //因为宽高度不好计算，不取也可以
        Dahua_Cfg_GetVideoWidget(id, sid, root, 1);
    } else {
...
static int Dahua_Restore_VideoColor()
{
    ...
    image_cfg.contrast = tmp_image_cfg.contrast = 50;
    image_cfg.brightness = tmp_image_cfg.brightness = 50;
    image_cfg.saturation = tmp_image_cfg.saturation = 50;
    image_cfg.sharpness = tmp_image_cfg.sharpness = 50;
    image_cfg.hue = tmp_image_cfg.hue = 50;
    PRINT_INFO("%s, %d, contrast:%d, brightness:%d, saturation:%d, sharpness:%d, hue:%d\n",__func__, __LINE__, 
            image_cfg.contrast,image_cfg.brightness,image_cfg.saturation,image_cfg.sharpness,image_cfg.hue);
...
static void Dahua_Restore_VideoInOptions()
{
    GK_NET_IMAGE_CFG image_cfg;
    memcpy(&image_cfg, &runImageCfg, sizeof(GK_NET_IMAGE_CFG));
...
static int Dahua_Cfg_SetVideoWidget(cJSON *params)
{
    ...
    if (!ChannelTitle) {
        ...
        cJSON *EncodeBlend = cJSON_GetObjectItem(ChannelTitle, "EncodeBlend");
        if (EncodeBlend->type == cJSON_True) {
            runChannelCfg.channelInfo[0].osdChannelName.enable = 1;
            runChannelCfg.channelInfo[1].osdChannelName.enable = 1;
        } else {
            runChannelCfg.channelInfo[0].osdChannelName.enable = 0;
            runChannelCfg.channelInfo[1].osdChannelName.enable = 0;
        }
    ...
    if (!TimeTitle) {
        ...
        cJSON *EncodeBlend = cJSON_GetObjectItem(TimeTitle, "EncodeBlend");
        if (EncodeBlend->type == cJSON_True) {
            ...
            runChannelCfg.channelInfo[1].osdDatetime.enable = 0;
        }
...
static int Dahua_Cfg_SetChannelTitle(cJSON *params)
{
    ...
    if (!table0) {
        cjname = cJSON_GetObjectItem(table, "name");
        if (!cjname) {
            PRINT_ERR("cjson get item error!\n");
            return -1;
        }
    } else {
        cjname = cJSON_GetObjectItem(table0, "Name");
        if (!cjname) {
            PRINT_ERR("cjson get item error!\n");
            return -1;
        }
    ...
    runChannelCfg.channelInfo[0].osdChannelName.enable = 1;
    netcam_video_set_stream_name(0, cjname->valuestring);
    //sprintf(runChannelCfg.channelInfo[0].osdChannelName.text, "%s", cjname->valuestring);
    runChannelCfg.channelInfo[1].osdChannelName.enable = 1;    
    netcam_video_set_stream_name(1, cjname->valuestring);
...
static int DahuaSetVideoColor(cJSON *tmp, int index, int is_save)
{
    ...
    if (is_change == 1) {
        ...
        if (pColor->day_night_color == 0) //彩色
            image.irCutMode = 1;
        else if (pColor->day_night_color == 2) //黑白
            image.irCutMode = 2;
        else
            image.irCutMode = 0;
		...
		tmp_image_cfg.sharpness = image.sharpness;
...
static void Dahua_Cfg_MotionDetectResume(void)
{
    Dahua_Cfg_MotionDetect_Handle(0);
    motionTimer = 0;
}
...
static int Dahua_Cfg_MotionDetect_Handle(char type)
{
    //32字节头，4字节数据, 数据位第一位为1表示有运动，为0表示没有运动
    //有运动时需要发一个1的数据，连续运动不继续发，运动停止时发0
    unsigned char ack[] =
        "\x69\x00\x00\x68\x04\x00\x00\x00\x02\x00\x00\x00\x02\x00\x00\x00" \
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" \
        "\x01\x00\x00\x00";

    ack[32] = type;
    PRINT_INFO("motion detect:%d, gsock:%x\n", ack[32], gSock);
    if (gSock != NULL)
    {
        if (motionLast != type)
        {
            ZhiNuo_SockSend(gSock, ack, sizeof(ack) - 1, 100);
        }
        ...
        if (type == 1)
        {
            if (motionTimer != 0)
            {
                netcam_timer_del_task(Dahua_Cfg_MotionDetectResume);
            }
            motionTimer = netcam_timer_add_task(Dahua_Cfg_MotionDetectResume, NETCAM_TIMER_ONE_SEC * 10, SDK_FALSE, SDK_TRUE);
        }
    }
    return 0;
}

static int Dahua_alarm_cb_func(int nChannel, int nAlarmType, int nAction, void* pParam)
{
	...
	switch(nAlarmType)
    {
        ...
        case GK_ALARM_TYPE_VMOTION:          //3:移动侦测
            Dahua_Cfg_MotionDetect_Handle(1);
            break;
...
static int Dahua_Cfg_SetVideoInOptions(cJSON *params, int is_save)
{
    ...
    if (day_night_color[profile_index] == 0) {
        image.irCutMode = 1; //彩色 白天
    } else if (day_night_color[profile_index] == 2) {
        image.irCutMode = 2; //黑白 晚上
    } else {
        image.irCutMode = 0; //自动
    }
...
int ZhiNuo_InfoNew_Cmd(char* bufPost, DAHUA_SESSION_CTRL *pCtrl)
{
    ...
    if (t_msg_head->dvrip.dvrip_extlen > 0) {
        ...
        if (!strcmp(method, "alarm.listMethod")) {
            ...
            ret = Dahua_Encode_GetConfigCaps(pCtrl, id, sid, result, json);
        } else {
...
static int DahuaRecvMsg(int sock, char *szbuf, int len, int timeout)
{
    ...
    do
    {
        ...
        if(FD_ISSET(sock, &read_set))
        {
            ...
            n_timeout_num = 0;
            curLen = recv(sock, (char*)(szbuf + recvLen), len, 0);
            recvLen += curLen;
            len -= recvLen;
            if (curLen <= 0)
            {
...
static int DahuaHandleMsg(DAHUA_SESSION_CTRL *pCtrl, char* bufPost, int datalen)
{
    ...
    //有可能nvr几条命令连着一起发过来，所以要拆分一个命令一个命令处理
    while(1)
    {
        ...
        PRINT_INFO("sock:%d, sid:%d, ip:%s, t_msg_head->dvrip.cmd @@@>>>>>>>:%x, len:%d\n", pCtrl->fd, pCtrl->fSessionInt, pCtrl->c_ip, t_msg_head->dvrip.cmd, datalen);
        switch(t_msg_head->dvrip.cmd)
        {
            case CMD_DAHUA_LOG_ON:
                PRINT_INFO("dahua logon, sock:%d\n", pCtrl->fd);
                ret = Dahua_LogOn(bufPost + n_offset, pCtrl); //登录
                break;
            ...
            case CMD_STATUS_SEARCH:
                ...
                ret = ZhiNuo_Search_Status(bufPost + n_offset, pCtrl); //查询工作及报警状态 1
                break;
            case CMD_DEV_EVENT_SEARCH:
                PRINT_INFO("EVENT_SEARCH \n");
                ret = ZhiNuo_Dev_Event(bufPost + n_offset, pCtrl); //查询设备事件 1
                break;
            case CMD_MEDIA_REQUEST:
                PRINT_INFO("MEDIA_REQUEST \n");
                ret = ZhiNuo_Media_Request(bufPost + n_offset, pCtrl); //媒体数据请求
                break;
            case CMD_MEDIA_CAPACITY_SEARCH:
                PRINT_INFO("MEDIA_CAPACITY_SEARCH \n");
                ret = ZhiNuo_Search_Media_Capacity(bufPost + n_offset, pCtrl); //查询设备媒体能力信息 ok
                break;
            case CMD_CONFIG_SEARCH:
                PRINT_INFO("CONFIG_SEARCH \n");
                ret = ZhiNuo_Search_Config(bufPost + n_offset, pCtrl); //查询配置参数
                break;
            case CMD_CHANNEL_NAME_SEARCH:
                PRINT_INFO("CHANNEL_NAME_SEARCH \n");
                ret = ZhiNuo_Search_Channel_Name(bufPost + n_offset, pCtrl); //查询通道名称
                break;
            case CMD_SYSTEM_INFO_SEARCH:
                PRINT_INFO("SYSTEM_INFO_SEARCH \n");
                ret = ZhiNuo_Search_System_Info(bufPost + n_offset, pCtrl); //查询系统信息
                break;
            case CMD_CONNECT_REQUEST:
                PRINT_INFO("CONNECT_REQUEST \n");
                ret = Dahua_ConnectRequest(bufPost + n_offset, pCtrl); //请求建立连接关系
                //if (pCtrl->fSessionType == CONNECT_SESSION)
                //    return 1; //不关闭sock并关闭这个回话线程
                break;
            case CMD_I_FRAME_REQUEST:
                PRINT_INFO("I_FRAME_REQUEST \n");
                ret = ZhiNuo_I_Frame_Request(bufPost + n_offset, pCtrl); //强制I帧
                break;
            case CMD_DEV_CONTROL:
                PRINT_INFO("DEV_CONTROL\n");
                ret = ZhiNuo_DEV_Control(bufPost + n_offset, pCtrl); //设备控制
                break;
            case CMD_SET_CONFIG:
                PRINT_INFO("SET_CONFIG datalen:%d\n", datalen);
                ret = ZhiNuo_Set_Config(bufPost + n_offset, pCtrl); //修改配置参数
                break;
            case CMD_TIME_MANAGE:
                PRINT_INFO("TIME_MANAGE\n");
                ret = ZhiNuo_Time_Manage(bufPost + n_offset, pCtrl); //时间管理
                break;
            case CMD_SET_CHANNEL_NAME:
                PRINT_INFO("SET_CHANNEL_NAME\n");
                ret = ZhiNuo_Set_Channel_Name(bufPost + n_offset, pCtrl); //修改通道名称
                break;
            case CMD_PTZ_CONTROL:
                PRINT_INFO("PTZ_CONTROL\n");
                ret = ZhiNuo_PTZ_Control(bufPost + n_offset, pCtrl); //云台控制
                break;
            case CMD_TALK_REQUEST:
                PRINT_INFO("TALK_REQUEST \n");
                //ret = ZhiNuo_Talk_Request(bufPost + n_offset, pCtrl); //请求对讲
                break;
            case CMD_TALK:
                PRINT_INFO("TALK \n");
                //ret = ZhiNuo_Talk(bufPost + n_offset, pCtrl); //开始对讲
                break;
            case CMD_TALK_CONTROL:
                PRINT_INFO("TALK_CONTROL\n");
                //ret = ZhiNuo_Talk_Control(bufPost + n_offset, pCtrl); //对讲控制
                break;
            case CMD_RECORD_SEARCH:
                PRINT_INFO("RECORD_SEARCH\n");
                //ret = ZhiNuo_Record_Search(bufPost + n_offset, pCtrl); //录像搜索
                break;
            case CMD_RECORD_PLAYBACK:
                PRINT_INFO("RECORD_PLAYBACK\n");
                //ret = ZhiNuo_Record_Playback(bufPost + n_offset, pCtrl); //录像回放
                break;
            case CMD_RECORD_DOWNLOAD:
                PRINT_INFO("RECORD_DOWNLOAD\n");
                //ret = ZhiNuo_Record_Download(bufPost + n_offset, pCtrl); //录像下载
                break;
            case CMD_INFO_NEW:
                PRINT_INFO("CMD_INFO_NEW [0x%02x]\n", t_msg_head->dvrip.cmd);
                ret = ZhiNuo_InfoNew_Cmd(bufPost + n_offset, pCtrl); //不知名的命令
                break;
            case CMD_CONTROL_CONNECT:
                PRINT_INFO("CMD_CONTROL_CONNECT [0x%02x]\n", t_msg_head->dvrip.cmd);
                ret = Dahua_ControlConnect(bufPost + n_offset, pCtrl); //不知名的命令
                //if (pCtrl->fSessionType == CONNECT_SESSION)
                //    return 1; //不关闭sock并关闭这个回话线程
...
void *DahuaSessionThread(void *param)
{
    ...
    snprintf(thread_name, sizeof(thread_name)-1, "DahuaSession.%d", pClientInfo->sock);

    DahuaSetThreadName(thread_name);
...
void * DahuaMainThread(void *param)
{
    ...
    void *dahuaMdHandle = NULL;
    int n_len = sizeof(t_clnt_addr);
    /* 循环监听，等待nvr主动连接 */
    n_len = sizeof(t_clnt_addr);

    dahuaMdHandle = event_alarm_open(Dahua_alarm_cb_func);
    PRINT_INFO("open md alarm : 0x%x\n", dahuaMdHandle);
    while (1)
    {
        PRINT_INFO("begin to accept\n");
        sock_fd = accept(fListenfd, (struct sockaddr *)&t_clnt_addr, (socklen_t *)&n_len);/* 接收连接 */
        if(0 == g_dahua_thread_run)
        {
            break;
        }
    ...
    event_alarm_close(dahuaMdHandle);
...
static int DahuaCreateBroadcastSock2(int port)
{
    int socket_fd = -1;
	int s32Buflen = 8192;
    ...
    bzero(&addrto, sizeof(struct sockaddr_in));
    addrto.sin_family = AF_INET;
    addrto.sin_addr.s_addr = htons(INADDR_ANY);//inet_addr("239.255.255.251");
    addrto.sin_port = htons(port);
    if(bind(socket_fd,(struct sockaddr *)&(addrto), sizeof(struct sockaddr_in)) == -1)
    {
        PRINT_ERR("bind error...\n");
	...
	if (setsockopt(socket_fd, SOL_SOCKET, SO_SNDBUF, (char*)&s32Buflen, sizeof(int)))
	{
		PRINT_ERR("setsockopt so sendbuf failed!\n");
        return -1;
	}

	if (setsockopt(socket_fd, SOL_SOCKET, SO_RCVBUF, (char*)&s32Buflen, sizeof(int)))
	{
		PRINT_ERR("setsockopt so rcvbuf failed!\n");
        return -1;
	}
	...
	if(setsockopt(socket_fd, IPPROTO_IP, IP_MULTICAST_LOOP, (void *)&msgLoop, sizeof(msgLoop)) < 0)
	{
		PRINT_ERR("Fail to do IP_MULTICAST_LOOP");
		return -1;
	}
	if(setsockopt(socket_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&mreq, sizeof(mreq)) < 0)
	{
		PRINT_ERR("set IP_ADD_MEMBERSHIP error.\n");
		return -1;
	}
...
static int DahuaBoardCastSendTo2(int fSockSearchfd, char* fromIp, int port, const char* inData, const int inLength)
{
	int ret = 0;
	int sendlen = 0;
    struct sockaddr_in addr;
    ...
    memset(&addr, 0,sizeof(&addr));
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = inet_addr("239.255.255.251");
    addr.sin_port = htons(port);
	...
	PRINT_INFO("DahuaBoardCastSendTo2: %d\n", sendlen);
...
static int DahuaAckSearch(int sock_send)
{
    ...
    t_msg_head->c[2] = 17 + strlen(dev_mode);   //mac地址长度加上设备类型
    t_msg_head->c[3] = 1;
    t_msg_head->dvrip.dvrip_extlen = 0x58;  //为extra_data数组的长度  88 = 0x58
    t_msg_head->c[16] = 2; //固定为2
    t_msg_head->c[19] = 0; //返回码 0:正常 1:无权限 2:无对应配置提供
    t_msg_head->c[20] = 0xa6; //根据抓包报文里面的值做的填充
    //t_msg_head->c[20] = 0xb7; //根据抓包报文里面的值做的填充
    ...
    str_dev_search->Version[0] = 0X02;  //版本信息
    ...
    strcpy(str_dev_search->HostName, "IPC"); //主机名
...
//    PRINT_INFO("ip:%s\n", (char *)inet_ntoa(net_addr));
    ...
    ////////////////////
    strcpy(send_msg + n_offset, (char*)st_ipc_net_info.mac);//xx:xx:xx:xx:xx:xx格式
    ...
    //n_extra_len = strlen(a_extra_sting);
    n_extra_len = strlen(a_extra_sting) + 1; //多个 00
    strncpy(send_msg + n_offset, a_extra_sting, n_extra_len);
    n_offset += n_extra_len;

    //从广播地址发送消息
    usleep(300000); //睡眠500毫秒，以免nvr搜索数据太多，溢出缓冲区，保证nvr每次都能搜到设备
    //sleep(2);
...
static int DahuaAckSearch2(int sock_send, char* fromIp, char* bufPost)
{
    int dataLen = 0;
    cJSON *json, *pj;
    char *out = NULL;
    ...
    if(pj)
    {
        DVRIP_HEAD_T *t_msg_head = (DVRIP_HEAD_T *)send_msg;
        memset(send_msg, 0, sizeof(send_msg));
        t_msg_head->dvrip.cmd = CMD_CONTROL_SEARCH;
        strcpy(&(t_msg_head->c[4]), "DHIP");
        if (!strcmp(pj->valuestring, "DHDiscover.search"))
        {
            //获取本机网络参数，
            //////////////////////DVRIP_HEAD_T_SIZE
            ...
            /* 添加消息内容 */
            cJSON_AddStringToObject(root, "mac", st_ipc_net_info.mac);
            cJSON_AddStringToObject(root, "method", "client.notifyDevInfo");

            cJSON *params;
            cJSON_AddItemToObject(root, "params", params = cJSON_CreateObject());
            cJSON *devInfo;
            cJSON_AddItemToObject(params, "deviceInfo", devInfo = cJSON_CreateObject());
            cJSON_AddNumberToObject(devInfo, "AlarmInputChannels", 0);
            cJSON_AddNumberToObject(devInfo, "AlarmOutputChannels", 0);
            cJSON_AddStringToObject(devInfo, "DeviceClass", "IPC");
            cJSON_AddStringToObject(devInfo, "DeviceType", "IPC");
            cJSON_AddStringToObject(devInfo, "Find", "A");
            cJSON_AddNumberToObject(devInfo, "HttpPort", 80);
            cJSON *ipv4;
            cJSON_AddItemToObject(devInfo, "IPv4Address", ipv4 = cJSON_CreateObject());
            cJSON_AddStringToObject(ipv4, "DefaultGateway", st_ipc_net_info.gateway);
            if (st_ipc_net_info.dhcp == 1)
            {
                cJSON_AddTrueToObject(ipv4, "DhcpEnable");
            }
            else
            {
                cJSON_AddFalseToObject(ipv4, "DhcpEnable");
            }
            cJSON_AddStringToObject(ipv4, "IPAddress", st_ipc_net_info.ip);
            cJSON_AddStringToObject(ipv4, "SubnetMask", st_ipc_net_info.mask);

            cJSON_AddNumberToObject(devInfo, "Init", 2714);
            cJSON_AddStringToObject(devInfo, "MachineName", "IPC");
            cJSON_AddStringToObject(devInfo, "Manufacturer", "Private");
            cJSON_AddNumberToObject(devInfo, "Port", DAHUA_TCP_LISTEN_PORT);
            cJSON_AddNumberToObject(devInfo, "RemoteVideoInputChannels", 0);
            cJSON_AddStringToObject(devInfo, "SerialNo", st_ipc_net_info.mac);
            cJSON_AddStringToObject(devInfo, "Vendor", "Private");
            cJSON_AddStringToObject(devInfo, "Version", "2.622.0000000.39.R");
            cJSON_AddNumberToObject(devInfo, "VideoInputChannels", 1);
            cJSON_AddNumberToObject(devInfo, "VideoInputChannels", 0);
            ...
            // 把 cjson 信息加入发送缓冲区
            sprintf(send_msg + sizeof(DVRIP_HEAD_T), "%s", out);
            dataLen = strlen(out);
            free(out);
            cJSON_Delete(root);

            //sleep(2);
            t_msg_head->c[16] = dataLen & 0xFF; //长度
            t_msg_head->c[17] = dataLen >> 8;
            t_msg_head->c[24] = dataLen & 0xFF; //长度
            t_msg_head->c[25] = dataLen >> 8;

            #if 1
            DahuaBoardCastSendTo2(sock_send, fromIp, DAHUA_SEARCH_SEND_PORT, send_msg, dataLen + sizeof(DVRIP_HEAD_T));
            #else

            int sendLen;
        	struct sockaddr_in addr;
        	addr.sin_family = AF_INET;
        	addr.sin_addr.s_addr = inet_addr(fromIp);
        	addr.sin_port = htons(DAHUA_SEARCH_SEND_PORT);

            sendLen = sendto(sock_send, send_msg, dataLen + sizeof(DVRIP_HEAD_T), 0, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));
            PRINT_INFO("-->sendLen:%d\n", sendLen);
            #endif
        }
        else if (!strcmp(pj->valuestring, "DHDiscover.setConfig"))
        {
            pj = cJSON_GetObjectItem(json, "mac");
            if (strcmp(pj->valuestring, st_ipc_net_info.mac) != 0)
            {
                cJSON_Delete(json);
                return 0;
            }

            cJSON *parmas = cJSON_GetObjectItem(json, "params");
            cJSON *devInfo = cJSON_GetObjectItem(parmas, "deviceConfig");
            cJSON *ipInfo = cJSON_GetObjectItem(devInfo, "IPv4Address");
            cJSON *ipv4 = cJSON_GetObjectItem(ipInfo, "IPAddress");
            strcpy(st_ipc_net_info.ip, ipv4->valuestring);
            cJSON *gate = cJSON_GetObjectItem(ipInfo, "DefaultGateway");
            strcpy(st_ipc_net_info.gateway, gate->valuestring);
            cJSON *dhcp = cJSON_GetObjectItem(ipInfo, "DhcpEnable");
            st_ipc_net_info.dhcp = dhcp->valueint;
            //设置本机网络参数，
            if(0 != goke_api_set_ioctrl(SET_NETWORK_CFG,&st_ipc_net_info))
            {
                PRINT_ERR("DMS_NET_GET_PLATFORMCFG fail\n");
            ...
            /* 添加消息内容 */
            cJSON_AddStringToObject(root, "mac", st_ipc_net_info.mac);
            cJSON_AddStringToObject(root, "method", "client.notifyCfgChange");

            cJSON *params;
            cJSON_AddItemToObject(root, "params", params = cJSON_CreateObject());
            cJSON_AddStringToObject(params, "encryption", "Default");
            cJSON_AddStringToObject(params, "realm", "Login to ");
            cJSON_AddTrueToObject(params, "result");
...
static void DahuaSearchHandleLoop2(int g_dahua_sock)
{
    // 广播地址
    struct sockaddr_in from;
    bzero(&from, sizeof(struct sockaddr_in));
    ...
    while(1)
    {
        ...
        if (FD_ISSET(g_dahua_sock, &readfd))
        {
            ...
            else
            {
                ...
                switch(t_msg_head->dvrip.cmd)
                {
                    case CMD_CONTROL_SEARCH:
                        PRINT_INFO("get search from %s\n", (char *)inet_ntoa(from.sin_addr));
                        DahuaAckSearch2(g_dahua_sock, (char *)inet_ntoa(from.sin_addr), smsg);
                        break;
                    default:
                        PRINT_ERR("Dahua_Broadcast unknown cmd[%x] not support\n", t_msg_head->dvrip.cmd);
                        break;
...
void * DahuaBroadcastThreadSearch(void *param)
{
    DahuaSetThreadName("DahuaBroadcastThreadSearch");

    int g_dahua_sock = DahuaCreateBroadcastSock2(DAHUA_SEARCH_RCV_PORT);
    if( g_dahua_sock < 0)
    {
        PRINT_ERR("create dahua broadcast fail\n");
        return NULL;
    ...
    DahuaSearchHandleLoop2(g_dahua_sock);
	...
	if(g_dahua_sock > 0)
	{
		...
		char send_msg[] = "shutdown";
    	int send_len = strlen(send_msg);
		DahuaBoardCastSendTo(g_dahua_sock, DAHUA_SEARCH_SEND_PORT, send_msg, send_len);
...
int dahua_protocol_start()
{
    ...
    DahuaCreateThread(DahuaBroadcastThreadSearch, NULL, NULL);
