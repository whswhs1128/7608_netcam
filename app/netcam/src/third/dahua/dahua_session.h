...
#define CMD_CONTROL_SEARCH 0x20    //搜索配网
...
#define DAHUA_SEARCH_RCV_PORT 37810
#define DAHUA_SEARCH_SEND_PORT 37810
...
#define EXPAND_PICTURE_SIZE2 0x82 //分辨率大于 0xff*8,该字段8个字节，最后4个字节，长宽各占2个字节，赋值完整数值
...
typedef enum capture_size_t {
    CAPTURE_SIZE_D1,        ///00< 704*576(PAL)   704*480(NTSC)
    CAPTURE_SIZE_HD1,       ///01< 352*576(PAL)   352*480(NTSC)
    CAPTURE_SIZE_BCIF,      ///02< 704*288(PAL)   704*240(NTSC)
    CAPTURE_SIZE_CIF,       ///03< 352*288(PAL)   352*240(NTSC)
    CAPTURE_SIZE_QCIF,      ///04< 176*144(PAL)   176*120(NTSC)
    CAPTURE_SIZE_VGA,       ///05< 640*480(PAL)   640*480(NTSC)
    CAPTURE_SIZE_QVGA,      ///06< 320*240(PAL)   320*240(NTSC)
    CAPTURE_SIZE_SVCD,      ///07< 480*480(PAL)   480*480(NTSC)
    CAPTURE_SIZE_QQVGA,     ///08< 160*128(PAL)   160*128(NTSC)
    CAPTURE_SIZE_SVGA,      ///09< 800*592
    ...
    CAPTURE_SIZE_LTF,       ///16< 240*192(PAL);浪涛平台使用
    CAPTURE_SIZE_720p ,     ///17< 1280*720
    CAPTURE_SIZE_1080p,     ///18< 1920*1080
    CAPTURE_SIZE_1_3M,      ///19< 1280*960
    CAPTURE_SIZE_2_5M,      ///20< 1872*1408
    CAPTURE_SIZE_5M,        ///21< 3744*1408
    CAPTURE_SIZE_3M,        ///22< 2048*1536
    CAPTURE_SIZE_5_0M,      ///23< 2432*2048
    CAPTURE_SIZE_1_2M,      ///24< 1216*1024
    CAPTURE_SIZE_1_5M,      ///25< 1408*1024
    CAPTURE_SIZE_8M,        ///26< 3296*2472
    CAPTURE_SIZE_5M_2,      ///27< 2560*1920
    CAPTURE_SIZE_960H,      ///28< 960*576
    CAPTURE_SIZE_QNATIVE,   ///29< 320*256
    CAPTURE_SIZE_NATIVE,    ///30< 640*512
    CAPTURE_SIZE_HVGAW,     ///31< 640*360
...
//扩展帧头图像尺寸
typedef struct tagZhiNuo_Expand_Picture_Size2
{
    unsigned char ch_cmd;
    unsigned char ch_coding_type; //ZhiNuo_coding_type_e
    unsigned char reserve[2];
    unsigned short ch_width;
    unsigned short ch_high;
}ZhiNuo_Expand_Picture_Size2;
