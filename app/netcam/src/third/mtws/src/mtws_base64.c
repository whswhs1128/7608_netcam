...
static const char *g_sysBase64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

static void mtws_base64_encode_triple(unsigned char triple[3], char result[4])
{
    int tripleValue, i;
...
int mtws_base64_encode(unsigned char *source, unsigned int sourcelen, char *target,unsigned int targetlen)
{
    ...
    while (sourcelen >= 3)
    {
		mtws_base64_encode_triple(source, target);
		sourcelen -= 3;
		source += 3;
		target += 4;
    }
    ...
    if (sourcelen > 0)
    {
		...
		mtws_base64_encode_triple(temp, target);
		target[3] = '=';
		if (sourcelen == 1)
		    target[2] = '=';
