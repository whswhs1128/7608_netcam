...
extern char g_call_id_str[2][100];
...
int  gk_gb28181_playback_get_record_time(char *period_start, char *period_end, TIME_RECORD_T *timeInfo)
{
	...
	printf("period_start:%llu,period_end:%llu\n",gk_gb28181_time_to_u64t(period_start),gk_gb28181_time_to_u64t(period_end));

	u64t start;
	u64t stop;
	int i=0,sched_file_cnt=0,lst_len=0;
    FILE_NODE node;
	FILE_LIST *list = NULL;
	...
	for(i=0; i<lst_len; i++)
	{
		...
		timeInfo[sched_file_cnt].start_time  = node.start;
		timeInfo[sched_file_cnt].end_time 	 = node.stop;
		sched_file_cnt++;
	}
...
static void *gb28181_playback_thread(void *arg)
{
	...
	{	
		...
		while(netcam_is_prepare_update() || (netcam_get_update_status() != 0))
		{
		...
		if(pbCtrlInfo->stopCmd == 1)
		{
			pthread_mutex_lock(&pbCtrlInfo->playbackLock);
			pbCtrlInfo->stopCmd = 0;
		...
		if(ret < 0)
		{
            ...
            if(pbCtrlInfo->sessionID == 0)
                rtp_stream_status_send(1, pbCtrlInfo->remoteip, pbCtrlInfo->port, pbCtrlInfo->ssrc, 0, ret,0,0,0,video_frame_no,&playback_speed, pbCtrlInfo->callidStr);
            else
                rtp_stream_status_send(2, pbCtrlInfo->remoteip, pbCtrlInfo->port, pbCtrlInfo->ssrc, 0, ret,0,0,0,video_frame_no,&playback_speed, pbCtrlInfo->callidStr);
		...
		else
		{
			...
			if (pbCtrlInfo->FileHandle->node.frame_type == 0x11)
			{
                ...
                send_data = pbCtrlInfo->FileHandle->node.one_frame_buf;
			...
			else
			{
                ...
                //continue;
				frame_size = audio_alaw_encode((unsigned char*)G711AData, (short*)pbCtrlInfo->FileHandle->node.one_frame_buf, header.frame_size);
                send_data = G711AData;
			}
			...
			if((isSendIframe && header.frame_type == GK_NET_FRAME_TYPE_I) || !isSendIframe)
			{
            	...
            	ret = gk_rtp_playback_send(send_data, frame_size, &header,pbCtrlInfo->sessionID,&u16CSeq);
			}
            #if 0
            if (sendCnt % 80 == 0)
            {
                    ...
                    char *media_server_port, unsigned int enOverTcp, int ssrc, int eventId)
{
	...
	for(i=0; i<MAX_SESSION_CNT; i++)
	{
		...
		if(playbackCtrlInfo[i].sessionID == session_id)
		{
            ...
            strcpy(playbackCtrlInfo[i].callidStr, g_call_id_str[i]);
