...
unsigned char h2641440p[] = 
{
    #include "iframe/1440p.h264"
};

unsigned char h2651440p[] = 
{
    #include "iframe/1440p.h265"
};

unsigned char h2641296p[] = 
{
    #include "iframe/1296p.h264"
};

unsigned char h2651296p[] = 
{
    #include "iframe/1296p.h265"
};
    
unsigned char h264720p[] = 
{
    #include "iframe/720p.h264"
};

unsigned char h265720p[] = 
{
    #include "iframe/720p.h265"
};

unsigned char h264360p[] = 
{
    #include "iframe/360p.h264"
};

unsigned char h265360p[] = 
{
    #include "iframe/360p.h265"
};

static int h2641440pSize = sizeof(h2641440p);
static int h2651440pSize = sizeof(h2651440p);
static int h2641296pSize = sizeof(h2641296p);
static int h2651296pSize = sizeof(h2651296p);
static int h264720pSize = sizeof(h264720p);
static int h265720pSize = sizeof(h265720p);
static int h264360pSize = sizeof(h264360p);
static int h265360pSize = sizeof(h265360p);

static int privacyCloudType = -1; //-1:normal  1:privacy start 0:stop
#endif
...
void rtp_stream_status_send(int rtptype, char *ip, int port, int ssrc, int channel,int result, char is_connect, int begin, int end, int total, float *speed, char *callidStr);
unsigned int MjEvtcloudSendCnt = 0;
unsigned int MjEvtcloudSuccessCnt = 0;
#ifdef MODULE_SUPPORT_MOJING_LOG_AES
udp_log_info_s udpLogCnts;
#endif
...
int ite_make_rtp_header(char *pData, int marker_flag,  unsigned int curpts, unsigned int ssrc, int rtpSeq)
{
    bits_buffer_s   bitsBuffer;
    ...
    bitsBuffer.p_data[2] = (rtpSeq & 0xff00) >> 8;
...
int ite_RtpSendTcp_PS_H264frame(ite_rtp_Info *pSockInfo, char *pData, int nFrameLen, frameData_Info_s* pPacker, int stream_id, int rtpsockfd, struct sockaddr *to)
{
    ...
    */
    ...
    while(nFrameLen > 0)
    {
        ...
        {
            //saveps(frameBuffer, nSize + PES_HDR_LEN);
            if( rtp_sendtcp_h264ps(rtpsockfd, to, pSockInfo->frameBuffer, nSize + PES_HDR_LEN, endpack, pPacker, pSockInfo->rtpType) != 0 )
            {
                printf("rtp_sendtcp_h264ps  failed 2!\n");
                return -1;
            }
        }
        //分片后每次发送的数据移动指针操作
...
#if defined(MODULE_SUPPORT_MOJING_V5) && defined(MODULE_SUPPORT_MOJING_V5_PRIVACY)
int rtp_get_privacy_data(void **data, int *size, int encType, int channel)
{
    if (size == NULL || data == NULL)
    {
        printf("--->rtp_get_privacy_data is null\n");
        return 0;
    }
    if (encType == 1)
    {
        if (runVideoCfg.vencStream[channel].h264Conf.height == 1440)
        {
            *data = h2641440p;
            *size = h2641440pSize;
        }
        else if (runVideoCfg.vencStream[channel].h264Conf.height == 1296)
        {
            *data = h2641296p;
            *size = h2641296pSize;
        }
        else if (runVideoCfg.vencStream[channel].h264Conf.height == 720)
        {
            *data = h264720p;
            *size = h264720pSize;
        }
        else if (runVideoCfg.vencStream[channel].h264Conf.height == 360)
        {
            *data = h264360p;
            *size = h264360pSize;
        }
...
static int SendVideoOverUdp(ite_rtp_Info *pSockInfo)
{
    ...
    netcam_video_force_i_frame(pSockInfo->mediaChannel);
...
static int SendVideoOverTcp(ite_rtp_Info *pSockInfo)
{
        ...
        */
        ...
        {  
            ...
            if (mojing_qiuji_is_privacy())
            {
                //if (frameCnt % 5 != 0)
                    //continue;
                //if (frameType == M_FRAME_TYPE_P)
                    //continue;
                //printf("set privacy data 1.\n");   
                frameType = M_FRAME_TYPE_I;
                rtp_get_privacy_data((void **)&pH264frame, &veFrameLen, stH264Config.enctype, pSockInfo->mediaChannel);
            }
            
            //printf("lastPrivacy:%d, cur:%d.\n", lastPrivacy, mojing_qiuji_is_privacy());    
            if (lastPrivacy == 1 && mojing_qiuji_is_privacy() != lastPrivacy)
            {
        ...
        if (veFrameLen > 0)
        {
			...
			else if (frameType == M_FRAME_TYPE_AUDIO)
			{
                ...
                frameInfo_rtp.IFrame = 0;
				alaw_len = audio_alaw_encode((unsigned char*)alaw_data, (short*)pH264frame, veFrameLen);
				//printf("-->send audio len:%d,%d pts:%d\n", veFrameLen, alaw_len, frameInfo_rtp.u32CurPts);
                #ifndef MODULE_SUPPORT_MOJING
                //常规8k 采样率pcma计算pts方法
                audiopts +=  (alaw_len * 45) >> 2;
                frameInfo_rtp.u32RtpPts = audiopts;
                frameInfo_rtp.u32CurPts = frameInfo_rtp.u32RtpPts;
                #endif
	...
	rtp_stream_status_send(pSockInfo->rtpType, pSockInfo->remoteip, pSockInfo->videoPort, pSockInfo->ssrc, pSockInfo->mediaChannel, retVal,0,0,0,total_frame,NULL,pSockInfo->callidStr);
...
void rtp_stream_status_send(int rtptype, char *ip, int port, int ssrc, int channel,int result, char is_connect, int begin, int end, int total, float *speed, char *callidStr)
{
	...
	{
        #ifdef MODULE_SUPPORT_MOJING_LOG_AES
        udpLogCnts.realPlayCnt++;
        #endif
		if(is_connect)
		{
            if (result == 0)
            {
                sprintf(resultInfo, "%s", "succ");
            }
            else
            {
                #ifdef MODULE_SUPPORT_MOJING_LOG_AES
                sprintf(resultInfo, "%s,errno=%d", "fail", errno);
                #else
                sprintf(resultInfo, "%s", "fail");
                #endif
            }
			snprintf(msgStr, sizeof(msgStr),"connect(ip=%s,port=%d,ssrc=%d,stream_type=%s) %s",\
			ip,port,ssrc,channel==0?"Main":"Sub",resultInfo);
		}
		else
		{
			...
			else
			{
				...
				snprintf(msgStr, sizeof(msgStr),"sendto(ip=%s,port=%d,ssrc=%d,stream_type=%s) fail,send frames=%d,errno=%d,callId=%s,failureTime=%s",
                        ip,port,ssrc,channel==0?"Main":"Sub",total, errno, callidStr, localTime);
                #else
	...
	else if(rtptype == 1 || rtptype == 2)
	{
		...
		else
		{
			...
			else
			{
                ...
                snprintf(msgStr, sizeof(msgStr),"sendto(ip=%s,port=%d,ssrc=%d) fail,send frames=%d,errno=%d,callId=%s,failureTime=%s",
                        ip,port,ssrc,total, errno, callidStr, localTime);
#else
	...
	{
        //audio support from aes version
    #ifdef MODULE_SUPPORT_MOJING_LOG_AES
		if(is_connect)
		{
            if (result == 0)
		...
		else
		{
			...
			{                
                ...
                snprintf(msgStr, sizeof(msgStr),"sendto(ip=%s,port=%d,ssrc=%d) fail,errno=%d,callId=%s,failureTime=%s",
                        ip,port,ssrc, errno, callidStr, localTime);
			}
		}

		#ifdef MODULE_SUPPORT_MOJING
        mojing_log_send(NULL, result, "gb28181_broadcast", msgStr);
		#endif
    #endif
	}
	else if(rtptype == 4)
	{
		...
		{
            #ifdef MODULE_SUPPORT_MOJING_LOG_AES
            udpLogCnts.cloudAll10++;
            udpLogCnts.cloudAllDay++;
            #endif
            if (result == 0)
            {
                sprintf(resultInfo, "%s", "succ");
            ...
            else
            {
                #ifdef MODULE_SUPPORT_MOJING_LOG_AES
                sprintf(resultInfo, "%s,errno=%d", "fail", errno);
                udpLogCnts.couldAbortError++;
                udpLogCnts.couldAbortError10++;
                #else
                sprintf(resultInfo, "%s", "fail");
                #endif
            }
		...
		{
            #ifdef MODULE_SUPPORT_MOJING_LOG_AES
            char localtimeBegin[64] = {0};
            char localtimeEnd[64] = {0};
            netcam_sys_get_local_time_string_bytime(begin, localtimeBegin);
            netcam_sys_get_local_time_string_bytime(end, localtimeEnd);
            #endif
			if(result == 0)
			{
                #ifdef MODULE_SUPPORT_MOJING_LOG_AES
                udpLogCnts.cloudSuccess10++;
                udpLogCnts.cloudSuccessDay++;
				snprintf(msgStr, sizeof(msgStr),"sendto(ip=%s,port=%d,ssrc=%d)over,beg_time=%s,end_time=%s,total=%d",
				    ip,port,ssrc,localtimeBegin,localtimeEnd,total);
                #else
			...
			{
                #ifdef MODULE_SUPPORT_MOJING_LOG_AES
                udpLogCnts.couldAbortExcept10++;
                udpLogCnts.couldAbortExcept++;
				snprintf(msgStr, sizeof(msgStr),"sendto(ip=%s,port=%d,ssrc=%d)fail,beg_time=%s,end_time=%s,total=%d,errno=%d",
				    ip,port,ssrc,localtimeBegin,localtimeEnd,total,errno);
                #else
...
void *SendVideoRtpTask(void *pData)
{
    ...
    if (retVal != 0)
    {
		...
		rtp_stream_status_send(pSockInfo->rtpType, pSockInfo->remoteip, pSockInfo->videoPort, pSockInfo->ssrc, pSockInfo->mediaChannel, -1,1,0,0,0,NULL, pSockInfo->callidStr);
	...
	rtp_stream_status_send(pSockInfo->rtpType, pSockInfo->remoteip, pSockInfo->videoPort, pSockInfo->ssrc, pSockInfo->mediaChannel, 0,1,0,0,0,NULL,pSockInfo->callidStr);
...
#ifdef MODULE_SUPPORT_AO_AUTO_CLOSE
    netcam_audio_open_ao();
#endif
		...
		if(len > 12)
		{
			...
			else
			{
				if(payloadType == 8 && payloadOffset < len)
            		netcam_audio_output((char *)&buf[payloadOffset],len-payloadOffset, NETCAM_AUDIO_ENC_A_LAW, SDK_TRUE);
				else
...
void *RecvAudioRtpTask(void *pData)
{
    ...
    if (pSockInfo->rtpType == 3)
    {
		...
		if(retVal < 0)
		{
			rtp_stream_status_send(pSockInfo->rtpType, pSockInfo->remoteip, pSockInfo->videoPort, pSockInfo->ssrc, pSockInfo->mediaChannel, -1,0,0,0,0,NULL,pSockInfo->callidStr);
		}
        else
        {
			rtp_stream_status_send(pSockInfo->rtpType, pSockInfo->remoteip, pSockInfo->videoPort, pSockInfo->ssrc, pSockInfo->mediaChannel, 0,0,0,0,0,NULL,pSockInfo->callidStr);
        }
    }
    return (void *)NULL;
}
                    ...
                    char *media_server_port, unsigned int enOverTcp, int channleIndex, int ssrc, int eventId, char *callidStr)
{
    ...
    strcpy(rtpHandle->callidStr, callidStr);
...
static int mojing_cloud_storage_send_over_tcp(ite_rtp_Info *pSockInfo)
{
    ...
    while (pSockInfo->isRun)
    {
        ...
        if (header.frame_type == M_FRAME_TYPE_I || header.frame_type == M_FRAME_TYPE_P)
        {
            ...
            //隐私模式开始后的数据需要替换
            isSetPrivacy = 0;
            //printf("privacyCloudType:%d, time:%d\n", privacyCloudType, pSockInfo->cloudCtrl.motionStartTime - header.sec);
            if (mojing_qiuji_is_privacy())
            {
                if (privacyCloudType && ((int)(pSockInfo->cloudCtrl.motionStartTime - header.sec) >= 0))
                {
                    //隐私模式主动触发的云存，开始时间前的数据不能替换
                }
                else
                {
                    //printf("set privacy data1...\n");
                    isSetPrivacy = 1;
                    frameType = M_FRAME_TYPE_I;
                    rtp_get_privacy_data((void **)&pH264frame, &frameLen, stH264Config.enctype, pSockInfo->mediaChannel);
                }
            }

            //隐私模式结束时主动触发云存，前面5s需要传黑屏数据
            if (privacyCloudType == 0 && ((int)(pSockInfo->cloudCtrl.motionStartTime - header.sec) >= 0))
            {
                //printf("set privacy data2...\n");
                isSetPrivacy = 1;
                frameType = M_FRAME_TYPE_I;
                rtp_get_privacy_data((void **)&pH264frame, &frameLen, stH264Config.enctype, pSockInfo->mediaChannel);
            }

            //printf("lastPrivacy:%d, cur:%d, %d\n", lastPrivacy, mojing_qiuji_is_privacy(), isSetPrivacy);
            //隐私模式结束触发的云存需要等到I帧才切换
            if (privacyCloudType == 0 && isSetPrivacy == 0 && lastPrivacy == 1)
            {
                //结束时等到下一个I帧才真正切换，避免切换花屏
                if (frameType == M_FRAME_TYPE_P)
                {
                    //printf("replace privcy continue;\n");
                    frameType = M_FRAME_TYPE_I;
                    rtp_get_privacy_data((void **)&pH264frame, &frameLen, stH264Config.enctype, pSockInfo->mediaChannel);
                }
                else
        ...
        {
            //音频rtp的pts不修改，与上次视频一致，pes pts修改为与视频一致,
            frameInfo_rtp.u32CurPts = curPts;
            frameInfo_rtp.u32RtpPts = curPts;
            #if defined(MODULE_SUPPORT_MOJING_V5) && defined(MODULE_SUPPORT_MOJING_V5_PRIVACY)
            if (mojing_qiuji_is_privacy())
            ...
            //隐私模式结束时主动触发云存，前面5s不传音频
            if (privacyCloudType == 0 && (pSockInfo->cloudCtrl.motionStartTime - header.sec >= 0))
            {
                continue;
            }
            #endif
...
SEND_END:
	...
	rtp_stream_status_send(pSockInfo->rtpType, pSockInfo->remoteip, pSockInfo->videoPort, pSockInfo->ssrc, pSockInfo->mediaChannel, retVal,0,first_timetick,cur_timetick,cur_timetick - first_timetick,NULL,pSockInfo->callidStr);
										...
										int ssrc, CloudStorageControlT *cloudInfo)
{
	...
	rtpHandle->isRun = 1;
	rtpHandle->eventId = 0;
    strcpy(rtpHandle->callidStr, cloudInfo->taskID);
...
void mojing_udp_log_reset(void)
{
    cloudSendCnt = 0;
    cloudSuccessCnt = 0;
    #ifdef MODULE_SUPPORT_MOJING_LOG_AES
    memset(&udpLogCnts, 0, sizeof(udpLogCnts));
    #endif
}

void mojing_udp_log_reset10(void)
{
    #ifdef MODULE_SUPPORT_MOJING_LOG_AES
    udpLogCnts.cloudAll10 = 0;
    udpLogCnts.cloudResponseError10 = 0;
    udpLogCnts.cloudResponseTimeout10 = 0;
    udpLogCnts.cloudSuccess10 = 0;
    udpLogCnts.couldAbortError10 = 0;
    udpLogCnts.couldAbortExcept10 = 0;
    #endif
}

#if defined(MODULE_SUPPORT_MOJING_V5) && defined(MODULE_SUPPORT_MOJING_V5_PRIVACY)
int mojing_cloud_storage_privacy(int isOn)
{
    mojing_get_cloud_storage_addr_request(CLOUD_STORAGE_DYNAMIC);
    privacyCloudType = isOn;
    return 0;
}
#endif
...
int ProcessMediaStop(int eventdid)
{
    int i;
    for (i = 0; i < MAX_CHANNEL_NUM; i++)
    {
        if (rtpInfo[i].isUsed && eventdid == rtpInfo[i].eventId && rtpInfo[i].rtpType != 4)
        {
            printf("ProcessMediaStop:eventdid = %d\n",eventdid);
            rtpInfo[i].isRun = 0;
        }
    }
    return 0;
...
{
	printf("ProcessVoiceStop:eventdid = %d\n",eventdid);
	
    int i;
    for (i = 0; i < MAX_CHANNEL_NUM; i++)
    {
        if (rtpInfo[i].isUsed && eventdid == rtpInfo[i].eventId && rtpInfo[i].rtpType == 3)
        {
            rtpInfo[i].isRun = 0;
        }
    }
