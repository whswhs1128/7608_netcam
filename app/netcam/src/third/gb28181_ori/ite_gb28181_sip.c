...
int g_VoiceFlowMode=1; // 0-UDP, 1-TCP 语音流模式
...
typedef struct _ite_logUploadParam_t{
    long expiredTime;   //日志订阅过期时间utc
    long startTime;     //utc时间
    long endTime;       //utc时间
    int  interval;      //日志订阅过期时间
    int  fileType;      //0:"Alarm", 1:"Error",2:"Operation",3:"General"
    char fileName[128]; //上传到ftp服务器的日志名
...
static char *glogContentEn[LOG_CONTENT_MAX_E] = //内容只能添加，不能修改,修改会导致解析出错
{
    "regFail",//0
    "pMdf",
    "vencFmt",
    ...
    for (;;)
    {
        ...
        if (EXOSIP_REGISTRATION_FAILURE == je->type)/*注册失败*/
        {
            ...
            else if ((NULL != je->response) && (302 == je->response->status_code))
            {
                ...
                {
                    printf("Date(%s):%s\n", sip_etag->hname,sip_etag->hvalue);
                    SetSystemTime(sip_etag->hvalue);/*将SIP服务器时间设置到系统中*/
                }
...
void ite_eXosip_sendUploadResult(struct eXosip_t *excontext, IPNC_SipIpcConfig * SipConfig, int result, ite_logUploadParam_t param)
{
    ...
    {
        eXosip_message_build_request(excontext,&rqt_msg, "NOTIFY", to, from, NULL);/*构建"NOTIFY"请求*/
    ...
    eXosip_lock (excontext);
    eXosip_message_send_request(excontext,rqt_msg);/*回复"NOTIFY"请求*/
    eXosip_unlock (excontext);
...
static int ite_eXosip_BroadcastInvite(struct eXosip_t *excontext,eXosip_event_t *p_event,IPNC_SipIpcConfig * SipConfig)
{
    ...
    else
    {
        ...
        if (1) //(strstr(sdp_buf, "active")) // SIP 服务器暂不支持配置
        {
...
int log_bplus_tree_del_loop(void)
{
    int i = 0;
    value_t info;
    int delNum = 1;//删除1条//LOG_BPT_DEL_INDEX_NUM;//删除固定数目
    memset(&info, 0, sizeof(info));
    if(g_stLogTree.pLogIndexTree != NULL)
    {
...
int log_bplus_tree_refresh(void)
{
    ...
    if((g_stLogTmpTree.pLogIndexTree != NULL) && (g_stLogTree.pLogIndexTree != NULL))
    {
        ...
        else if((times.tv_sec - last_timestamp) >= LOG_REFRESH_INTERVAL) //每时间间隔存一次
        {
            delNum = g_stLogTmpTree.u32LogTotalIndex;
            last_timestamp = times.tv_sec;
#if LOG_DEBUG
...
int ite_eXosip_log_upload(int logType, int starttime, int endtime, char *ftpurl, char *uploadfile)
{
    ...
    sprintf(LogPath, "%s/%s", LOG_PATH, LOG_INDEX);
    sprintf(LogTmpPath, "%s/%s", LOG_TMP_PATH, LOG_INDEX);
    if((access(LogPath, F_OK) != 0) && (access(LogTmpPath, F_OK) != 0))//本地日志不存在，则退出
    {
        printf("[%s][%d]%s and %s does not exist.\n", __func__, __LINE__, LogPath, LogTmpPath);
        return 0;
    }
    
    FILE *uploadfp = NULL;//待上传临时文件
...
static int ite_eXosip_ftp_upload(char *url, char *username, char *passdword, char *filename)
{
	...
	ret = FtpPut(filename,uploadname,FTPLIB_IMAGE,conn);//FTP 上传文件
	if(ret)
	{
...
int ite_eXosip_CreateDetachThread(ThreadEntryPtr entry, void *para, pthread_t *pid)
{
    ...
    if(pthread_create(&ThreadId, &attr, entry, param) == 0) { //创建线程
...
void ite_eXosip_ProcessMsgBody(struct eXosip_t *excontext,eXosip_event_t *p_event,IPNC_SipIpcConfig * SipConfig)
{
    ...
    if (0 == strcmp(xml_cmd_type, "DeviceControl"))/*设备控制*/
    {
        ...
        p_str_begin = strstr(p_xml_body, "<AlarmOut>");/*查找字符串"<AlarmOut>"*//*SVAC 解码配置(可选)*/
        if (NULL != p_str_begin)
        {
            int alarmOut = 0;
            int save = 0;
            ...
            memcpy(xml_command, p_str_begin + strlen("<AlarmOut>"), p_str_end - p_str_begin - strlen("<AlarmOut>")); /*保存<AlarmOut>到xml_config_body*/

            printf("AlarmOut:%s\n", xml_command);//声光报警开关1:打开；0:关闭
        ...
        p_str_begin = strstr(p_xml_body, "<FormatSDCard>");/*查找字符串"<FormatSDCard>"*//*SVAC 解码配置(可选)*/
        if (NULL != p_str_begin)
        {
            p_str_end = strstr(p_xml_body, "</FormatSDCard>");
            memcpy(xml_command, p_str_begin + strlen("<FormatSDCard>"), p_str_end - p_str_begin - strlen("<FormatSDCard>")); /*保存<FormatSDCard>到xml_config_body*/
        ...
        p_str_begin = strstr(p_xml_body, "<Mirror>");/*查找字符串"<Mirror>"*//*SVAC 解码配置(可选)*/
        if (NULL != p_str_begin)
        {
            GK_NET_IMAGE_CFG imagJsonAttr;
            unsigned char flipEnabled, mirrorEnabled, mirror=0;
            p_str_end = strstr(p_xml_body, "</Mirror>");
            memcpy(xml_command, p_str_begin + strlen("<Mirror>"), p_str_end - p_str_begin - strlen("<Mirror>")); /*保存<Mirror>到xml_config_body*/
            printf("Mirror:%s\n", xml_command);
        ...
        p_str_begin = strstr(p_xml_body, "<FileType>");/*查找字符串"<FileType>"*//*SVAC 解码配置(可选)*/
        if (NULL != p_str_begin)
        {
            p_str_end = strstr(p_xml_body, "</FileType>");
            memcpy(xml_command, p_str_begin + strlen("<FileType>"), p_str_end - p_str_begin - strlen("<FileType>")); /*保存<FileType>到xml_config_body*/
            printf("FileType:%s\n", xml_command);
            ...
            {
                opt = 1;                
                p_str_begin = strstr(p_xml_body, "<FilePath>");/*查找字符串"<FileType>"*//*SVAC 解码配置(可选)*/
                if (NULL != p_str_begin)
                {
                    p_str_end = strstr(p_xml_body, "</FilePath>");
                    memcpy(para, p_str_begin + strlen("<FilePath>"), p_str_end - p_str_begin - strlen("<FilePath>")); /*保存<FilePath>到xml_config_body*/
                    printf("FilePath:%s\n", para);
                }
            }
        ...
        p_str_begin = strstr(p_xml_body, "<Upload>");/*查找字符串"<Upload>"*/
        if (NULL != p_str_begin)
        {
...
DeviceControl_Next:
    ...
    else if (0 == strcmp(xml_cmd_type, "DeviceInfo"))/*网络设备信息查询：设备信息查询*/
    {
        ...
        snprintf(rsp_xml_body, 4096, "<?xml version=\"1.0\"  encoding=\"GB2312\" ?>\r\n"
                 ...
                 /*<element name= "Info" minOccurs= "0" maxOccurs="unbounded"> <!--  扩展信息，可多项 -->   
    ...
    else if (0 == strcmp(xml_cmd_type, "DeviceStatus"))/*网络设备信息查询：设备状态查询*/
    {
        ...
        p_str_begin = strstr(p_xml_body, "<Storage>");/*查找字符串"<Storage>"*/
        p_str_end   = strstr(p_xml_body, "</Storage>");
		if(p_str_begin && p_str_end)
		{
            char xml_storage[30];
	        memcpy(xml_storage, p_str_begin + strlen("<Storage>"), p_str_end - p_str_begin - strlen("<Storage>")); /*保存<Storage>到xml_file_path*/
	        printf("<Storage>:%s\n", xml_storage);
    ...
    else if (0 == strcmp(xml_cmd_type, "ConfigDownload"))/*配置参数下载及设备参数能力*/
    {
        ...
        if (NULL != strstr(xml_config_type, "AlarmReportSwitch"))/*报警开关配置查询*/
        {
            memset(rsp_item_body, 0, sizeof(rsp_item_body));
            memset(rsp_xml_body, 0, sizeof(rsp_xml_body));
    ...
    else if (0 == strcmp(xml_cmd_type, "DeviceConfig"))/* 设备配置*/
    {
        ...
        if (NULL != p_str_begin)
        {
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<LocalPort>"), p_str_end - p_str_begin - strlen("<LocalPort>"));/*本地端口*/
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<Domain>"), p_str_end - p_str_begin - strlen("<Domain>"));/*SIP域*/
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<SipIp>"), p_str_end - p_str_begin - strlen("<SipIp>"));/*SIP服务的IP*/
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<SipPort>"), p_str_end - p_str_begin - strlen("<SipPort>"));/*SIP服务的端口*/
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<SipId>"), p_str_end - p_str_begin - strlen("<SipId>"));/*SIP编码*/
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<VideoCompressionCodingProtocol>"), p_str_end - p_str_begin - strlen("<VideoCompressionCodingProtocol>"));/*SIP编码*/
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<VoiceFlowMode>"), p_str_end - p_str_begin - strlen("<VoiceFlowMode>"));/*SIP编码*/
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<RegisterExpiredInterval>"), p_str_end - p_str_begin - strlen("<RegisterExpiredInterval>"));/*SIP服务的端口*/
                if(SipConfig->regExpire_s != atoi(xml_param))
                {
                    snprintf(modify_content, sizeof(modify_content), \
            ...
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<KeepaliveInterval>");/*查找字符串"<HeartBeatInterval>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</KeepaliveInterval>");
                memcpy(xml_param, p_str_begin + strlen("<KeepaliveInterval>"), p_str_end - p_str_begin - strlen("<KeepaliveInterval>"));/*保存心跳间隔时间*/
                if(SipConfig->sipDevHBCycle_s != atoi(xml_param))
                {
                    snprintf(modify_content, sizeof(modify_content), \
            ...
            if (NULL != p_str_begin)
            {
                ...
                memcpy(xml_param, p_str_begin + strlen("<AdminPassword>"), p_str_end - p_str_begin - strlen("<AdminPassword>"));/*SIP编码*/
            ...
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_param, "<ChannelList");/*查找字符串"<ChannelList>"*/
            if (NULL != p_str_begin)
            {
                int list_num=0;
                ...
                memcpy(xmlChannelList, p_str_begin + strlen("<ChannelList"), p_str_end - p_str_begin - strlen("<ChannelList"));/*保存ChannelList*/
                p_str_begin = strstr(xml_param, "Num=");/*查找字符串"<ROINumber>"*/
                if (NULL != p_str_begin)
                {
                    p_str_end = strstr(p_str_begin, ">");
                    memcpy(xml_param_value, p_str_begin + strlen("Num="), p_str_end - p_str_begin - strlen("Num="));/*保存ROINumber*/
                ...
                for(i = 0; i < list_num; i++)
                {
                    ...
                    if (NULL != p_str_begin)
                    {
                        ...
                        memcpy(tmp_item_body, p_str_begin + strlen("<Item>"), p_str_end - p_str_begin - strlen("<Item>"));/*保存Item*/
                        p_str_begin = strstr(tmp_item_body, "<ChannelId>");/*查找字符串"<ROISeq>"*/
                        if (NULL != p_str_begin)
                        {
                            p_str_end = strstr(tmp_item_body, "</ChannelId>");
                            memcpy(xml_param_value, p_str_begin + strlen("<ChannelId>"), p_str_end - p_str_begin - strlen("<ChannelId>"));/*保存ROISeq*/
                            printf("ChannelId:%s\n", xml_param_value);
                            memset(xml_param_value, 0, 30);
                        }
                        p_str_begin = strstr(tmp_item_body, "<GbId>");/*查找字符串"<TopLeft>"*/
                        if (NULL != p_str_begin)
                        {
                            p_str_end = strstr(tmp_item_body, "</GbId>");
                            memcpy(xml_param_value, p_str_begin + strlen("<GbId>"), p_str_end - p_str_begin - strlen("<GbId>"));/*保存TopLeft*/
                            printf("GbId:%s\n", xml_param_value);
                            memset(xml_param_value, 0, 30);
                        }
                        p_str_begin = strstr(tmp_item_body, "<Type>");/*查找字符串"<BottomRight>"*/
                        if (NULL != p_str_begin)
                        {
                            p_str_end = strstr(tmp_item_body, "</Type>");
                            memcpy(xml_param_value, p_str_begin + strlen("<Type>"), p_str_end - p_str_begin - strlen("<Type>"));/*保存BottomRight*/
                            printf("Type:%s\n", xml_param_value);
                            memset(xml_param_value, 0, 30);
                        }
                    }
        ...
        p_str_begin = strstr(p_xml_body, "<AlarmReportSwitch>");/*查找字符串"<AlarmReportSwitch>"*//*AlarmReportSwitch配置(可选)*/
        if (NULL != p_str_begin)
        {
            int mdEnable = 0;
            int fdEnable = 0;
            ...
            memcpy(xml_config_body, p_str_begin + strlen("<AlarmReportSwitch>"), p_str_end - p_str_begin - strlen("<AlarmReportSwitch>")); /*保存<AlarmReportSwitch>到xml_config_body*/
            p_str_begin = strstr(xml_config_body, "<MotionDetection>");/*查找字符串"<MotionDetection>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</MotionDetection>");
                memcpy(xml_param_value, p_str_begin + strlen("<MotionDetection>"), p_str_end - p_str_begin - strlen("<MotionDetection>"));/*保存MotionDetection*/
            ...
            memset(xml_param_value, 0, sizeof(xml_param_value));
            p_str_begin = strstr(xml_config_body, "<FieldDetection>");/*查找字符串"<FieldDetection>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</FieldDetection>");
                memcpy(xml_param_value, p_str_begin + strlen("<FieldDetection>"), p_str_end - p_str_begin - strlen("<FieldDetection>"));/*保存FieldDetection*/
        ...
        p_str_begin = strstr(p_xml_body, "<VideoRecordLocalIfNetFault>");/*查找字符串"<Mirror>"*//*SVAC 解码配置(可选)*/
        if (NULL != p_str_begin)
        {
        	GK_NET_GB28181_CFG gbCfg;
            unsigned char rec=0;
            p_str_end = strstr(p_xml_body, "</VideoRecordLocalIfNetFault>");
            memcpy(xml_command, p_str_begin + strlen("<VideoRecordLocalIfNetFault>"), p_str_end - p_str_begin - strlen("<VideoRecordLocalIfNetFault>")); /*保存<Mirror>到xml_config_body*/
    ...
    else if (0 == strcmp(xml_cmd_type, "DeviceUpgrade"))/*固件升级*/
    {
        ...
        p_str_begin = strstr(p_xml_body, "<FirmwareUpgrade>");/*查找字符串"<FirmwareUpgrade>"*/
        p_str_end   = strstr(p_xml_body, "</FirmwareUpgrade>");
        
        if(p_str_begin && p_str_end)
        {
            memcpy(xml_upgrade, p_str_begin + strlen("<FirmwareUpgrade>"), p_str_end - p_str_begin - strlen("<FirmwareUpgrade>")); /*保存<CmdType>到xml_cmd_type*/
            printf("<FirmwareUpgrade>:%s\r\n", xml_upgrade);
            if(strlen(xml_upgrade) > 0)
            {
                p_str_begin = strstr(xml_upgrade, "<Firmware>");/*查找字符串"<Firmware>"*/
                ...
                p_str_begin = strstr(xml_upgrade, "<FileURL>");/*查找字符串"<FileURL>"*/
        ...
        //"设备后台操作固件升级，原固件版本：xxx，更新后版本：xxx，操作ip：xxx.xxx.xxx.xxx，升级时间：yyyy-MM-dd HH:mm:ss.SSS."
    ...
    else if (0 == strcmp(xml_cmd_type, "NetworkStatus"))/*网络状态*/
    {
        int MediaStatus = 1;
    ...
    else if (0 == strcmp(xml_cmd_type, "OSD"))/*查询OSD*/
    {
        int SumNum=0;
        char TextList[512];
		...
		snprintf(rsp_xml_body, sizeof(rsp_xml_body), "<?xml version=\"1.0\" encoding=\"GB2312\" ?>\r\n"
			 ...
			 "<TimeX>%f</TimeX>\r\n" /*<!--时间戳坐标-->*/
             "<TimeY>%f</TimeY>\r\n" /*<!--时间戳坐标-->*/
             "<TimeEnable>%s</TimeEnable>\r\n" /*<!--是否显示时间：false:不显示 true:显示-->*/
             "<WeekEnable>%s</WeekEnable>\r\n" /*<!--是否显示星期：TimeEnable为true时有效无该参数时，默认为false -->*/
             "<TimeType>%d</TimeType>\r\n"/*<!—时间显示类型：0: XXXX-XX-XX 年月日 ，1: XX-XX-XXXX 月日年 ，2: XXXX年XX月XX日，3: XX月XX日XXXX年-->*/
             "<TextEnable>%s</TextEnable>\r\n"
             "<SumNum>%d</SumNum>\r\n"/*<!—文字行数，可扩展-->*/
             "<TextList>\r\n"/*<!—文字内容-->*/
             "%s"
             "</TextList>\r\n"
             "<BlinkEnable>false</BlinkEnable>\r\n"/*<!—文字是否闪烁：false：不闪烁 true：闪烁，无该参数时，默认为false -->*/
             "<TranslucentEnable>true</TranslucentEnable>\r\n"/*<!—文字是否透明：false：不闪烁 true：闪烁，无该参数时，默认为true -->*/
			 /*"</Query>\r\n"*/
			 "</Response>\r\n",
			 xml_cmd_type,
			 xml_cmd_sn,
    ...
    else if (0 == strcmp(xml_cmd_type, "SetOSD"))/*配置OSD*/
    {
        ...
        p_str_begin = strstr(p_xml_body, "<Control>");/*查找字符串"<BasicParam>"*//*基本参数配置类型(可选) */
        if (NULL != p_str_begin)
        {
            p_str_end = strstr(p_xml_body, "</Control>");
            memcpy(xml_config_body, p_str_begin + strlen("<Control>"), p_str_end - p_str_begin - strlen("<Control>")); /*保存<BasicParam>到xml_config_body*/

            p_str_begin = strstr(xml_config_body, "<TimeX>");/*查找字符串"<Name>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</TimeX>");
                memcpy(xml_param, p_str_begin + strlen("<TimeX>"), p_str_end - p_str_begin - strlen("<TimeX>"));/*保存设备名称*/
                channelInfo.osdDatetime.x = atof(xml_param); /*改变注册过期时间*/
            }
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<TimeY>");/*查找字符串"<Name>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</TimeY>");
                memcpy(xml_param, p_str_begin + strlen("<TimeY>"), p_str_end - p_str_begin - strlen("<TimeY>"));/*保存设备名称*/
                channelInfo.osdDatetime.y = atof(xml_param); /*改变注册过期时间*/
            }
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<TimeEnable>");/*查找字符串"<HeartBeatInterval>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</TimeEnable>");
                memcpy(xml_param, p_str_begin + strlen("<TimeEnable>"), p_str_end - p_str_begin - strlen("<TimeEnable>"));/*保存心跳间隔时间*/
                channelInfo.osdDatetime.enable = (0 == strcmp(xml_param, "true"))?1:0;
            }
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<WeekEnable>");/*查找字符串"<HeartBeatCount>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</WeekEnable>");
                memcpy(xml_param, p_str_begin + strlen("<WeekEnable>"), p_str_end - p_str_begin - strlen("<WeekEnable>"));/*保存心跳超时次数*/
                channelInfo.osdDatetime.displayWeek = (0 == strcmp(xml_param, "true"))?1:0;
            }
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<TimeType>");/*查找字符串"<HeartBeatCount>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</TimeType>");
                memcpy(xml_param, p_str_begin + strlen("<TimeType>"), p_str_end - p_str_begin - strlen("<TimeType>"));/*保存心跳超时次数*/
                channelInfo.osdDatetime.dateFormat = atof(xml_param);
            }
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<TextEnable>");/*查找字符串"<HeartBeatCount>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</TextEnable>");
                memcpy(xml_param, p_str_begin + strlen("<TextEnable>"), p_str_end - p_str_begin - strlen("<TextEnable>"));/*保存心跳超时次数*/
            ...
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<SumNum>");/*查找字符串"<HeartBeatCount>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</SumNum>");
                memcpy(xml_param, p_str_begin + strlen("<SumNum>"), p_str_end - p_str_begin - strlen("<SumNum>"));/*保存心跳超时次数*/
            ...
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<TextList>");/*查找字符串"<HeartBeatCount>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</TextList>");
                memcpy(xml_param, p_str_begin + strlen("<TextList>"), p_str_end - p_str_begin - strlen("<TextList>"));/*保存心跳超时次数*/
                p_xml_item = xml_param;
                ...
                for(i = 0; i < SumNum; i++)
                {
                    ...
                    if (NULL != p_str_begin)
                    {
                        ...
                        memcpy(tmp_item_body, p_str_begin + strlen("<Item>"), p_str_end - p_str_begin - strlen("<Item>"));/*保存Item*/
                        p_str_begin = strstr(tmp_item_body, "<Text>");/*查找字符串"<ROISeq>"*/
                        if (NULL != p_str_begin)
                        {
                            p_str_end = strstr(tmp_item_body, "</Text>");
                            memcpy(xml_param_value, p_str_begin + strlen("<Text>"), p_str_end - p_str_begin - strlen("<Text>"));/*保存ROISeq*/
                            strcpy(osdText->text, xml_param_value);;
                            memset(xml_param_value, 0, sizeof(xml_param_value));
                        }
                        p_str_begin = strstr(tmp_item_body, "<TextLen>");/*查找字符串"<TopLeft>"*/
                        if (NULL != p_str_begin)
                        {
                            p_str_end = strstr(tmp_item_body, "</TextLen>");
                            memcpy(xml_param_value, p_str_begin + strlen("<TextLen>"), p_str_end - p_str_begin - strlen("<TextLen>"));/*保存TopLeft*/
                            int TextLen = atoi(xml_param_value);
                            printf("**********SetOSD TextLen:%d**********\r\n", TextLen);
                            memset(xml_param_value, 0, sizeof(xml_param_value));
                        }
                        p_str_begin = strstr(tmp_item_body, "<TextX>");/*查找字符串"<BottomRight>"*/
                        if (NULL != p_str_begin)
                        {
                            p_str_end = strstr(tmp_item_body, "</TextX>");
                            memcpy(xml_param_value, p_str_begin + strlen("<TextX>"), p_str_end - p_str_begin - strlen("<TextX>"));/*保存BottomRight*/
                            osdText->x = atof(xml_param_value);                            
                            printf("**********SetOSD TextX:%f(%s)**********\r\n", osdText->x, xml_param_value);
                            memset(xml_param_value, 0, sizeof(xml_param_value));
                        }
                        p_str_begin = strstr(tmp_item_body, "<TextY>");/*查找字符串"<ROIQP>"*/
                        if (NULL != p_str_begin)
                        {
                            p_str_end = strstr(tmp_item_body, "</TextY>");
                            memcpy(xml_param_value, p_str_begin + strlen("<TextX>"), p_str_end - p_str_begin - strlen("<TextX>"));/*保存ROIQP*/
            ...
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<BlinkEnable>");/*查找字符串"<HeartBeatCount>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</BlinkEnable>");
                memcpy(xml_param, p_str_begin + strlen("<BlinkEnable>"), p_str_end - p_str_begin - strlen("<BlinkEnable>"));/*保存心跳超时次数*/
                printf("**********SetOSD BlinkEnable:%s**********\r\n", xml_param);
            }
            memset(xml_param, 0, sizeof(xml_param));
            p_str_begin = strstr(xml_config_body, "<TranslucentEnable>");/*查找字符串"<HeartBeatCount>"*/
            if (NULL != p_str_begin)
            {
                p_str_end = strstr(xml_config_body, "</TranslucentEnable>");
                memcpy(xml_param, p_str_begin + strlen("<TranslucentEnable>"), p_str_end - p_str_begin - strlen("<TranslucentEnable>"));/*保存心跳超时次数*/
    ...
    else if (0 == strcmp(xml_cmd_type, "UpLoad"))/*日志订阅*/
    {
        int expires_val = 0;
...
#endif
                        if((0 != g_call_id) && (0 == g_did_backPlay) && (0 == g_did_fileDown)) /*设置全局INVITE连接ID*/
                        {
