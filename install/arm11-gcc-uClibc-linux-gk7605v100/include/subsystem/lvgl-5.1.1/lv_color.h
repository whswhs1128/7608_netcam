...
#define LV_COLOR_LIME    LV_COLOR_MAKE(0xFF,0x00,0xFF)
...
#define LV_COLOR_BLACK_BLUE_1  LV_COLOR_MAKE(0x0d,0x14,0x27)   
#define LV_COLOR_BLACK_BLUE_2  LV_COLOR_MAKE(0x16,0x1d,0x2e)   

#define LV_COLOR_BUTTON_BLUE_N  LV_COLOR_MAKE(0x1A,0x4F,0x6D)   
#define LV_COLOR_BUTTON_BLUE_H  LV_COLOR_MAKE(0x0A,0x88,0xD1)   

#define LV_COLOR_BUTTON_GRAY_N  LV_COLOR_MAKE(0x50,0x5c,0x7f)   
#define LV_COLOR_BUTTON_RED_H  LV_COLOR_MAKE(0xff,0x4e,0x64)  

#define LV_COLOR_SW_BG  LV_COLOR_MAKE(0x18,0xB2,0xDD)   

//#define LV_COLOR_TRANSP  LV_COLOR_MAKE(0xFF,0x00,0xFF)  

/*hemu UI color end*/

#define LV_OPA_TRANSP    1
#define LV_OPA_0         0
#define LV_OPA_10        25
#define LV_OPA_20        51
#define LV_OPA_30        76
...
#elif LV_COLOR_DEPTH == 4444
typedef uint16_t lv_color_int_t;
typedef lv_color4444_t lv_color_t;
...
#elif LV_COLOR_DEPTH == 4444
#define LV_COLOR_MAKE(r8, g8, b8) ((lv_color_t){{0xff >> 4, r8 >> 4, g8 >> 4, b8 >> 4}})
#elif LV_COLOR_DEPTH == 24
...
#elif LV_COLOR_DEPTH == 4444
#define LV_COLOR_MAKE(r8, g8, b8) ((lv_color_t){{b8 >> 4,  g8>> 4, r8 >> 4, 0xff >> 4}})
#elif LV_COLOR_DEPTH == 24
...
#elif LV_COLOR_DEPTH == 16
    lv_color8_t ret;
    ret.red = color.red >> 2;       /* 5 - 2  = 3*/
    ret.green = color.green >> 3;   /* 6 - 3  = 3*/
    ret.blue = color.blue >> 3;     /* 5 - 3  = 2*/
    return ret.full;
#elif LV_COLOR_DEPTH == 4444
	lv_color8_t ret;
    ret.red = color.red >> 1;       /* 4 - 1  = 3*/
    ret.green = color.green >> 1;   /* 4 - 1  = 3*/
    ret.blue = color.blue >> 2;     /* 4 - 2  = 2*/
    return ret.full;
#elif LV_COLOR_DEPTH == 24
    lv_color8_t ret;
...
#elif LV_COLOR_DEPTH == 4444
    lv_color24_t ret;
    ret.red = color.red * 17;       /*(2^8 - 1)/(2^4 - 1) = 255/15 = 17*/
    ret.green = color.green * 17;   /*(2^8 - 1)/(2^4 - 1) = 255/15 = 17*/
    ret.blue = color.blue * 17;     /*(2^8 - 1)/(2^4 - 1) = 255/15 = 17*/
    ret.alpha = color.alpha * 17;
    return ret.full;
#elif LV_COLOR_DEPTH == 24
    return color.full;
#endif
...
{
    lv_color_t ret;
    lv_color_t color = LV_COLOR_LIME;
    lv_color_t color_bk = LV_COLOR_BLACK_BLUE_2;
    
#if LV_COLOR_DEPTH != 1
   if((c1.red == color.red)&&(c1.green == color.green)&&(c1.blue == color.blue))
    {
       ret.red =   (uint16_t)((uint16_t) color_bk.red * mix + (c2.red * (255 - mix))) >> 8;
       ret.green = (uint16_t)((uint16_t) color_bk.green * mix + (c2.green * (255 - mix)))>> 8;
       ret.blue =  (uint16_t) ((uint16_t) color_bk.blue * mix + (c2.blue * (255 - mix))) >> 8;
    }
    else if((c2.red == color.red)&&(c2.green == color.green)&&(c2.blue == color.blue))
    {
        ret.red =   (uint16_t)((uint16_t) c1.red * mix + (color_bk.red * (255 - mix))) >> 8;
        ret.green = (uint16_t)((uint16_t) c1.red * mix + (color_bk.red * (255 - mix))) >> 8;
        ret.blue =  (uint16_t)((uint16_t) c1.red * mix + (color_bk.red * (255 - mix))) >> 8;
    }
    else
    {
