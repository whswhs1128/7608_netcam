...
lv_obj_t * lv_list_create(lv_obj_t * par, lv_obj_t * copy)
{
    ...
    ext->styles_btn[LV_BTN_STATE_TGL_REL] = &lv_style_btn_pr;
...
lv_obj_t * lv_list_add(lv_obj_t * list, const void * img_src, const char * txt, lv_action_t rel_action)
{
	...
	lv_btn_set_fit(liste, false, false);
 ...
 * @param img_fn file name of an image before the text (NULL if unused)
 * @param txt text of the list element (NULL if unused)
 * @param txt2 text of the list element (NULL if unused)
 * @param txt3 text of the list element (NULL if unused)
 * @param rel_action pointer to release action function (like with lv_btn)
 * @return pointer to the new list element which can be customized (a button)
 */
lv_obj_t * lv_list_param3_add(lv_obj_t * list, const void * img_src, const char * txt, const char * txt2, const char * txt3, lv_action_t rel_action, const bool w_type)
{
	lv_style_t * style = lv_obj_get_style(list);
    lv_list_ext_t * ext = lv_obj_get_ext_attr(list);
	...
	if(txt != NULL) {
		...
		lv_obj_set_click(label, false);
        lv_label_set_long_mode(label, LV_LABEL_LONG_ROLL);
        lv_label_set_offset_x(label, 15);
		if(true == w_type)
		{
	...
	if(txt2 != NULL) {
		lv_obj_t * label1 = lv_label_create(liste, NULL);
		lv_label_set_text(label1, txt2);
		lv_obj_set_click(label1, false);
		lv_label_set_long_mode(label1, LV_LABEL_LONG_ROLL);
		if(true == w_type)
		{
		...
		{
			lv_obj_set_width(label1, 0);
		}
	}
	if(txt3 != NULL) {
		lv_obj_t * label2 = lv_label_create(liste, NULL);
		lv_label_set_text(label2, txt3);
		lv_obj_set_click(label2, false);
		lv_label_set_long_mode(label2, LV_LABEL_LONG_ROLL);
		lv_obj_set_width(label2, 25);
	}
...
/**
 * focus the btn by the text of the btn.
 * @param list pointer to a list object
 * @param text to compare
 * @return btn pointer to the the same text btn or NULL
 */
lv_obj_t * lv_list_focus_by_text(lv_obj_t * list, char *text)
{
    lv_obj_t * label = NULL;
    lv_obj_t * btn = NULL;
    ...
    btn_prev = lv_obj_get_child_back(lv_page_get_scrl(list), NULL);
    if (btn != NULL && btn != btn_prev) {
    ...
    if(sign == LV_SIGNAL_CORD_CHG) {
        ...
        if(c == LV_GROUP_KEY_RIGHT || c == LV_GROUP_KEY_DOWN) {
            ...
            if(btn_prev != NULL && btn != NULL) {
                ...
                //go to the first
                btn_prev = lv_obj_get_child_back(lv_page_get_scrl(list), NULL);
                btn = lv_obj_get_child(lv_page_get_scrl(list), NULL);
                lv_btn_set_state(btn, LV_BTN_STATE_REL);
                lv_btn_set_state(btn_prev, LV_BTN_STATE_TGL_REL);
                lv_page_focus(list, btn_prev, ext->anim_time);
            }
            ...
            if(btn != NULL) {
                ...
                if(btn_prev != NULL) {
                    ...
                    //go to the last
                    btn = lv_obj_get_child_back(lv_page_get_scrl(list), NULL);
                    btn_prev = lv_obj_get_child(lv_page_get_scrl(list), NULL);
                    lv_btn_set_state(btn, LV_BTN_STATE_REL);
                    lv_btn_set_state(btn_prev, LV_BTN_STATE_TGL_REL);
                    lv_page_focus(list, btn_prev, ext->anim_time);
                }
            }
        } else if(c == LV_GROUP_KEY_ENTER || c == LV_GROUP_KEY_ENTER_LONG||c == LV_GROUP_KEY_ESC||c == LV_GROUP_KEY_CUT) {
            /*Get the 'pressed' button*/
            lv_obj_t * btn = NULL;
            btn = get_next_btn(list, btn);
            while(btn != NULL) {
