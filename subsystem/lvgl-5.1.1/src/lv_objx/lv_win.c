...
lv_obj_t * lv_win_create(lv_obj_t * par, lv_obj_t * copy)
{
    ...
    ext->btn_size = LV_WIN_TITLE_H;//( LV_DPI) / 2;
...
lv_obj_t * lv_win_notitle_create(lv_obj_t * par, lv_obj_t * copy)
{
    /*Create the ancestor object*/
    lv_obj_t * new_win = lv_obj_create(par, copy);
    lv_mem_assert(new_win);
    ...
    ext->header = NULL;
    ext->title = NULL;
    ext->style_header = &lv_style_title_label;
    ext->style_btn_rel = &lv_style_btn_rel;
    ext->style_btn_pr = &lv_style_btn_pr;
    ext->btn_size = 1;//( LV_DPI) / 2;
    ...
    if(copy == NULL) {
        ...
        if(th) {
            ...
            lv_win_set_style(new_win, LV_WIN_STYLE_CONTENT_BG, &lv_style_transp);
            lv_win_set_style(new_win, LV_WIN_STYLE_CONTENT_SCRL, &lv_style_transp);
            lv_win_set_style(new_win, LV_WIN_STYLE_HEADER, &lv_style_title_label);
...
{
       static lv_style_t label_style_title;
       lv_style_copy(&label_style_title,&lv_style_scr); 
       label_style_title.text.font = LV_FONT_TITLE;
       
	lv_win_ext_t * ext = lv_obj_get_ext_attr(win);
    
       lv_label_set_style(ext->title, &label_style_title);
...
static void lv_win_realign(lv_obj_t * win)
{
    ...
    lv_obj_set_size(ext->header, lv_obj_get_width(win), ext->btn_size + 2 * header_style->body.padding.ver+8);
    ...
    lv_obj_align(ext->title, NULL, LV_ALIGN_CENTER, ext->style_header->body.padding.hor, 0);
