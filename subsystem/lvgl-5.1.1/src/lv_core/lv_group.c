 ...
 *      DEFINES
 *********************/
static char lv_group_exit_key = 0;
...
static void style_red_mod_def(lv_style_t * style);
 ...
 * @return pointer to the new object group
 */
lv_group_t * lv_group_red_mode_create(void)
{
    lv_group_t * group = lv_mem_alloc(sizeof(lv_group_t));
    lv_ll_init(&group->obj_ll, sizeof(lv_obj_t *));

    group->style_mod = style_red_mod_def;
    group->obj_focus = NULL;
    group->frozen = 0;
    group->focus_cb = NULL;
...
static void style_mod_def(lv_style_t * style)
{
    ...
    style->body.border.color = LV_COLOR_BLACK_BLUE_2;

    /*If not empty or has border then emphasis the border*/
    if(style->body.empty == 0 || style->body.border.width != 0) style->body.border.width = 1;//LV_DPI / 40;

    style->body.main_color = LV_COLOR_BUTTON_BLUE_H;//lv_color_mix(style->body.main_color, LV_COLOR_BLACK_BLUE_2, LV_OPA_0);
    style->body.grad_color = LV_COLOR_BUTTON_BLUE_H;//lv_color_mix(style->body.grad_color, LV_COLOR_BLACK_BLUE_2, LV_OPA_0);
    style->body.shadow.color = LV_COLOR_BLACK_BLUE_2;//lv_color_mix(style->body.shadow.color, , LV_OPA_0);
    style->body.shadow.width = 1;
    style->body.shadow.type = LV_SHADOW_FULL;
...
static void style_red_mod_def(lv_style_t * style)
{
    ...
    style->body.main_color = LV_COLOR_BUTTON_RED_H;//lv_color_mix(style->body.main_color, LV_COLOR_BLACK_BLUE_2, LV_OPA_0);
    style->body.grad_color = LV_COLOR_BUTTON_RED_H;//lv_color_mix(style->body.grad_color, LV_COLOR_BLACK_BLUE_2, LV_OPA_0);
    style->body.shadow.color = LV_COLOR_BLACK_BLUE_2;//lv_color_mix(style->body.shadow.color, , LV_OPA_0);
    style->body.shadow.width = 1;
    style->body.shadow.type = LV_SHADOW_FULL;
   
    style->text.color = LV_COLOR_WHITE;//lv_color_mix(style->text.color, LV_COLOR_BLUE_H, LV_OPA_0);
#else
    style->body.border.opa = LV_OPA_COVER;
    style->body.border.color = LV_COLOR_BLACK;
    style->body.border.width = 3;
...
char lv_group_get_cur_key_type(void)
{
    return lv_group_exit_key;
}

void lv_group_set_cur_key_type(char c_key)
{
     lv_group_exit_key = c_key;
}
