...
void lv_indev_init(void)
{
    ...
    //lv_task_create(indev_proc_task, LV_INDEV_READ_PERIOD, LV_TASK_PRIO_HIGHEST, NULL);
#endif
...
static void indev_keypad_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    else if(data->state == LV_INDEV_STATE_REL && i->proc.last_state == LV_INDEV_STATE_PR)
    {
        ...
        if(data->key == LV_GROUP_KEY_RIGHT){
                   lv_group_focus_next(i->group);
        }
        else if(data->key == LV_GROUP_KEY_LEFT){
            lv_group_focus_prev(i->group);
        }
        else {

            if((data->key != LV_GROUP_KEY_UP)&&(data->key != LV_GROUP_KEY_DOWN)\
              &&(data->key != LV_GROUP_KEY_ESC)&&(data->key != LV_GROUP_KEY_ENTER)\
              &&(data->key != LV_GROUP_KEY_NEXT)&&(data->key != LV_GROUP_KEY_PREV)\
              &&(data->key != LV_GROUP_KEY_CUT))
              {
                    return;
              }
