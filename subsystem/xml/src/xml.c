...
** \file        xml.c
**
** \version     $Id: xml.c 1110 2016-07-26 17:26:58Z heyong
**
** \brief       xml
**
...
/*FIXME(heyong): porting apis */
#ifndef Y_PRINTF
#define Y_PRINTF(fmt, args...)      printf("[Xml] "fmt, ##args)//GM_Printf("[Xml] "fmt, ##args)
#endif

#ifndef Y_MALLOC
#define Y_MALLOC(x)                 malloc(x)//gadi_sys_malloc(x)
#endif

#ifndef Y_FREE
#define Y_FREE(x)                   free(x)//gadi_sys_free((void *)x)
#endif

char * Y_STRDUP(const char *s)
{
	char *new;
	if ((s == NULL) || ((new = Y_MALLOC (strlen(s) + 1)) == NULL) )
	{
		return NULL;
	}
	strcpy(new, s);
...
XMLN *xml_node_add_new(XMLN *parent, char *nodeName)
{
    ...
    if (p_node == NULL)
    {
        Y_PRINTF("xml_node_add_new:memory alloc fail!\n");
        return NULL;
    }
	memset(p_node, 0, sizeof(XMLN));

    p_node->type = NTYPE_TAG;
	p_node->name = (char *)Y_STRDUP(nodeName);//p_node->name = name;
...
XMLN *xml_node_add(XMLN *parent, XMLN *newNode)
{
    ...
    {
        Y_PRINTF("xml_node_add:parent or newNode is NULL\n");
        return NULL;
    }
...
void xml_node_del(XMLN *rootNode)
{
	...
	if(rootNode->name)
	    Y_FREE(rootNode->name);
	if(rootNode->data) 
	    Y_FREE(rootNode->data);
...
XMLN *xml_node_set_data(XMLN *node, const char *data)
{
    ...
    node->data = Y_MALLOC((strlen(data) + 1) * sizeof(char));
    if(NULL == node->data)
    {
        Y_PRINTF("xml_node_set_data: memory alloc fail!\n");
    }
...
XMLN *xml_attr_add(XMLN *node, const char *attrName, const char *attrData)
{
	...
	XMLN *p_attr = (XMLN *)Y_MALLOC(sizeof(XMLN));
    if (p_attr == NULL)
    {
        Y_PRINTF("xml_attr_add: memory alloc fail!\n");
        return NULL;
    }
	...
	p_attr->name = (char *)Y_STRDUP(attrName);    //p_attr->name = name;
	p_attr->data = (char *)Y_STRDUP(attrData);	//p_attr->data = value;
...
XMLN * xml_stream_parse(char *p_xml, int len)
{
    ...
    if (status < 0)
    {
        Y_PRINTF("xml_stream_parse:err[%d]\r\n", status);
        xml_node_del(p_root);
        p_root = NULL;
    }
...
XMLN *xml_file_parse(FILE *fp)
{
    ...
    if(0 != fseek(fp, 0, SEEK_SET))
        return NULL;
    XMLdata = Y_MALLOC(XMLsize * sizeof(char));
    if(!XMLdata)
        return NULL;
    memset(XMLdata, 0, XMLsize);
    if (fread(XMLdata, 1, XMLsize-1, fp) != XMLsize-1) {
        Y_PRINTF("fread error!\n");
        return NULL;
	}
    *(XMLdata+XMLsize-1) = '\0';
...
void stream_startElement(void *userData, const char *nodeName, const char **attrs)//handle new node and attributes.
{
    ...
    if (attrs)
    {
        ...
        while (attrs[i] != NULL)//attributes of one element will be connected to each other one by one.
        {
            ...
            if(p_attr == NULL)
            {   
                Y_PRINTF("xml add attr node failed!\n");
                break;
            }         
...
void stream_charData(void *userData, const char *data, int dataLen)
{
	...
	p_node->data = (char *)Y_STRDUP(data);   //p_node->data = data;
...
int stream_parse(LTXMLPRS *parse)
{
    ...
    if(IS_XMLH_START(ptr))
    {
        ...
        if(ret < 0)
        {
            Y_PRINTF("hxml parse xml header failed!!!\r\n");
            return -1;
        }
    }
...
int stream_parse_element_end(LTXMLPRS *parse)
{
    ...
    *(parse->ptr) = '\0';
    ...
    {
        Y_PRINTF("stream_parse_element_end::cur name[%s] != stack name[%s]!!!\r\n", end_name, stack_name);
        return -1;
    }
