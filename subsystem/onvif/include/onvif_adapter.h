...
typedef struct
{
    GK_S32  chEnable;            /* 0-禁用, 1-使能 */
	GK_CHAR chName[MAX_32_LENGTH];
    float   chx;
    float   chy;

    GK_S32  enable; /* 0-禁用, 1-使能 */
    GK_U8   dateFormat; /*日期格式(年月日格式)：0－YYYY-MM-DD;1－MM-DD-YYYY;2－YYYY/MM/DD;3－MM/DD/YYYY;4－DD-MM-YYYY;5－DD/MM/YYYY*/
    GK_U8   dateSprtr; //0：":"，1："-"，2："/" 3："."
    GK_U8   timeFmt; //时间格式 (0-24小时，1－12小时).
    float   x;
    float   y;
	GK_U8	displayWeek; //是否显示星期几
}ONVIF_OsdInfor_S;

typedef struct
{
    GK_S32 enable;     /* 遮挡使能 0-禁用, 1-使能 */
    float  x;          /* 遮挡区域左上角的x坐标 */
    float  y;          /* 遮挡区域左上角的y坐标 */
    float  width;      /* 遮挡区域宽度 */
    float  height;     /* 遮挡区域高度 */
    GK_U32 color;      /* 遮挡颜色, 默认 0:黑色 按RGB格式*/
}ONVIF_PMInfor_S;
...
typedef struct
{
    ...
    void   (*SaveCfg)(void);
    //osd
    GK_S32 (*GetOsdInfor)(ONVIF_OsdInfor_S *pstOsdInfor);
    GK_S32 (*SetOsdInfor)(ONVIF_OsdInfor_S *pstOsdInfor);
    //pm
    GK_S32 (*GetPMInfor)(ONVIF_PMInfor_S *pstPMInfor);
    GK_S32 (*SetPMInfor)(ONVIF_PMInfor_S *pstPMInfor);
...
typedef struct
{
	...
	GK_S32 (*GetNetworkInfor_loca)(GK_CHAR *netCardName, ONVIF_DeviceNet_S *netInfor);
	#endif
