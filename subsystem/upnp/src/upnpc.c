...
void *add_port_redirection(char *ip, int port, int external_port, char *protocol)
{
    //./upnpc-static -a 192.168.1.101 12345 88888 TCP
    char* argv[6] = {"","-a"};
    int argc = 6;
    ...
    memset(port_buffer, 0, 128);
    sprintf(port_buffer, "%d", port);
    argv[3] = port_buffer;

    char eport_buffer[128];
    memset(eport_buffer, 0, 128);
    sprintf(eport_buffer, "%d", external_port);
    argv[4] = eport_buffer;

    argv[5] = protocol;

    upnpc_main(argc, argv);
}

void *add_port_redirection_localip(int port, int external_port, char *protocol)
{
    //./upnpc-static -a 192.168.1.101 12345 88888 TCP
    char* argv[4] = {"","-r"};
    int argc = 4;
...
char g_lanaddr[64];   /* my ip address on the LAN */
struct UPNPUrls g_urls;
struct IGDdatas g_data;

int init_port_redirection()
{
	int retcode = 0;
	int error = 0;
	int ipv6 = 0;
    ...
    memset(g_lanaddr, 0 , sizeof(g_lanaddr));

    if(g_devlist = upnpDiscover(2000, NULL, NULL,
	                             0/*sameport*/, ipv6, ttl, &error))
	{
		struct UPNPDev * device;
		printf("List of UPNP devices found on the network :\n");
		for(device = g_devlist; device; device = device->pNext)
		{
			printf(" desc: %s\n st: %s\n\n",
				   device->descURL, device->st);
		}
		i = 1;
		if(i = UPNP_GetValidIGD(g_devlist, &g_urls, &g_data, g_lanaddr, sizeof(g_lanaddr)))
		{
			switch(i) {
			case 1:
				printf("Found valid IGD : %s\n", g_urls.controlURL);
			...
			#if 0
			printf("getting \"%s\"\n", g_urls.ipcondescURL);
			descXML = miniwget(g_urls.ipcondescURL, &descXMLsize);
			if(descXML)
			{
				/*fwrite(descXML, 1, descXMLsize, stdout);*/
				free(descXML); descXML = NULL;
...
void add_port_redirection(int port, int external_port, char *protocol)
{
    ...
    SetRedirectAndTest(&g_urls, &g_data,
               g_lanaddr, port_buffer,
               eport_buffer, protocol,"0",0, 0, 1);
}
...
void del_port_redirection(int external_port, char *protocol)
{
    ...
    RemoveRedirect(&g_urls, &g_data, eport_buffer, protocol, NULL);
}

void del_all_port_redirection()
{
    //just for igd2
    //RemoveRedirectRange(&g_urls, &g_data, "0", "65535", "TCP", NULL);
    //RemoveRedirectRange(&g_urls, &g_data, "0", "65535", "UDP", NULL);
...
void set_port_redirection_state(int port, int external_port, char *protocol, int enable)
{
    ...
    SetRedirectAndTest(&g_urls, &g_data,
               g_lanaddr, port_buffer,
               eport_buffer, protocol,"0",0, 0, enable);
}
