...
#define TS_FREE(p)	do{ if(p){ free((void*)(p));(p)=NULL;} }while(0)
...
/*
static char *version ="ftplib Release 3.1-1 11/6/03, copyright 1996-2003 Thomas Pfau";
*/
 
GLOBALDEF int ftplib_debug = 0;
 
//*************************************修正处
GLOBALDEF int totalfilesize=0;//自定义
GLOBALDEF int saveflag=1;//自定义
 
//*************************************修正处 
 
#if defined(__unix__) || defined(VMS)
#define net_read TSread
#define net_write TSwrite
#define net_close TSclose
#elif defined(_WIN32)
#define net_read(x,y,z) TSrecv(x,y,z,0)
#define net_write(x,y,z) TSsend(x,y,z,0)
#define net_close TSclosesocket
#endif
...
static int socket_wait(netbuf *ctl)
{
	...
	do
	{
		...
		tv = ctl->idletime;
		rv = TSselect(ctl->handle+1, rfd, wfd, NULL, &tv);
		if (rv == -1)
		{
			rv = 0;
...
{ 
    ...
    { 
		printf("Control socket read failed");
		return 0;     
	}
    if (ftplib_debug > 1)
...
{ 
#if defined(_WIN32) && !defined(LWIP)
...
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl)
{
	...
	if ((sin.sin_addr.s_addr = TSinet_addr(lhost)) == -1)
	{
		struct in_addr server;
		int ret;
		//if ((phe = TSgethostbyname(lhost)) == NULL)

		ret = utility_net_resolve_host(&server,lhost);
		if(ret)
		{
			printf("gethostbyname_goke");
			return 0;
		}
		memcpy((char *)&sin.sin_addr, &server, sizeof(server));
	}
	TS_FREE(lhost);
	sControl = TSsocket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
	if (sControl == -1)
	{
		printf("socket");
		return 0;
	}
	if (TSsetsockopt(sControl,SOL_SOCKET,SO_REUSEADDR,	SETSOCKOPT_OPTVAL_TYPE &on, sizeof(on)) == -1)
	{
		printf("setsockopt");
		net_close(sControl);
		return 0;
...
#ifdef WIN32
	TSioctlsocket(sControl, FIONBIO, &flags); //设置为非阻塞模式
#else
	if ((flags = fcntl(sControl,F_GETFL,0)) < 0)
	{
		printf("F_GETFL failed");
	}
	else
	{
		if (fcntl(sControl, F_SETFL, flags|O_NONBLOCK) < 0)
		{
	...
	if (TSconnect(sControl, (struct sockaddr *)&sin, sizeof(sin)) == -1)
	{
		FD_ZERO(&fd);
		FD_SET( sControl,&fd);
		tv.tv_sec = 20;
		tv.tv_usec = 0;
		if ( 0 >= TSselect((sControl+1), NULL, &fd, NULL, &tv) )
		{
...
#ifdef WIN32
	flags = 0;
	TSioctlsocket(sControl, FIONBIO, &flags); //设置为阻塞模式
#else
	...
	ctrl->idlecb = NULL;
	ctrl->idletime.tv_sec =5;//modify by jerry chen 0 -> 2
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
	...
	{
		if (TSgetsockname(nControl->handle, &sin.sa, &l) < 0)
		{
			printf("getsockname");
			return 0;
		}
	}
	sData = TSsocket(PF_INET,SOCK_STREAM,IPPROTO_TCP);
	if (sData == -1)
	{
		printf("socket");
		return -1;
	}
	if (TSsetsockopt(sData,SOL_SOCKET,SO_REUSEADDR,
	...
	if (TSsetsockopt(sData,SOL_SOCKET,SO_LINGER,
	...
	if (nControl->cmode == FTPLIB_PASSIVE)
	{
		if (TSconnect(sData, &sin.sa, sizeof(sin.sa)) == -1)
		{
			printf("connect");
			net_close(sData);
			return -1;
	...
	{
		sin.in.sin_port = 0;
		if (TSbind(sData, &sin.sa, sizeof(sin)) == -1)
		{
			printf("bind");
			net_close(sData);
			return 0;
		}
		if (TSlisten(sData, 1) < 0)
		{
			printf("listen");
			net_close(sData);
			return 0;
		}
		if (TSgetsockname(sData, &sin.sa, &l) < 0)
			return 0;
...
{ 
	...
	if (i < nData->handle)
	i = nData->handle;
	i = TSselect(i+1, &mask, NULL, NULL, &tv);
	if (i == -1)
	{
	...
	else
	{
		...
		{
			l = sizeof(addr);
			sData = TSaccept(nData->handle, &addr, &l);
			i = errno;
			net_close(nData->handle);
			if (sData > 0)
			{
    ...
    netbuf **nData)
{
	...
	int dir;
	if ((path == NULL) &&
	((typ == FTPLIB_FILE_WRITE) || (typ == FTPLIB_FILE_READ || (typ == FTPLIB_FILE_APPE))))
	{
		sprintf(nControl->response,
		"Missing path argument for file transfer\n");
		printf("Missing path argument for file transfer\n");
		return 0;
	}
	sprintf(buf, "TYPE %c", mode);
	if (!FtpSendCmd(buf, '2', nControl))
	{
		printf("FtpSendcmd TYPE error\r\n");
		return 0;
	}
	switch (typ)
	{
	...
	if (nControl->cmode == FTPLIB_PORT)
	{
		if (!FtpAcceptConnection(*nData,nControl))
		{
			...
			*nData = NULL;
			nControl->data = NULL;
			printf("FtpAcceptConnection error\r\n");
			return 0;
		}
	}
	return 1;
...
netbuf *nControl, int typ, int mode)
{
	...
	{
		if ((local != NULL)&&(local!=stdin)&&(local!=stdout))
		{
			//add by JenshyHong,资源（文件描述符）泄露
			fclose(local);
		}
		printf("FtpAccess error\r\n");
		return 0;
	}
	...
	else
	{
		...
		{
			totalfilesize=totalfilesize+l;
			if (l<= 0)
			{
				printf("==========localfile write===========");
				rv = 0;
				break;
			}
 ...
 * mode:1--dir,2--file
 * netbuf *nControl:A handle returned by FtpConnect().
 *
 * return:1 if and only if the file or directory denoted by this abstract pathname exists; 0 otherwise
 */
GLOBALREF int FtpExists(const char* RemotePath,int mode,netbuf *nControl)
{

	netbuf* net_buf;
	int inner_mode=0;
	int result = 0;
	if(RemotePath==NULL||!strlen(RemotePath))
	{
		//fprintf(stderr,"flie--%s,line--%d,func--%s\nerror path!\n",__FILE__,__LINE__,__func__);
		return 0;
	}

	if(mode==1)
	{
		inner_mode=FTPLIB_DIR;
	}
	else if(mode==2)
	{
		inner_mode=FTPLIB_FILE_READ;
	}
	else
	{
	...
	result = FtpAccess(RemotePath,inner_mode,FTPLIB_IMAGE,nControl,&net_buf);
	if(result==1)
	{
		FtpClose(net_buf);
	}
...
//**************************************修正处获取下载文件数据量
GLOBALDEF int Getftpfilesize(void)
{ 
    //printf("totalfilesize++++++++++++++++++++%d\n",totalfilesize);
    return totalfilesize; 
}
 
GLOBALDEF int Setsavetype(int num)
{ 
   printf("===========saveflag is begin==========\n");
   saveflag=num;
   return 0; 
} 
//**************************************修正处获取下载文件数据量
