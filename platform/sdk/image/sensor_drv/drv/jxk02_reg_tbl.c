...
*****************************************************************************
*/
GADI_VI_SensorDrvInfoT    jxk02_sensor =
{
    ...
    .HwInfo         =
    {
        ...
        .id_reg =
        {
            {0x0A, 0x04},   // PIDH[7:0]:Product ID MSBs. 0x04
    ...
    .field_format       = 1,
    .adapter_id         = 0,
    .sensor_id          = GADI_ISP_SENSOR_JXK02,
    ...
    .fmt_num            = 1,
    .auto_fmt           = GADI_VIDEO_MODE(2048,  1536,  25, 1),
    .init_reg           =
    {
        // 0x03~0x04    // RSVD
        ...
        {0x12, 0x40},   // SYS[7]: Soft reset initialize, 1: initial system reset,
                        ...
                        //      110: V-FastBin,HSkip,
                        //      111: VSkip,HBin
        {0x0D, 0x07},   // DVP2[7:6]: Pump clock frequency selection.
                        // DVP2[5:4]: N-Pump clock frequency selection.
                        // DVP2[3:2]: PAD drive capability. 00: min, 11: max.
                        // DVP2[1:0]: AGC Latch option
                        // "00"Gain Delay Off;"11": Gain Delay one frame;"01","10" RSVD.
        ...
        //{0x1F, 0x20}, // Group latch control
                        ...
                        // GLat[0]: RSVD
        {0x2A, 0xD1},   // Column shift control 1 Cshift1[7:6]: Column SRAM data shift start position LSBs.
        // 0x31~0x38    // RSVD
        {0x32, 0x04},   // RSVD
        {0x33, 0x04},   // RSVD
        //{0x39, 0x40},
        {0x39, 0x00},   // Sensor timing control 9
                        ...
                        // SenT9[5:0]: RSVD
        // 0x3A~0x40    // RSVD
        //{0x3D, 0x00}, // RSVD same with default
        {0x3E, 0x20},   // RSVD
        ...
        {0x49, 0x90},   // Black level calibration target level.
                        // BLC_TGT[7]: sign bit."1" : Positive;"0" : Negative
        ...
        //{0x4A, 0x03}, // BLC control
                        ...
                        // BLCCtrl[3:0]: RSVD
        //{0x4B, 0x3A}, // BLC_B B channel black value LSBs. BLC_B[7:0]
        //{0x4C, 0x3A}, // BLC_Gb Gb channel black value LSBs. BLC_Gb[7:0]
        //{0x4D, 0x39}, // BLC_Gr Gr channel black value LSBs. BLC_Gr[7:0]
        //{0x4E, 0x3A}, // BLC_R R channel black value LSBs. BLC_R[7:0]
        ...
        {0x58, 0x90},   // RSVD
        {0x5B, 0x55},   // RSVD
        //{0x5C, 0x0F}, // RSVD same with default
        {0x5D, 0x2F},   // RSVD
        ...
        // 0x61~0x66    // RSVD
        {0x62, 0xC4},
        //{0x63, 0x00}, // same with default
        {0x64, 0x78},
        {0x67, 0x38},   // PWC1[7]: regulator power down
                        // PWC1[6:4]: RSVD
                        // PWC1[3:2]: VrgMipi control
                        // PWC1[1:0]: RSVD
        {0x68, 0x00},   // PWC2[7:4]: band-gap reference control
                        // PWC2[3:0]: small band-gap reference control
        // 0x69~0x6A    // RSVD
        {0x6A, 0x4F},
        ...
        {0x70, 0x70},   // Mipi timing control 1
                        // Mipi1[7:5]: Tlpx control
                        // Mipi1[4:2]: Tck-preP, before CK_ZERO
                        // Mipi1[1]: Mipi 8/10 bit mode switch, 0: 10-bit, 1: 8-bit mode
                        // Mipi1[0]: RSVD
        ...
        //{0x75, 0x03}, // Mipi6[7:6]: Manual Virtual ID for normal mode or HDR long packet
                        // Mipi6[5:4]: MIPI deskew calibration option: 00: off 01:4line 10:8line 11:before FS
    ...
    .pll_table =
    {
        [0] =
        {
            ...
            .regs = {
                                ...
                                //        = 864000000 * (1 + 0) / 2 / 2 =
                {0x0C, 0x80},   // DVP1[7:6]: HCLK clock divider from sysclk. 0: /1, 1: /2, 2: /4, 3: /8
                                // DVP1[5:4]: SRAM_CLK delay option
                                // DVP1[3:2]: HCLK delay option
                                // DVP1[1:0]: DVP test mode output option.
                                // 00: DVP output normal image data.
                                // 01: DVP[9:0] = Output 10-bit walk 1 test pattern.
                                // 10:,11: RSVD
                {0x0E, 0x11},   // PLL1[7]: PLL bypass selection. 1: PLLclk = Input clock/PLL_Pre_Ratio. 0: see PLL2[3:0].
                ...
                {0x0F, 0x20},   // PLL2[7:6]: DVP_PCLK clock divider. 0:by 1, 1: by2, 2: by 4, 3: by8
                                ...
                                //                                      = 864000000/(1 + 0) = 864000000
                                // PLL2[1:0]: Sys clock divider. PLLclk = VCO/(1+PLL2[3:0])
                                //                                      = 864000000/(1 + 0) = 864000000
                {0x10, 0x40},   // PLL3 VCO = Input clock*PLL3[7:0]/PLL_Pre_Ratio
                                //     = (27000000*64/2)=864000000
    ...
    .video_fmt_table =
    {
        [0] = // 2048x1536@25fps (540 * 4) * 1600 * 25 = 86400000   encode max 21fps
        {
            .video_mode_table =
            {
            ...
            .fmt_reg =
            {
                ...
                {0x24, 0x00},   // HWin_L       0x200 = 512 * 4 = 2048
                {0x25, 0x00},   // VWin_L       0x600 = 1536
                {0x26, 0x62},   // VWin_H HWin_H
                {0x27, 0xEF},   // HWinSt_L     0x0EF = 239
                {0x28, 0x1B},   // VWinSt_L     0x01B = 27
                {0x29, 0x00},   // VWinSt_L HWinSt_H
                {0x2B, 0x20},   // [7:6]SenHASt_H
                ...
                {0x2C, 0x52},   // SenHASt_L    0x052 =  82 * 4 =  328
...
#define JXK02_V_FLIP    (1<<4)
#define JXK02_H_MIRROR  (1<<5)
    .mirror_table =
    {
        [GADI_VI_SRC_MIRROR_HORRIZONTALLY_VERTICALLY] =
        {
            {0x12, 0x40, 0x40},
            {0x12, JXK02_H_MIRROR | JXK02_V_FLIP, JXK02_H_MIRROR | JXK02_V_FLIP},
            {0x28, 0x36},   // VWinSt_L     0x036 = 54
            {0x12, 0x00, 0x40},
            {GADI_VI_SENSOR_TABLE_FLAG_END,},
        },
        ...
        {
            {0x12, 0x40, 0x40},
            {0x12, JXK02_H_MIRROR, JXK02_H_MIRROR | JXK02_V_FLIP},
            {0x28, 0x1B},   // VWinSt_L     0x01B = 27
            {0x12, 0x00, 0x40},
            {GADI_VI_SENSOR_TABLE_FLAG_END,},
        },
        ...
        {
            {0x12, 0x40, 0x40},
            {0x12, JXK02_V_FLIP, JXK02_H_MIRROR | JXK02_V_FLIP},
            {0x28, 0x36},   // VWinSt_L     0x036 = 54
            {0x12, 0x00, 0x40},
            {GADI_VI_SENSOR_TABLE_FLAG_END,},
        },
        ...
        {
            {0x12, 0x40, 0x40},
            {0x12, 0x00, JXK02_H_MIRROR | JXK02_V_FLIP},
            {0x28, 0x1B},   // VWinSt_L     0x01B = 27
            {0x12, 0x00, 0x40},
            {GADI_VI_SENSOR_TABLE_FLAG_END,},
        },
    ...
    .gain_reg =
    {
        ...
        {0x0D, 0x00},   // DVP2[7:6]: Pump clock frequency selection.
                        // DVP2[5:4]: N-Pump clock frequency selection.
                        // DVP2[3:2]: PAD drive capability. 00: min, 11: max.
                        // DVP2[1:0]: AGC Latch option
                        // "00"Gain Delay Off;"11": Gain Delay one frame;"01","10" RSVD.
