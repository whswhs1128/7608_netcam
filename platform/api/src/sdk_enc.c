...
//                    }		xqq
                ...
                //一直开启mjeg的那一路编码，同时抓拍的时候不停编码，防止osd出错
                #if 0
                if((encodeType == 2) )
                {
...
int sdk_enc_set_stream_h264(int vin, int stream, PS_GK_ENC_STREAM_H264_ATTR h264_attr)
{
    ...
    int chage_fps = 0;
    int bpsLimit;
    /*该标志决定是否需要重启编码器，一般只在设置分辨率时，需要重启编码，设置码率，fps都不需要重启编码*/
...
/*开始音视频流的读取线程*/
/* 音频和视频分离，因为消回音算法中音频包很频繁，所以2个线程要分开 */
...
/* 截图功能，根据传入的宽，高来决定从最接近的通道上获取数据，若宽，高不与编码大小一致，则会从大于该高宽的编码通道上截取部分图像*/
...
//#else		xqq
    ...
    #if 1
    /* 因为160字节一个音频，做一个处理，让2个160字节的音频帧组成一个新的320字节的音频帧 */
    if (abuf == NULL) {
