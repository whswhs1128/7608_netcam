...
static irqreturn_t gk_sd_irq(int irq, void *devid)
{
    ...
    static u32 erro_cnt = 0;
    u32 need_reset_all = 0;
    u16 nis=0;
    u16 eis=0;
//    u32 enabled = 0;
    int handle = 0;
        ...
        if (eis & (SD_EIS_CMD_TMOUT_ERR | SD_EIS_CMD_CRC_ERR |
            ...
            if(!(eis &SD_EIS_CMD_TMOUT_ERR))
            {
                ...
                if(erro_cnt >= 10)
                {
                    erro_cnt = 0;
                    need_reset_all = true;
                    printk("erro_cnt >= 10,need reset all\n");
                }
                printk("-- nis = 0x%x eis = 0x%x\n", nis, eis);
            }
        }
    ...
    struct gk_sd_mmc_info *pslotinfo)
{
    ...
    if (pslotinfo->mrq->data)
    {
        if(0 != gk_sd_dat_line_isvalid(pinfo)||
            0 != gk_sd_cmd_line_isvalid(pinfo))
        {
...
{
    int retval = 0;
    printk("mmc test version:20191113-1!\n");
