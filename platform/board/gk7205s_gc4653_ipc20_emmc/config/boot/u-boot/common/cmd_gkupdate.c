...
void backups_update_write_zone(void* data, int addr, int len)
{
    cmd_tbl_t * cmdtp = NULL;
    int flag =2,argc;
    int i = 0;
...
void gkupdate_check(void)
{
	...
	#endif
	if((old_head.update_flag == 1) && (old_head.partitnon_addr < mtd_size_all) && (old_head.partitnon_size < mtd_size_all))
	{
		gk_read_update_zone(buffer, old_buff_offset, old_head.partitnon_size);
        ...
        else
        {
            ...
            if(crc != old_head.partitnon_crc)
            {
                ...
                if((crc != new_head.partitnon_crc) || (new_head.magic != BACKUPS_HEAD_MAGIC))
                {
                    /*reset partitnon data */
                    printf("read backups data\n");
                    gk_read_update_zone(buffer, old_buff_offset, old_head.partitnon_size);
                    printf("write backups data 2\n");
                    backups_update_write_zone(buffer, old_head.partitnon_addr, old_head.partitnon_size);
                    printf("write backups data end\n");
                }
                else
                    printf("new partitnon crc is ok\n");
            }
        }
		...
		backups_update_write_zone(old_head_buffer, old_head_offset, OLD_HEAD_SIZE);
	}
}
